...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	*/
	...
	forecast: {
		...
		onModifyAtkPriority: 5,
		onModifyAtk(atk, pokemon) {
			if (this.field.isWeather(['hail', 'snow', 'sunnyday', 'desolateland', 'raindance', 'primordialsea', 'sandstorm'])) {
				return this.chainModify(1.5);
			}
		},
	   onModifyDefPriority: 6,
	   ...
	   onModifySpD(spd, pokemon) {
		...
		rating: 2,
		num: 59,
		shortDesc: "Castform's type changes to the current weather's, except Sandstorm. 1.5x to all stats in weather.",
	},
	forewarn: {
		onStart(pokemon) {
			let warnMoves: (Move | Pokemon)[][] = [];
	...
	hustle: {
		...
		rating: 3.5,
		num: 55,
		shortDesc: "This Pokemon's Attack is 2x and accuracy of its physical attacks is 0.8x.",
	},
	hydration: {
		onResidualOrder: 5,
		onResidualSubOrder: 3,
	...
	merciless: {
		...
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				if (!activated) {
					this.add('-ability', pokemon, 'Merciless');
					activated = true;
				}
				if (target.volatiles['substitute']) {
					this.add('-immune', target);
		...
		shortDesc: "This Pokemon's attacks are critical hits. On switch, target is poisoned.",
		flags: {},
		name: "Merciless",
		rating: 1.5,
		num: 196,
	...
	*/
	...
	slowstart: {
		...
		condition: {
			...
			onStart(target) {
				this.add('-start', target, 'ability: Slow Start');
				this.add('-message', `This Pokemon's Attack and Speed will be halved in 5 turns!`);
			},
			onEnd(target) {
				this.add('-end', target, 'Slow Start');
				target.addVolatile('slowstartactive');
			},
		},
		shortDesc: "On switch-in, this Pokemon's Attack and Speed are halved in 5 turns.",
		flags: {},
		name: "Slow Start",
		rating: 0,
		num: 112,
	...
	slowstartactive: {
		onStart(pokemon) {
			pokemon.addVolatile('slowstartactive');
		},
		condition: {
			onStart(target) {
				this.add('-message', `This Pokemon's Attack and Speed are now halved!`);
			},
			onModifyAtkPriority: 5,
			onModifyAtk(atk, pokemon) {
				return this.chainModify(0.5);
		...
		shortDesc: "This Pokemon's Attack and Speed are halved.",
		flags: {},
		name: "Slow Start Active",
		rating: -1,
		num: 112.1,
	},
	slushrush: {
	...
	*/
	...
	terashift: {
		...
		onStart(pokemon) {
			...
			const target = pokemon.side.foe.active[pokemon.side.foe.active.length - 1 - pokemon.position];
			if (target) {
				pokemon.transformInto(target, this.dex.abilities.get('terashift'));
			}
			this.effectState.switchingIn = false;
		},
		flags: {failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1, notransform: 1},
		...
		rating: 3,
		num: 307,
		shortDesc: "If this Pokemon is a Terapagos, it transforms on entry.",
	},
	teravolt: {
		onStart(pokemon) {
			this.add('-ability', pokemon, 'Teravolt');
	...
	victorystar: {
		...
		rating: 2,
		num: 162,
		shortDesc: "This Pokemon and its allies' moves have their accuracy multiplied by 11.",
	},
	vitalspirit: {
		onUpdate(pokemon) {
			if (pokemon.status === 'slp') {
