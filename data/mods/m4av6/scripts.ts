...
export const Scripts: ModdedBattleScriptsData = {
	...
	runMoveEffects(
		...
		for (const [i, target] of targets.entries()) {
			...
			// Blackmail / Orderly Target section (WIP)
			const target = source.side.foe.active[source.side.foe.active.length - 1 - source.position];
			if (moveData.selfdestruct === 'ifHit' && damage[i] !== false && source.hasAbility('orderlytarget') && target.hasAbility('blackmail')
			&& target.hp && !target.fainted) {
