...
exports.BattleStatuses = {
	...
	necrosis: {
		...
		onStart: function (target, source, sourceEffect) {
			if (sourceEffect && sourceEffect.effectType === 'Ability') {
				this.add('-status', target, 'necrosis', '[from] ability: ' + sourceEffect.name, '[of] ' + source);
			} else {
				this.add('-status', target, 'necrosis');
			}
		},
	...
	trapped: {
		...
		onResidualOrder: 11,
		onResidual: function (pokemon) {
			if (pokemon.side.sideConditions['warpguard']) {
	...
	partiallytrapped: {
		...
		onResidualOrder: 11,
		onResidual: function (pokemon) {
			if (this.effectData.source && (!this.effectData.source.isActive || this.effectData.source.hp <= 0 || !this.effectData.source.activeTurns || pokemon.side.sideConditions['warpguard'])) {
	...
	raindance: {
		...
		durationCallback: function (source, effect) {
			...
			if (source && source.hasAbility('skycontrol')) {
				return 8;
			}
			if (source && source.hasAbility('skycontrol') && source.hasItem('damprock')) {
				return 11;
			}
			return 5;
		},
	...
	sunnyday: {
		...
		durationCallback: function (source, effect) {
			...
			if (source && source.hasAbility('skycontrol') && source.hasItem('heatrock')) {
				return 11;
			}
			return 5;
		},
	...
	sandstorm: {
		...
		durationCallback: function (source, effect) {
			...
			if (source && source.hasAbility('skycontrol') && source.hasItem('smoothrock')) {
				return 11;
			}
			return 5;
		},
	...
	hail: {
		...
		durationCallback: function (source, effect) {
			...
			if (source && source.hasAbility('skycontrol') && source.hasItem('icyrock')) {
				return 11;
			}
			return 5;
		},
	...
	frozenwasteland: {
		...
		duration: 0,
		onTryMove: function (target, source, effect) {
			if (effect.type === 'Fighting' && effect.category !== 'Status') {
				this.debug('Frozen Wasteland fighting suppress');
				this.add('-fail', source, effect, '[from] Frozen Wasteland');
				return null;
			}
		},
		onWeatherModifyDamage: function (damage, attacker, defender, move) {
		...
		onStart: function (battle, source, effect) {
			this.add('-weather', 'FrozenWasteland', '[from] ability: ' + effect, '[of] ' + source);
		},
		onResidualOrder: 1,
		onResidual: function () {
			this.add('-weather', 'FrozenWasteland', '[upkeep]');
			this.eachEvent('Weather');
		},
		onWeather: function (target) {
			this.damage(target.maxhp / 8);
