...
try {
 ...
 function writeTickets() {
	_fs.FS.call(void 0, TICKET_FILE).writeUpdate(
		() => JSON.stringify(exports.tickets)
	);
...
function writeStats(line) {
 ...
 class HelpTicket extends Rooms.RoomGame {
	...
	static ban(user, reason = '') {
		...
		const punishment = ['TICKETBAN', userid, Date.now() + TICKET_BAN_DURATION, reason];
	...
	static checkBanned(user) {
		...
		for (const ip of user.ips) {
			const curPunishment = _optionalChain([Punishments, 'access', _6 => _6.roomIps, 'access', _7 => _7.get, 'call', _8 => _8('staff'), 'optionalAccess', _9 => _9.get, 'call', _10 => _10(ip)]);
...
function notifyUnclaimedTicket(hasAssistRequest) {
 ...
 function notifyStaff() {
	...
	if (!room) return;
	let buf = ``;
	const keys = Object.keys(exports.tickets).sort((aKey, bKey) => {
		const a = exports.tickets[aKey];
		const b = exports.tickets[bKey];
		if (a.offline) {
	...
	for (const key of keys) {
		const ticket = exports.tickets[key];
		if (!ticket.open) continue;
		if (!ticket.active) continue;
		if (count >= 3) {
			hiddenTicketCount++;
...
const ticketPages = {
 ...
 const pages = {
	help: {
		request(query, user, connection) {
			...
			if (_optionalChain([ticket, 'optionalAccess', _11 => _11.open]) || ipTicket) {
 ...
 const commands = {
	report(target, room, user) {
		...
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _12 => _12.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	appeal(target, room, user) {
		...
		const meta = this.pmTarget ? `-user-${this.pmTarget.id}` : this.room ? `-room-${this.room.roomid}` : '';
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _13 => _13.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	helpticket: {
		...
		submit(target, room, user, connection) {
			...
			let ticket = exports.tickets[user.id];
			const ipTicket = checkIp(user.latestIp);
			if (_optionalChain([ticket, 'optionalAccess', _14 => _14.open]) || ipTicket) {
				if (!ticket && ipTicket) ticket = ipTicket;
				const helpRoom = Rooms.get(`help-${ticket.userid}`);
				if (!helpRoom) {
					// Should never happen
			...
			const introMsg = _utils.Utils.html`<h2 style="margin-top:0">${this.tr`Help Ticket`} - ${user.name}</h2>` +
		...
		ban(target, room, user) {
			...
			for (const userObj of affected) {
				...
				if (_optionalChain([targetTicket, 'optionalAccess', _15 => _15.open])) targetTicket.open = false;
 ...
 const punishmentfilter = (user, punishment) => {
	...
	if (_optionalChain([helpRoom, 'optionalAccess', _16 => _16.game, 'optionalAccess', _17 => _17.gameid]) !== 'helpticket') return;
