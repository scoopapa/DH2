...
export class RandomGen2Teams extends RandomGen3Teams {
	...
	randomSet(species: string | Species, restrictMoves: {[k: string]: number}): RandomTeamsTypes.RandomSet {
		...
		let discard = false;
		let rerollsLeft = 3;
		const isPhazingMove = (move: string) => (move === "roar" || move === "whirlwind");
		const isSleepMove = (move: string) => (move === "sleeppowder" || move === "lovelykiss" || move === "sing" || move === "hypnosis" || move === "spore");
