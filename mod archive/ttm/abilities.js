...
exports.BattleAbilities = {
	...
	"compassionate": {
		desc: "All non-damaging moves that check accuracy have their accuracy changed to 50% when used on this Pokemon. This change is done before any other accuracy modifying effects.",
		shortDesc: "Increases normal moves' BP by 1.2x, changes them to Heart-type",
		onModifyMovePriority: -1,
		onModifyMove: function (move, pokemon) {
			if (move.type === 'Normal' && move.id !== 'naturalgift' && !move.isZ) {
				move.type = 'Heart';
				if (move.category !== 'Status') pokemon.addVolatile('compassionate');
			}
		},
		effect: {
			duration: 1,
	...
	"nightinggalepledge": {
		...
		onUpdate: function (pokemon) {
			if (pokemon.volatiles['taunt']) {
				this.add('-activate', pokemon, 'ability: Nightingale Pledge');
				pokemon.removeVolatile('taunt');
				// Taunt's volatile already sends the -end message when removed
			}
			if (pokemon.volatiles['embargo']) {
				...
				// Embargo's volatile already sends the -end message when removed
			}
			if (pokemon.volatiles['torment']) {
				this.add('-activate', pokemon, 'ability: Nightingale Pledge');
				pokemon.removeVolatile('torment');
				// Torment's volatile already sends the -end message when removed
			}
		},
		onTryHit: function (pokemon, target, move) {
			if (move.id === 'torment' || move.id === 'embargo' || move.id === 'taunt') {
				this.add('-immune', pokemon, '[msg]', '[from] ability: Nightingale Pledge');
				return null;
			}
			if (move.type === 'Dark') {
				this.add('-immune', target, '[msg]', '[from] ability: Nightingale Pledge');
				return null;
			}
		},
		onAllyTryHitSide: function (target, source, move) {
	...
	"heartless": {
		desc: "Immune to Heart-type moves; when hit by one, +1 Atk.",
		shortDesc: "Immune to Heart-type moves; when hit by one, +1 Atk.",
		onTryHitPriority: 1,
		onTryHit: function (target, source, move) {
			if (target !== source && move.type === 'Heart') {
				if (!this.boost({atk:1})) {
					this.add('-immune', target, '[msg]', '[from] ability: Heartless');
				}
				return null;
			}
		},
	...
	"momentsnotice": {
		shortDesc: "The Pokemon's first move after entering battle gets +1 priority.",
		onModifyPriority: function (priority, pokemon, target, move) {
	...
	"patience": {
		desc: "This Pokemon is immune to Time-type moves and raises its Speed by 1 stage when hit by an Time-type move.",
		shortDesc: "This Pokemon's Speed is raised 1 stage if hit by an Time move; Time immunity.",
		onTryHit: function (target, source, move) {
			if (target !== source && move.type === 'Time') {
				if (!this.boost({spe:1})) {
					this.add('-immune', target, '[msg]', '[from] ability: Patience');
				}
				return null;
			}
		},
	...
	"temporalize": {
		desc: "Increases normal moves' BP by 1.2x, changes them to Time-type",
		shortDesc: "Increases normal moves' BP by 1.2x, changes them to Time-type",
		onModifyMovePriority: -1,
		onModifyMove: function (move, pokemon) {
			if (move.type === 'Normal' && move.id !== 'naturalgift' && !move.isZ) {
				move.type = 'Time';
				if (move.category !== 'Status') pokemon.addVolatile('temporalize');
			}
		},
		effect: {
			duration: 1,
		...
		onTryHit: function (target, source, move) {
			if (target !== source && move.type === 'Time') {
				if (!this.heal(target.maxhp / 8)) {
					this.add('-immune', target, '[msg]', '[from] ability: Immortal');
				}
				return null;
			}
		},
	...
	"zerogravity": {
		shortDesc: "Status moves will never miss (unless target is semi-invulnerable). ",
		onModifyMove: function (move) {
			if (move.category === 'Status' && typeof move.accuracy === 'number') {
				move.accuracy *= 100;
			}
	...
	"skycontrol": {
		shortDesc: "Weather-setting moves last three turns longer than they normally would.",
		id: "skycontrol",
		name: "skycontrol",
		rating: 0,
		num: 35,
	...
	"astralize": {
		desc: "This Pokemon's Normal-type moves become Space-type moves and have their power multiplied by 1.2. This effect comes after other effects that change a move's type, but before Ion Deluge and Electrify's effects.",
		shortDesc: "This Pokemon's Normal-type moves become Space type and have 1.2x power.",
		onModifyMovePriority: -1,
		onModifyMove: function (move, pokemon) {
			if (move.type === 'Normal' && move.id !== 'naturalgift' && !move.isZ) {
				move.type = 'Space';
				if (move.category !== 'Status') pokemon.addVolatile('astralize');
			}
		},
		effect: {
			duration: 1,
