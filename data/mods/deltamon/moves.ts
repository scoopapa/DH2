export const Moves: {[moveid: string]: ModdedMoveData} = {
	...
	*/
	...
	omegaperseverance: {
		...
		accuracy: true,
		pp: 1,
		shortDesc: "Dispels target's stat changes. Disables target's ability before damage. 50% confuse chance.",
	...
	omegajustice: {
		...
		accuracy: true,
		pp: 1,
		shortDesc: "Always critical hits, bypasses Substitute, ignores abilities.",
	...
	omegabravery: {
		...
		accuracy: true,
		pp: 1,
		shortDesc: "Hits 10 times. Priority +2. Prevents user from switching out.",
	...
	omegakindness: {
		...
		accuracy: true,
		pp: 1,
		shortDesc: "Full heal, clears negative boosts, restores PP, status + confusion.",
	...
	omegaintegrity: {
		...
		accuracy: true,
		pp: 1,
		shortDesc: "Defog effect for both sides. Sets a rainbow for 4 turns.",
		...
		onHit(target, source, move) {
				...
				for (const targetCondition of removeTarget) {
					if (target.side.removeSideCondition(targetCondition)) {
						...
						this.add('-sideend', target.side, this.dex.conditions.get(targetCondition).name, '[from] move: Omega Integrity', '[of] ' + source);
						success = true;
					}
				}
	...
	omegapatience: {
		...
		accuracy: true,
		pp: 1,
		shortDesc: "Hits 4 turns after use. Ignores stat changes. Fails if a Future Move is active.",
	...
	mewmewwand: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Pink: Psychic, 50% -Sp. Atk, Corporeal. Ghost, 50% -Atk, Ghost.",
