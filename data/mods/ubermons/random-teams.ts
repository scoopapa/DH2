...
export class RandomTeams {
	...
	randomCCTeam(): RandomTeamsTypes.RandomSet[] {
		...
		const items = Object.keys(this.dex.items.all());
		...
		for (let i = 0; i < 6; i++) {
			...
			if (this.gen >= 2) {
				...
				} while (this.dex.items.get(item).gen > this.gen || this.dex.items.all()[item].isNonstandard);
			}
			...
			let pool = ['struggle'];
			if (forme === 'Smeargle') {
				pool = Object.keys(this.dex.moves.all()).filter(moveid => {
					const move = this.dex.moves.all()[moveid];
	...
	random6Pokemon() {
		...
		for (const id in this.dex.data.FormatsData) {
			if (!this.dex.species.all()[id] || this.dex.data.FormatsData[id].isNonstandard && this.dex.data.FormatsData[id].isNonstandard !== 'Unobtainable') continue;
			const num = this.dex.species.all()[id].num;
		...
		const formes: string[][] = [[], [], [], [], [], []];
		for (const id in this.dex.species.all()) {
	...
	randomHCTeam(): PokemonSet[] {
		...
		const itemPool = Object.keys(this.dex.items.all());
		const abilityPool = Object.keys(this.dex.abilities.all());
		const movePool = Object.keys(this.dex.moves.all());
		...
		for (let i = 0; i < 6; i++) {
			...
			if (this.gen >= 3) {
				...
				} while (this.dex.abilities.get(ability).gen > this.gen || this.dex.abilities.all()[ability].isNonstandard);
			}
	...
	queryMoves(moves: string[] | null, hasType: {[k: string]: boolean} = {}, hasAbility: {[k: string]: boolean} = {}, movePool: string[] = []) {
		...
		const PhysicalSetup = [
			'bellydrum', 'bulkup', 'coil', 'curse', 'dragondance', 'honeclaws', 'howl', 'poweruppunch', 'swordsdance', 'shiftgear',
		];
	...
	randomSet(species: string | Species, teamDetails: RandomTeamsTypes.TeamDetails = {}, isLead = false, isDoubles = false): RandomTeamsTypes.RandomSet {
		...
		do {
			...
			for (const [k, moveId] of moves.entries()) {
				...
				case 'acrobatics': case 'astralbarrage': case 'junglehealing':
		...
		if (abilities[1]) {
			...
			if (species.name === 'Azumarill' && !isDoubles) {
				...
				ability = 'Guts';
			} else if (hasAbility['Intimidate'] && hasMove['guillotine']) {
				ability = 'Intimidate';
		...
		} else if (forme === 'Kyogre') {
			ability = 'Drizzle';
		} else if (forme === 'Groudon') {
			ability = 'Drought';
		...
		} else if (hasMove['meteorbeam'] || hasMove['fissure']) {
			item = 'Power Herb';
		...
		} else if (hasMove['acrobatics'] || hasMove['astralbarrage']) {
			item = (ability === 'Grassy Surge') ? 'Grassy Seed' : '';
		} else if (ability === 'Gorilla Tactics') {
			item = (hasMove['switcheroo']) ? 'Toxic Orb' : 'Sticky Barb';
