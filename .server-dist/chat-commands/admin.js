...
var _processmanager = require('../../.lib-dist/process-manager'); var ProcessManager = _processmanager;

function getDisabledCommands(table = Chat.commands) {
	const disabled = [];
	for (const k in table) {
		...
		if (typeof handler === 'function' && (handler ).disabled) {
			disabled.push(k);
		}
	}
	return disabled;
...
async function rebuild(context) {
 ...
 const commands = {
	...
	addhtmlbox(target, room, user, connection, cmd) {
		...
		target = Chat.collapseLineBreaksHTML(target);
		if (!user.can('addhtml')) {
			target += _utils.Utils.html`<div style="float:right;color:#888;font-size:8pt">[${user.name}]</div><div style="clear:both"></div>`;
		}
	...
	addrankhtmlbox(target, room, user, connection, cmd) {
		...
		if (!user.can('addhtml')) {
			html += _utils.Utils.html`<div style="float:right;color:#888;font-size:8pt">[${user.name}]</div><div style="clear:both"></div>`;
		}
	...
	sendhtmlpage(target, room, user) {
		room = this.requireRoom();
		this.checkCan('addhtml', null, room);
		let [targetID, pageid, content] = _utils.Utils.splitFirst(target, ',', 2);
	...
	highlighthtmlpage(target, room, user) {
		target = target.trim();
		let [userid, pageid, title, highlight] = _utils.Utils.splitFirst(target, ',', 3);
		pageid = `${user.id}-${toID(pageid)}`;
	...
	async hotpatch(target, room, user, connection, cmd) {
		...
		let patch = target;
		try {
			_utils.Utils.clearRequireCache({exclude: ['/.lib-dist/process-manager']});
			if (target === 'all') {
				if (lock['all']) {
				...
				for (const manager of processManagers.slice()) {
					if (
						manager.filename.startsWith(_fs.FS.call(void 0, 'server/chat-plugins').path) ||
						manager.filename.startsWith(_fs.FS.call(void 0, '.server-dist/chat-plugins').path)
					) {
				...
				_fs.FS.call(void 0, 'config/custom.css').unwatch();
				...
				const processManagers = ProcessManager.processManagers;
				for (const manager of processManagers.slice()) {
					if (manager.filename.startsWith(_fs.FS.call(void 0, '.server-dist/modlog').path)) void manager.destroy();
				}
	...
	async savelearnsets(target, room, user, connection) {
		...
		await _fs.FS.call(void 0, 'data/learnsets.js').write(`'use strict';\n\nexports.Learnsets = {\n` +
			Object.entries(Dex.data.Learnsets).map(([id, entry]) => (
				`\t${id}: {learnset: {\n` +
				Object.entries(Dex.getLearnsetData(id )).sort(
	...
	kill(target, room, user) {
		...
		if (!_optionalChain([logRoom, 'optionalAccess', _2 => _2.log, 'access', _3 => _3.roomlogStream])) return process.exit();
	...
	async updateserver(target, room, user, connection) {
		...
		const exec = (command) => bash(command, this);
	...
	async eval(target, room, user, connection) {
		...
		const generateHTML = (direction, contents) => (
			`<table border="0" cellspacing="0" cellpadding="0"><tr><td valign="top">` +
				_utils.Utils.escapeHTML(direction).repeat(2) +
				`&nbsp;</td><td>${Chat.getReadmoreCodeBlock(contents)}</td></tr><table>`
		);
		this.sendReply(`|html|${generateHTML('>', target)}`);
		_optionalChain([logRoom, 'optionalAccess', _4 => _4.roomlog, 'call', _5 => _5(`>> ${target}`)]);
		let uhtmlId = null;
		try {
			...
			if (_optionalChain([result, 'optionalAccess', _6 => _6.then])) {
				uhtmlId = `eval-${room.nextGameNumber()}`;
				this.sendReply(`|uhtml|${uhtmlId}|${generateHTML('<', 'Promise pending')}`);
				this.update();
				result = `Promise -> ${_utils.Utils.visualize(await result)}`;
			...
			_optionalChain([logRoom, 'optionalAccess', _7 => _7.roomlog, 'call', _8 => _8(`<< ${result}`)]);
		} catch (e) {
			...
			_optionalChain([logRoom, 'optionalAccess', _9 => _9.roomlog, 'call', _10 => _10(`<< ${message}`)]);
		}
	},
