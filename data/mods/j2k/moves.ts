...
export const Moves: {[moveid: string]: MoveData} = {
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeTarget = [
				'reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'fog',
			];
			const removeAll = [
	...
	bellofpeace: {
		...
		self: {
			onHit(target, source, move) {
				for (const pokemon of source.side.active) {
					this.heal(pokemon.maxhp / 4, pokemon, source, move);
				}
			},
		},
