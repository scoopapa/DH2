 ...
 const commands = {
	...
	modjoin(target, room, user) {
		...
		if (target === 'sync' && !room.settings.modchat) {
			const lowestStaffGroup = Config.groupsranking.filter(group => _optionalChain([Config, 'access', _2 => _2.groups, 'access', _3 => _3[group], 'optionalAccess', _4 => _4.mute]))[0];
	...
	permissions: {
		...
		clickset(target, room, user) {
			const [roomid, newTarget] = _utils.Utils.splitFirst(target, ',');
		...
		''(target, room, user) {
			...
			const permissions = allPermissions.filter(perm => {
				const handler = _optionalChain([Chat, 'access', _5 => _5.parseCommand, 'call', _6 => _6(perm), 'optionalAccess', _7 => _7.handler]);
				if (_optionalChain([handler, 'optionalAccess', _8 => _8.isPrivate]) && !user.can('lock')) return false;
			...
			const subPermissions = allPermissions.filter(perm => perm.startsWith('/') && perm.includes(' ')).filter(perm => {
				const handler = _optionalChain([Chat, 'access', _9 => _9.parseCommand, 'call', _10 => _10(perm), 'optionalAccess', _11 => _11.handler]);
				if (_optionalChain([handler, 'optionalAccess', _12 => _12.isPrivate]) && !user.can('lock')) return false;
				return perm.startsWith('/') && perm.includes(' ');
			});
	...
	unsubroom(target, room, user) {
		...
		if (_optionalChain([parent, 'optionalAccess', _16 => _16.subRooms])) {
			...
			if (room.auth.atLeast(user, '#')) {
				buf += roomGroups.map(group => (
					requiredRank === group ?
						...
						_utils.Utils.html`<button class="button" name="send" value="/permissions clickset ${room.roomid}, ${permission}, ${group}">${group}</button>`
				)).join(' ');
			} else {
