...
function readFile(path) {
	try {
		const json = _fs.FS.call(void 0, path).readIfExistsSync();
		if (!json) {
			return false;
		}
...
class MafiaPlayer extends Rooms.RoomGamePlayer {
	...
	constructor(user, game) {
		super(user, game);
		this.game = game;
		this.safeName = _utils.Utils.escapeHTML(this.name);
		this.role = null;
		this.lynching = '';
...
class MafiaTracker extends Rooms.RoomGame {
	...
	constructor(room, host) {
		...
		this.hostid = host.id;
		this.host = _utils.Utils.escapeHTML(host.name);
		this.cohosts = [];
	...
	setRoles(user, roleString, force = false, reset = false) {
		...
		if (force) {
			this.originalRoles = roles.map(r => ({
				name: r,
				safeName: _utils.Utils.escapeHTML(r),
				id: toID(r),
				alignment: 'solo',
				image: '',
	...
	static parseRole(roleString) {
		...
		const role = {
			name: roleName,
			safeName: _utils.Utils.escapeHTML(roleName),
			id: toID(roleName),
			image: '',
	...
	distributeRoles() {
		const roles = _utils.Utils.shuffle(this.roles.slice());
		if (roles.length) {
			for (const p in this.playerTable) {
				...
				if (_optionalChain([u, 'optionalAccess', _ => _.connected])) {
	...
	night(early = false, initial = false) {
		...
		for (const hostid of [...this.cohosts, this.hostid]) {
			...
			if (_optionalChain([host, 'optionalAccess', _2 => _2.connected])) host.send(`>${this.room.roomid}\n|notify|It's night in your game of Mafia!`);
		}
	...
	lynch(userid, target) {
		...
		if (this.getHammerValue(target) <= lynch.trueCount) {
			// HAMMER
			this.sendDeclare(`Hammer! ${target === 'nolynch' ? 'Nobody' : _utils.Utils.escapeHTML(name)} was lynched!`);
	...
	revive(user, toRevive, force = false) {
		...
		if (toRevive in this.dead) {
			...
			this.sendDeclare(_utils.Utils.html`${targetUser.name} has been added to the game by ${user.name}!`);
		}
		this.playerCount++;
		this.updateRoleString();
		this.updatePlayers();
	...
	sub(player, replacement) {
		...
		if (_optionalChain([newUser, 'optionalAccess', _3 => _3.connected])) {
	...
	customIdeaInit(user, choices, picks, rolesString) {
		...
		const roles = _utils.Utils.stripHTML(rolesString);
	...
	ideaDistributeRoles(user) {
		...
		for (const p in this.playerTable) {
			...
			const u = Users.get(p);
			if (_optionalChain([u, 'optionalAccess', _4 => _4.connected])) u.send(`>${this.room.roomid}\n|notify|Pick your role in the IDEA module.`);
		}
 ...
 const pages = {
	mafia(query, user) {
		...
		const game = _optionalChain([room, 'optionalAccess', _5 => _5.getGame, 'call', _6 => _6(MafiaTracker)]);
 ...
 const commands = {
	mafia: {
		...
		customidea(target, room, user) {
			...
			const [options, roles] = _utils.Utils.splitFirst(target, '\n');
		...
		revive(target, room, user, connection, cmd) {
			...
			let targetRoom = Rooms.get(args[0]);
			if (!targetRoom || targetRoom.type !== 'chat' || !targetRoom.users[user.id]) {
				if (_optionalChain([room, 'optionalAccess', _7 => _7.type]) !== 'chat') return this.errorReply(`This command is only meant to be used in chat rooms.`);
				targetRoom = room;
			} else {
				args.shift();
			}
		...
		nighttalk(target, room, user, connection, cmd) {
			room = this.requireRoom();
			const game = _optionalChain([room, 'optionalAccess', _8 => _8.getGame, 'call', _9 => _9(MafiaTracker)]);
			if (!game) return this.errorReply(`There is no game of mafia running in this room.`);
			if (game.hostid !== user.id && !game.cohosts.includes(user.id)) this.checkCan('mute', null, room);
			if (!game.started) return this.errorReply(`The game has not started yet.`);
		...
		priest(target, room, user, connection, cmd) {
			room = this.requireRoom();
			const game = _optionalChain([room, 'optionalAccess', _10 => _10.getGame, 'call', _11 => _11(MafiaTracker)]);
			if (!game) return this.errorReply(`There is no game of mafia running in this room.`);
			if (game.hostid !== user.id && !game.cohosts.includes(user.id)) this.checkCan('mute', null, room);
			if (!game.started) return this.errorReply(`The game has not started yet.`);
		...
		view(target, room, user, connection) {
			room = this.requireRoom();
			const game = _optionalChain([room, 'optionalAccess', _12 => _12.getGame, 'call', _13 => _13(MafiaTracker)]);
			if (!game) return this.errorReply(`There is no game of mafia running in this room.`);
			if (!this.runBroadcast()) return;
			if (this.broadcasting) {
		...
		autosub(target, room, user) {
			...
			let targetRoom = Rooms.get(args[0]);
			if (!targetRoom || targetRoom.type !== 'chat' || !targetRoom.users[user.id]) {
				if (_optionalChain([room, 'optionalAccess', _14 => _14.type]) !== 'chat') return this.errorReply(`This command is only meant to be used in chat rooms.`);
				targetRoom = room;
			} else {
				args.shift();
			}
		...
		subhost(target, room, user, connection, cmd) {
			...
			if (cmd.includes('cohost')) {
				...
				game.sendDeclare(_utils.Utils.html`${targetUser.name} has been added as a cohost by ${user.name}`);
				for (const conn of targetUser.connections) {
					void Chat.resolvePage(`view-mafia-${room.roomid}`, targetUser, conn);
				}
				...
				const queueIndex = hostQueue.indexOf(targetUser.id);
				if (queueIndex > -1) hostQueue.splice(queueIndex, 1);
				game.host = _utils.Utils.escapeHTML(targetUser.name);
				game.hostid = targetUser.id;
				game.played.push(targetUser.id);
				for (const conn of targetUser.connections) {
					void Chat.resolvePage(`view-mafia-${room.roomid}`, targetUser, conn);
				}
				game.sendDeclare(_utils.Utils.html`${targetUser.name} has been substituted as the new host, replacing ${oldHostid}.`);
		...
		removecohost(target, room, user) {
			...
			game.sendDeclare(_utils.Utils.html`${target} was removed as a cohost by ${user.name}`);
		...
		data(target, room, user, connection, cmd) {
			if (_optionalChain([room, 'optionalAccess', _15 => _15.settings, 'access', _16 => _16.mafiaDisabled])) return this.errorReply(`Mafia is disabled for this room.`);
			...
			// hack to let hosts broadcast
			const game = _optionalChain([room, 'optionalAccess', _17 => _17.getGame, 'call', _18 => _18(MafiaTracker)]);
		...
		addtheme(target, room, user, connection, cmd) {
			...
			for (const rolelist of rolelists) {
				const [players, roles] = _utils.Utils.splitFirst(rolelist, ':', 2).map(e => e.trim());
