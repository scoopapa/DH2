	...
	*/
	...
	dummythicc: {
		...
		onDamage(damage, target, source, effect) {
			if (target.hp === target.maxhp && damage >= target.hp && _optionalChain([effect, 'optionalAccess', _ => _.effectType]) === 'Move') {
		...
		onEnd(pokemon) {
			if (pokemon.illusion) {
				...
				if (!pokemon.volatiles[toID(pokemon.name)]) {
					...
					if (_optionalChain([status, 'optionalAccess', _2 => _2.exists])) {
	...
	optimize: {
		...
		onBeforeMove(pokemon, target, move) {
			switch (move.id) {
			case 'psychoboost':
				if (pokemon.species.name === 'Deoxys-Attack') return;
				...
				break;
			case 'recover':
				if (pokemon.species.name === 'Deoxys-Defense') return;
				...
				break;
			case 'extremespeed':
				if (pokemon.species.name === 'Deoxys-Speed') return;
				...
				break;
			case 'refactor':
				if (pokemon.species.name === 'Deoxys') return;
				pokemon.set.nature = 'Bold';
				pokemon.set.ivs = {hp: 31, atk: 0, def: 31, spa: 31, spd: 31, spe: 31};
	...
	beastboost2: {
		...
		onSourceFaint(target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _5 => _5.effectType]) === 'Move') {
				const statOrder = Object.keys(source.storedStats).sort(
					(stat1, stat2) => source.storedStats[stat2 ] - source.storedStats[stat1 ]
				);
	...
	illusion: {
		...
		onEnd(pokemon) {
			if (pokemon.illusion) {
				...
				if (!pokemon.volatiles[toID(pokemon.name)]) {
					const status = this.dex.getEffect(toID(pokemon.name));
					if (_optionalChain([status, 'optionalAccess', _7 => _7.exists])) {
						pokemon.addVolatile(toID(pokemon.name), pokemon);
					}
				}
			}
