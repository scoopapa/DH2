export const Pokedex: {[k: string]: ModdedSpeciesData} = {
	melmetal: {
		inherit: true,
		name: "Melmetal",
		num: -1,
		types: ["Steel"],
		baseStats: {hp: 135, atk: 143, def: 143, spa: 80, spd: 65, spe: 34},
		abilities: {0: "What the Sigma", H: "Iron Fist"},
	},
	ninjask: {
		inherit: true,
		gen: 3,
		copen: true,
		name: "Ninjask",
		num: -2,
		types: ["Bug", "Flying"],
		baseStats: {hp: 61, atk: 90, def: 45, spa: 50, spd: 50, spe: 160},
		abilities: {0: "Ultra Luck", 1: "Speed Boost", H: "Infiltrator"},
	},
	toxapex: {
		inherit: true,
		name: "Toxapex",
		num: -3,
		fish: true,
		types: ["Poison", "Water"],
		baseStats: {hp: 152, atk: 142, def: 53, spa: 50, spd: 63, spe: 35},
		abilities: {0: "Merciless", 1: "Gex Server", H: "Degenerator"},
	},
	silcoon: {
		inherit: true,
		gen: 3,
		name: "Silcoon",
		num: -4,
		fish: true,
		bird: true,
		types: ["Ground", "Flying"],
		baseStats: {hp: 89, atk: 145, def: 90, spa: 105, spd: 80, spe: 91},
		abilities: {0: "Intimidate", 1: "just the birds", H: "toxic masculinity"},
		evos: null,
	},
	togedemaru: {
		inherit: true,
		name: "Togedemaru",
		num: -5,
		types: ["Electric", "Steel"],
		baseStats: {hp: 100, atk: 50, def: 100, spa: 50, spd: 100, spe: 133},
		abilities: {0: "Iron Barbs", 1: "Lightning Rod", H: "Benevolent Blessing"},
	},
	bramblin: {
		inherit: true,
		name: "Bramblin",
		num: -6,
		bird: true,
		types: ["Grass", "Ghost"],
		baseStats: {hp: 75, atk: 125, def: 60, spa: 75, spd: 70, spe: 120},
		abilities: {0: "Wind Rider", 1: "!dt air slash", H: "Monster Mash"},
		evos: null,
	},
	anarlvet: {
		name: "Anarlvet",
		num: -7,
		diamondHand: true,
		gender: "M",
		types: ["Dark"],
		baseStats: {hp: 94, atk: 113, def: 73, spa: 98, spd: 65, spe: 132},
		abilities: {0: "Auctor Wile"},
		weightkg: 226.2,
	},
	gholdengo: {
		inherit: true,
		name: "Gholdengo",
		num: -8,
		types: ["Ghost", "Silly"],
		baseStats: {hp: 100, atk: 60, def: 90, spa: 125, spd: 90, spe: 110},
		abilities: {0: "Alpha Sigma Rizz"},
	},
	kingler: {
		inherit: true,
		name: "Kingler",
		num: -9,
		fish: true,
		types: ["Water"],
		baseStats: {hp: 70, atk: 130, def: 130, spa: 50, spd: 70, spe: 50},
		abilities: {0: "Carcinization", 1: "Shell Armor", H: "Tough Claws"},
	},
	kinglermega: {
		name: "Kingler-Mega",
		num: -10,
		baseSpecies: "Kingler",
		forme: "Mega",
		fish: true,
		types: ["Water", "Steel"],
		baseStats: {hp: 72, atk: 132, def: 212, spa: 62, spd: 102, spe: 32},
		abilities: {0: "Skeptic"},
		requiredItem: "Kinglerite",
		weightkg: 140.0,
	},
	feebas: {
		inherit: true,
		gen: 3,
		name: "Feebas",
		num: -11,
		fish: true,
		types: ["Water"],
		baseStats: {hp: 20, atk: 25, def: 20, spa: 10, spd: 55, spe: 80},
		abilities: {0: "Fashion icon", 1: "Oblivious", H: "Adaptability"},
	},
	microwave: {
		name: "microwave",
		num: -12,
		types: ["Fire", "Steel", "Electric"],
		baseStats: {hp: 51, atk: 89, def: 121, spa: 113, spd: 177, spe: 49},
		abilities: {0: "Lightning Rod", 1: "Parting Gift", H: "Transistor"},
		weightkg: 150.0,
	},
	glaceon: {
		inherit: true,
		name: "Glaceon",
		num: -13,
		types: ["Ice"],
		baseStats: {hp: 85, atk: 60, def: 110, spa: 130, spd: 95, spe: 65},
		abilities: {0: "Perfectionist", 1: "MILF", H: "Ice Body"},
		weightkg: 25.9,
	},
	lytlegai: {
		name: "Lytlegai",
		num: -14,
		types: ["Steel", "Fairy"],
		baseStats: {hp: 60, atk: 115, def: 75, spa: 60, spd: 85, spe: 85},
		abilities: {0: "Just a Little Guy"},
		weightkg: 30.0,
	},
	sylveon: {
		inherit: true,
		name: "Sylveon",
		trans: true,
		num: -15,
		types: ["Fairy"],
		baseStats: {hp: 60, atk: 130, def: 110, spa: 65, spd: 65, spe: 95},
		abilities: {0: "Gex Server", 1: "MILF", H: "Pixilate", S: "Pride Rally"},
	},
	ohmyrod: {
		name: "Ohmyrod",
		num: -16,
		fish: true,
		types: ["Water", "Grass"],
		baseStats: {hp: 75, atk: 130, def: 134, spa: 95, spd: 86, spe: 45},
		abilities: {0: "Water Absorb", 1: "Skeptic", H: "Innings Out"},
		weightkg: 96.9,
	},
	celesteela: {
		inherit: true,
		name: "Celesteela",
		num: -17,
		types: ["Steel", "Dragon"],
		baseStats: {hp: 107, atk: 101, def: 103, spa: 107, spd: 101, spe: 71},
		abilities: {0: "Levitate", H: "Compound Eyes"},
		weightkg: 999.9,
	},
	bigcrammer: {
		name: "Big Crammer",
		num: -18,
		fish: true,
		bird: true,
		types: ["Water", "Electric"],
		baseStats: {hp: 89, atk: 93, def: 69, spa: 119, spd: 125, spe: 110},
		abilities: {0: "Protostasis"},
		weightkg: 36.0,
		tags: ["Paradox"],
	},
	archeops: {
		inherit: true,
		name: "Archeops",
		num: -19,
		types: ["Ghost", "Steel"],
		baseStats: {hp: 85, atk: 110, def: 65, spa: 95, spd: 95, spe: 130},
		abilities: {0: "Armor Tail", 1: "Iron Barbs", H: "Chained Wrath"},
	},
	runerigus: {
		inherit: true,
		name: "Runerigus",
		num: -20,
		types: ["Ground", "Ghost"],
		baseStats: {hp: 85, atk: 100, def: 120, spa: 80, spd: 80, spe: 70},
		abilities: {0: "Wandering Spirit", 1: "Monster Mash", H: "Neutralizing Gas"},
		weightkg: 66.6,
	},
	crabominable: {
		inherit: true,
		name: "Crabominable",
		num: -21,
		fish: true,
		types: ["Ghost", "Dragon"],
		baseStats: {hp: 87, atk: 117, def: 117, spa: 67, spd: 75, spe: 67},
		abilities: {0: "Justified", 1: "Iron Fist", H: "Unseen Fist"},
		weightkg: 180.0,
	},
	samurottsinnoh: {
		name: "Samurott-Sinnoh",
		num: -22,
		fish: true,
		trans: true,
		types: ["Fire", "Dark"],
		baseStats: {hp: 80, atk: 105, def: 80, spa: 70, spd: 75, spe: 115},
		abilities: {0: "Blaze", 1: "Intimidate", H: "Sharpness"},
		weightkg: 90.6,
	},
	wyrdeer: {
		inherit: true,
		name: "Wyrdeer",
		num: -23,
		types: ["Normal", "Psychic"],
		baseStats: {hp: 150, atk: 105, def: 75, spa: 105, spd: 80, spe: 85},
		abilities: {0: "Intimidate", 1: "buy fish", H: "Sap Sipper", S: "Psychic Surge"},
	},
	goomba: {
		name: "Goomba",
		num: -24,
		trans: true,
		types: ["Poison", "Normal"],
		baseStats: {hp: 80, atk: 95, def: 80, spa: 95, spd: 80, spe: 55},
		abilities: {0: "Strong Jaw", 1: "Regenerator", 2: "Fashion icon", H: "Poison Point", S: "Intimidate"},
		weightkg: 138.0,
	},
	fridgile: {
		name: "Fridgile",
		num: -25,
		bird: true,
		types: ["Ice", "Rock"],
		baseStats: {hp: 255, atk: 35, def: 45, spa: 75, spd: 45, spe: 100},
		abilities: {0: "Snow Warning", 1: "Sand Stream", H: "Innings Out"},
		weightkg: 1.0,
	},
	zaciancrowned: {
		inherit: true,
		forme: null,
		battleOnly: null,
		name: "Zacian-Crowned",
		trans: true,
		num: -26,
		abilities: {0: "Stall", H: "Identity Crisis"},	
	},
	
	melmetal2: {
		name: "Melmetal 2",
		num: -27,
		types: ["Steel"],
		baseStats: {hp: 135, atk: 144, def: 144, spa: 80, spd: 65, spe: 34},
		abilities: {0: "Iron Fist", H: "Gex Server"},
		weightkg: 802.0,
	},
	pidown: {
		name: "Pidown",
		num: -28,
		bird: true,
		types: ["Fighting", "Poison"],
		baseStats: {hp: 130, atk: 125, def: 100, spa: 40, spd: 90, spe: 45},
		abilities: {0: "Guts", 1: "Gluttony", H: "Mold Breaker"},
		weightkg: 102.9,
	},
	silvally: {
		inherit: true,
		num: 773,
		fish: true,
		bird: true,
		abilities: {0: "RKS System", H: "Stall"},
		otherFormes: ["Silvally-Bug", "Silvally-Dark", "Silvally-Dragon", "Silvally-Electric", "Silvally-Fairy", "Silvally-Fighting", "Silvally-Fire", "Silvally-Flying", "Silvally-Ghost", "Silvally-Grass", "Silvally-Ground", "Silvally-Ice", "Silvally-Lemon", "Silvally-Poison", "Silvally-Psychic", "Silvally-Rock", "Silvally-Silly", "Silvally-Steel", "Silvally-Water"],
		formeOrder: [
			"Silvally", "Silvally-Fighting", "Silvally-Flying", "Silvally-Poison", "Silvally-Ground", "Silvally-Rock", "Silvally-Bug", "Silvally-Ghost", "Silvally-Steel",
			"Silvally-Fire", "Silvally-Water", "Silvally-Grass", "Silvally-Electric", "Silvally-Psychic", "Silvally-Ice", "Silvally-Dragon", "Silvally-Dark", "Silvally-Fairy", "Silvally-Lemon", "Silvally-Silly",
		],
	},
	vivillonplainshigh: {
		name: "Vivillon-Plains-High",
		num: -29,
		baseSpecies: "Vivillon",
		forme: "Plains-High",
		types: ["Bug", "Fighting"],
		baseStats: {hp: 73, atk: 79, def: 75, spa: 104, spd: 104, spe: 106},
		abilities: {0: "Shield Dust", 1: "Compound Eyes", H: "Natural Cure"},
		weightkg: 17.0,
		prevo: null,
	},
	moltres: {
		inherit: true,
		name: "Moltres",
		num: -30,
		bird: true,
		types: ["Fire", "Flying"],
		baseStats: {hp: 90, atk: 79, def: 90, spa: 115, spd: 85, spe: 121},
		abilities: {0: "Pressure", 1: "just the birds", H: "Flame Body"},
		weightkg: 60.0,
	},
	kurayami: {
		name: "Kurayami",
		num: -31,
		types: ["Dark"],
		baseStats: {hp: 85, atk: 85, def: 95, spa: 135, spd: 95, spe: 95},
		abilities: {0: "Magic Guard", 1: "Monster Mash", H: "Dark Aura"},
		weightkg: 999.7,
	},
	hitmontop: {
		inherit: true,
		name: "Hitmontop",
		num: -32,
		types: ["Fighting"],
		baseStats: {hp: 65, atk: 115, def: 65, spa: 35, spd: 145, spe: 50},
		abilities: {0: "Intimidate", 1: "Technician", H: "Lowkickenuinely"},
		weightkg: 48.0,
	},
	zelda: {
		name: "Zelda",
		trans: true,
		num: -33,
		types: ["Fighting", "Flying"],
		baseStats: {hp: 90, atk: 115, def: 90, spa: 90, spd: 80, spe: 100},
		abilities: {0: "Pride Rally", 1: "MILF", H: "Stamina"},
		weightkg: 70.0,
	},
	cascoon: {
		inherit: true,
		gen: 3,
		name: "Cascoon",
		num: -34,
		types: ["Bug", "Fairy"],
		baseStats: {hp: 105, atk: 65, def: 115, spa: 85, spd: 95, spe: 25},
		abilities: {0: "Shed Skin", 1: "Unaware", H: "Magic Guard"},
		evos: null,
	},
	sentret: {
		inherit: true,
		name: "Sentret",
		num: -35,
		types: ["Normal"],
		baseStats: {hp: 35, atk: 46, def: 34, spa: 35, spd: 45, spe: 86},
		abilities: {0: "Run Away", 1: "Parting Gift", H: "Frisk"},
	},
	drigike: {
		name: "Drigike",
		copen: true,
		num: -36,
		types: ["Rock"],
		baseStats: {hp: 65, atk: 80, def: 60, spa: 135, spd: 65, spe: 80},
		abilities: {0: "Crossover", 1: "Lightning Rod", H: "Storm Drain"},
		weightkg: 445.0,
	},
	phish: {
		name: "Phish",
		num: -37,
		fish: true,
		types: ["Ghost", "Electric"],
		baseStats: {hp: 52, atk: 123, def: 104, spa: 111, spd: 104, spe: 87},
		abilities: {0: "Illusion", 1: "Imposter", H: "Good as Gold"},
		weightkg: 69.0,
	},
	smelmetal: {
		name: "Smelmetal",
		num: -38,
		types: ["Steel", "Silly"],
		baseStats: {hp: 97, atk: 99, def: 144, spa: 77, spd: 107, spe: 37},
		abilities: {0: "Iron Nose"},
		weightkg: 570.0,
	},
	starmie: {
		name: "Starmie",
		num: -39,
		fish: true,
		types: ["Water", "Psychic"],
		baseStats: {hp: 60, atk: 85, def: 90, spa: 115, spd: 110, spe: 115},
		abilities: {0: "Protean", 1: "Natural Cure", H: "Analytic"},
		weightkg: 80.0,
	},
	victreebel: {
		inherit: true,
		name: "Victreebel",
		num: -40,
		types: ["Grass", "Poison"],
		baseStats: {hp: 80, atk: 125, def: 95, spa: 60, spd: 95, spe: 100},
		abilities: {0: "Fly Eater", H: "Gluttony"},
		weightkg: 15.5,
	},
	bondra: {
		name: "Bondra",
		copen: true,
		num: -41,
		types: ["Grass", "Fire"],
		baseStats: {hp: 103, atk: 113, def: 93, spa: 43, spd: 93, spe: 113},
		abilities: {0: "Strong Jaw", 1: "Hustle", H: "Chlorophyll"},
		weightkg: 312.0,
	},
	tangetteeternal: {
		name: "Tangette-Eternal",
		num: -42,
		types: ["Grass", "Fairy"],
		baseStats: {hp: 90, atk: 85, def: 100, spa: 120, spd: 90, spe: 75},
		abilities: {0: "Growth Veil"},
		weightkg: 64.75,
	},
	donmigo: {
		name: "Donmigo",
		num: -43,
		bird: true,
		types: ["Ground", "Flying"],
		baseStats: {hp: 90, atk: 120, def: 100, spa: 70, spd: 70, spe: 70},
		abilities: {0: "just the birds", H: "Bravery"},
		weightkg: 78.5,
	},
	dragoone: {
		name: "Dragoone",
		num: -44,
		gen: 3,
		types: ["Normal", "Dragon"],
		baseStats: {hp: 88, atk: 97, def: 50, spa: 78, spd: 175, spe: 78},
		abilities: {0: "Snakewood"},
		weightkg: 206.5,
	},
	collachet: {
		name: "Collachet",
		num: -45,
		diamondHand: true,
		gender: "M",
		types: ["Fighting"],
		baseStats: {hp: 102, atk: 148, def: 113, spa: 80, spd: 78, spe: 54},
		abilities: {0: "toxic masculinity", 1: "Intimidate", H: "Parting Gift"},
		weightkg: 538.3,
	},
	guiltrism: {
		name: "Guiltrism",
		num: -46,
		diamondHand: true,
		bird: true,
		gender: "M",
		types: ["Flying"],
		baseStats: {hp: 95, atk: 95, def: 95, spa: 95, spd: 95, spe: 100},
		abilities: {0: "just the birds", H: "Prism Wings"},
		weightkg: 77.2,
	},
	swampert: {
		inherit: true,
		name: "Swampert",
		num: -47,
		fish: true,
		gen: 3,
		types: ["Water", "Ground"],
		baseStats: {hp: 100, atk: 110, def: 90, spa: 75, spd: 90, spe: 60},
		abilities: {0: "Torrent", H: "Steel Drummer"},
		weightkg: 81.9,
	},
	swooliobat: {
		name: "Swooliobat",
		num: -48,
		types: ["Psychic", "Flying"],
		baseStats: {hp: 97, atk: 57, def: 105, spa: 107, spd: 105, spe: 64},
		abilities: {0: "Unaware", 1: "Klutz", H: "Prankster"},
		weightkg: 325.5,
	},
	electrode: {
		inherit: true,
		name: "Electrode",
		num: -49,
		types: ["Electric"],
		baseStats: {hp: 60, atk: 50, def: 70, spa: 80, spd: 80, spe: 150},
		abilities: {0: "What the Sigma", 1: "Static", H: "Aftermath"},
	},
	electrodemega: {
		name: "Electrode-Mega",
		num: -50,
		baseSpecies: "Electrode",
		forme: "Mega",
		types: ["Electric"],
		baseStats: {hp: 52, atk: 52, def: 52, spa: 152, spd: 52, spe: 242},
		abilities: {0: "Time Bomb"},
		weightkg: 133.2,
	},
	mariokartwii: {
		name: "Mario Kart Wii",
		num: -51,
		types: ["Steel"],
		baseStats: {hp: 100, atk: 80, def: 100, spa: 70, spd: 40, spe: 135},
		abilities: {0: "Gorilla Tactics", H: "Greatest Video Game of All Time"},
		weightkg: 1.0,
	},
	impalpitoad: {
		name: "Impalpitoad",
		num: -52,
		fish: true,
		types: ["Water", "Ground"],
		baseStats: {hp: 75, atk: 65, def: 55, spa: 65, spd: 55, spe: 69},
		abilities: {0: "DILF", H: "Impalpable"},
		weightkg: 983.9,
	},
	scrubby: {
		name: "Scrubby",
		num: -53,
		fish: true,
		types: ["Water", "Dark"],
		baseStats: {hp: 110, atk: 60, def: 75, spa: 125, spd: 90, spe: 90},
		abilities: {0: "Quick Feet", 1: "What the Sigma", H: "Screen Cleaner"},
		weightkg: 0.1,
	},
	palpitoadissocool: {
		name: "palpitoad is so cool",
		num: -54,
		fish: true,
		types: ["Water", "Silly"],
		baseStats: {hp: 120, atk: 100, def: 85, spa: 100, spd: 85, spe: 70},
		abilities: {0: "MILF", H: "champion"},
		weightkg: 17.0,
	},
	spewpa: {
		inherit: true,
		name: "Spewpa",
		num: -55,
		types: ["Bug", "Ghost"],
		baseStats: {hp: 70, atk: 50, def: 120, spa: 110, spd: 120, spe: 30},
		abilities: {0: "Shed Skin", 1: "Perish Body", H: "Magic Bounce"},
		evos: null,
	},
	
	moltresmega: {
		name: "Moltres-Mega",
		num: -56,
		baseSpecies: "Moltres",
		forme: "Mega",
		types: ["Fire", "Flying"],
		baseStats: {hp: 92, atk: 127, def: 102, spa: 162, spd: 102, spe: 107},
		abilities: {0: "Magic Bounce"},
		weightkg: 70.0,
		requiredItem: "Moltresite",
	},
	jirachitwo: {
		name: "Jirachitwo",
		num: -57,
		gen: 3,
		types: ["Psychic", "Ground"],
		baseStats: {hp: 95, atk: 95, def: 100, spa: 95, spd: 100, spe: 70},
		abilities: {0: "Benevolent Blessing"},
		weightkg: 5.0,
	},
	shinxfishing: {
		name: "Shinx-Fishing",
		num: -58,
		fish: true,
		types: ["Electric"],
		baseStats: {hp: 46, atk: 66, def: 35, spa: 41, spd: 35, spe: 46},
		abilities: {0: "Fishercat", 1: "toxic masculinity", H: "Guts"},
		weightkg: 9.5,
	},
	conquescape: {
		name: "Conquescape",
		num: -59,
		diamondHand: true,
		gender: "N",
		types: ["Ice"],
		baseStats: {hp: 130, atk: 25, def: 75, spa: 116, spd: 170, spe: 59},
		abilities: {0: "Frozen Landscape", H: "No Guard"},
		weightkg: 999.9,
	},
	daiyakuza: {
		name: "Daiyakuza",
		num: -60,
		diamondHand: true,
		gender: "M",
		types: ["Normal"],
		baseStats: {hp: 125, atk: 100, def: 100, spa: 100, spd: 100, spe: 50},
		abilities: {0: "What the Sigma", H: "The Diamond Hand"},
		weightkg: 999.9,
	},
	metapod: {
		inherit: true,
		name: "Metapod",
		num: -61,
		types: ["Bug", "Grass"],
		baseStats: {hp: 120, atk: 120, def: 120, spa: 120, spd: 120, spe: 120},
		abilities: {0: "Shed Skin", 1: "Multiscale", H: "Grassy Surge"},
		weightkg: 9.9,
		evos: null,
	},
	kakuna: {
		inherit: true,
		name: "Kakuna",
		num: -62,
		types: ["Bug", "Poison"],
		baseStats: {hp: 100, atk: 120, def: 100, spa: 80, spd: 100, spe: 100},
		abilities: {0: "Shed Skin", 1: "Sharpness", H: "Tinted Lens"},
		weightkg: 10.0,
		evos: null,
	},
	enamorustherian: {
		inherit: true,
		name: "Enamorus-Therian",
		num: -63,
		bird: true,
		types: ["Fairy", "Dragon"],
		baseStats: {hp: 74, atk: 105, def: 110, spa: 135, spd: 100, spe: 46},
		abilities: {0: "Overcoat", 1: "Fashion icon", H: "Gex Server"},
		weightkg: 48.0,
	},
	pokestarfisherman: {
		name: "Pokestar Fisherman",
		num: -64,
		fish: true,
		types: ["Normal"],
		baseStats: {hp: 100, atk: 100, def: 100, spa: 100, spd: 100, spe: 100},
		abilities: {0: "Skeptic", H: "I Love Fishing"},
		weightkg: 100.0,
	},
	magnegiri: {
		name: "Magnegiri",
		num: -65,
		fish: true,
		types: ["Water", "Electric"],
		baseStats: {hp: 70, atk: 60, def: 90, spa: 125, spd: 95, spe: 80},
		abilities: {0: "Magnetic Storm"},
		weightkg: 94.0,
	},
	mario: {
		name: "mario",
		num: -66,
		types: ["Ghost", "Steel"],
		baseStats: {hp: 104, atk: 44, def: 104, spa: 104, spd: 44, spe: 44},
		abilities: {0: "Stall", 1: "Cursed Body", H: "Mega Launcher"},
		weightkg: 999.9,
	},
	contamicow: {
		name: "Contamicow",
		copen: true,
		num: -67,
		types: ["Dark", "Poison"],
		baseStats: {hp: 80, atk: 110, def: 73, spa: 73, spd: 120, spe: 79},
		abilities: {0: "Sheer Force", 1: "Strong Jaw", H: "Cud Chew"},
	},
	whonhef: {
		name: "Whonhef",
		copen: true,
		num: -68,
		types: ["Normal", "Fighting"],
		baseStats: {hp: 80, atk: 110, def: 73, spa: 73, spd: 120, spe: 79},
		abilities: {0: "Sharpness", 1: "Berserk", H: "Hyper Cutter"},
	},
	fishfactory: {
		name: "Fish Factory",
		num: -69,
		fish: true,
		types: ["Steel", "Fire"],
		baseStats: {hp: 120, atk: 40, def: 100, spa: 90, spd: 90, spe: 60},
		abilities: {0: "White Smoke", 1: "MILF", H: "Flash Fire"},
		weightkg: 999.0,
	},
	cowboybandido: {
		name: "cowboy_bandido",
		num: -70,
		types: ["Dark", "Fighting"],
		baseStats: {hp: 100, atk: 135, def: 120, spa: 70, spd: 95, spe: 80},
		abilities: {0: "Sand Rush", 1: "What the Sigma", H: "River Thief"},
		weightkg: 58.0,
	},
	basculinbluestriped: {
		inherit: true,
		name: "Basculin-Blue-Striped",
		num: -71,
		fish: true,
		types: ["Water"],
		baseStats: {hp: 85, atk: 92, def: 95, spa: 90, spd: 95, spe: 98},
		abilities: {0: "Rock Head", 1: "Adaptability", H: "Fishy Surge"},
		weightkg: 18.0,
	},
	pokestargiant: {
		inherit: true,
		name: "Pokestar Giant",
		num: -72,
		types: ["Normal"],
		baseStats: {hp: 150, atk: 100, def: 80, spa: 100, spd: 80, spe: 60},
		abilities: {0: "Supreme Overlord", 1: "Pressure", H: "Big Lady"},
		weightkg: 999.0,
	},
	richardpetty: {
		name: "Richard Petty",
		num: -73,
		types: ["Normal", "Steel"],
		baseStats: {hp: 43, atk: 43, def: 43, spa: 43, spd: 43, spe: 200},
		abilities: {0: "Speed Boost", 1: "Steely Spirit", H: "Racer's Spirit"},
		weightkg: 60.0,
	},
	impidimp: {
		inherit: true,
		name: "Impidimp",
		num: -74,
		types: ["Fairy", "Dark"],
		baseStats: {hp: 45, atk: 45, def: 30, spa: 55, spd: 40, spe: 50},
		abilities: {0: "Prankster", 1: "Frisk", H: "Pickpocket"},
		weightkg: 5.5,
	},
	impidimpmega: {
		name: "Impidimp-Mega",
		num: -75,
		baseSpecies: "Impidimp",
		forme: "Mega",
		types: ["Fairy"],
		baseStats: {hp: 47, atk: 87, def: 42, spa: 37, spd: 52, spe: 112},
		abilities: {0: "Pure Power"},
		weightkg: 5.5,
		requiredItem: "Impidimpite",
	},
	lemon: {
		name: "Lemon",
		num: -76,
		types: ["Lemon"],
		baseStats: {hp: 70, atk: 75, def: 105, spa: 117, spd: 120, spe: 55},
		abilities: {0: "Mold Breaker", 1: "Ripen", H: "Acidic Drizzle"},
		weightkg: 0.1,
	},
	fishingzombie: {
		name: "Fishing Zombie",
		num: -77,
		fish: true,
		types: ["Ghost", "Water"],
		baseStats: {hp: 110, atk: 85, def: 110, spa: 90, spd: 80, spe: 5},
		abilities: {0: "PVZ Fishing", H: "Damp"},
		weightkg: 4.8,
	},
	pokestarmt: {
		name: "Pokestar MT",
		num: -78,
		types: ["Steel"],
		baseStats: {hp: 105, atk: 103, def: 105, spa: 100, spd: 97, spe: 90},
		abilities: {0: "Analytic", H: "Kaiju Killer"},
		weightkg: 999.0,
	},
	margaretthatcher: {
		name: "Margaret Thatcher",
		num: -79,
		types: ["Normal", "Poison"],
		baseStats: {hp: 120, atk: 80, def: 90, spa: 80, spd: 70, spe: 69},
		abilities: {0: "Poison Puppeteer", 1: "Gex Server", H: "Iron Lady"},
		weightkg: 81.0,
	},
	fleshvaliant: {
		name: "Flesh Valiant",
		num: -80,
		types: ["Fairy", "Fighting"],
		baseStats: {hp: 70, atk: 105, def: 85, spa: 95, spd: 115, spe: 80},
		abilities: {0: "Skill Issue"},
		weightkg: 50.0,
	},
	fleshvaliantmega: {
		name: "Flesh Valiant-Mega",
		num: -81,
		baseSpecies: "Flesh Valiant",
		forme: "Mega",
		types: ["Fairy", "Fighting"],
		baseStats: {hp: 72, atk: 137, def: 67, spa: 137, spd: 137, spe: 112},
		abilities: {0: "Mystic Slicer"},
		weightkg: 50.0,
		requiredItem: "Fleshvaliantite",
	},
	abomasnow: {
		inherit: true,
		name: "Abomasnow",
		num: -82,
		types: ["Ice", "Grass"],
		baseStats: {hp: 108, atk: 68, def: 62, spa: 112, spd: 68, spe: 107},
		abilities: {0: "Snow Warning", 1: "DILF", H: "Skeptic", S: "Aboma Care"},
		weightkg: 135.5,
	},
	komala: {
		inherit: true,
		name: "Komala",
		num: -83,
		types: ["Normal", "Silly"],
		baseStats: {hp: 75, atk: 120, def: 70, spa: 75, spd: 105, spe: 35},
		abilities: {0: "Skeptic", H: "Bramblin Mentality"},
		weightkg: 19.9,
	},
	inkay: {
		inherit: true,
		fish: true,
	},

	silvallybug: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallydark: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallydragon: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyelectric: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyfairy: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyfighting: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyfire: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyflying: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyghost: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallygrass: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyground: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyice: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallypoison: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallypsychic: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallyrock: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallysteel: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallywater: {
		inherit: true,
		num: 773,
		fish: true,
		abilities: {0: "RKS System", H: "Stall"},
	},
	silvallylemon: {
		name: "Silvally-Lemon",
		baseSpecies: "Silvally",
		forme: "Lemon",
		types: ["Lemon"],
		num: 773,
		fish: true,
		baseStats: {hp: 95, atk: 95, def: 95, spa: 95, spd: 95, spe: 95},
		abilities: {0: "RKS System", H: "Stall"},
		requiredItem: "Lemon Memory",
		changesFrom: "Silvally",
	},
	ronaldreagan: {
		name: "Ronald Reagan",
		num: -85,
		types: ["Fire", "Dark"],
		baseStats: {hp: 125, atk: 85, def: 90, spa: 105, spd: 95, spe: 60},
		abilities: {0: "Reaganomics"},
		weightkg: 83.0,
	},
	limelips: {
		name: "Lime Lips",
		num: -86,
		types: ["Lemon", "Fairy"],
		baseStats: {hp: 80, atk: 110, def: 70, spa: 90, spd: 70, spe: 110},
		abilities: {0: "Dazzling", H: "Strong Jaw"},
		weightkg: 123.4,
	},
	lemotic: {
		name: "Lemotic",
		num: -87,
		types: ["Lemon", "Rock"],
		baseStats: {hp: 139, atk: 146, def: 57, spa: 115, spd: 71, spe: 72},
		abilities: {0: "Minus", 1: "Mega Launcher", H: "Perish Body"},
		weightkg: 150.0,
	},
	zestii: {
		name: "Zestii",
		num: -88,
		diamondHand: true,
		gender: "F",
		types: ["Lemon"],
		baseStats: {hp: 80, atk: 65, def: 90, spa: 120, spd: 125, spe: 95},
		abilities: {0: "Lemon Squeezy"},
		weightkg: 44.6,
	},
	sunflora: {
		inherit: true,
		name: "Sunflora",
		num: -89,
		types: ["Grass", "Fire"],
		baseStats: {hp: 85, atk: 110, def: 75, spa: 110, spd: 110, spe: 50},
		abilities: {0: "Chlorophyll", 1: "Solar Power", H: "Drought"},
		weightkg: 8.5,
	},
	rawringmoon: {
		name: "Rawring Moon",
		num: -90,
		bird: true,
		types: ["Dragon", "Silly"],
		baseStats: {hp: 105, atk: 129, def: 71, spa: 55, spd: 101, spe: 109},
		abilities: {0: "just the birds", H: "Protosynthesis"},
		weightkg: 360.0,
	},
	boogerponclownerstone: {
		name: "Boogerpon-CLOWNerstone",
		num: -91,
		types: ["Grass", "Silly"],
		gender: 'F',
		baseStats: {hp: 80, atk: 120, def: 84, spa: 60, spd: 96, spe: 110},
		abilities: {0: "Honker"},
		weightkg: 39.8,
	},
	boogerponclownerstonetera: {
		name: "Boogerpon-CLOWNerstone-Tera",
		baseSpecies: "Boogerpon-CLOWNerstone",
		forme: "Tera",
		num: -91,
		types: ["Grass", "Silly"],
		gender: 'F',
		baseStats: {hp: 80, atk: 120, def: 84, spa: 60, spd: 96, spe: 110},
		abilities: {0: "Embody Aspect (CLOWN)"},
		weightkg: 39.8,
	},
	keisbergif: {
		name: "Keisberg-IF",
		num: -92,
		types: ["Ice", "Silly"],
		baseStats: {hp: 105, atk: 120, def: 110, spa: 75, spd: 110, spe: 25},
		abilities: {0: "Eternal Rice", H: "Technician"},
		weightkg: 236.9,
	},
	indeedee: {
		name: "Indeedee",
		num: -93,
		types: ["Psychic", "Silly"],
		baseStats: {hp: 60, atk: 65, def: 55, spa: 115, spd: 95, spe: 135},
		abilities: {0: "Inner Focus", 1: "Psychic Surge", H: "Synchronize"},
		weightkg: 28.0,
	},
	applesnewestemoji: {
		name: "Apple\'s Newest Emoji",
		num: -94,
		bird: true,
		types: ["Fire", "Lemon"],
		baseStats: {hp: 92, atk: 65, def: 100, spa: 110, spd: 82, spe: 108},
		abilities: {0: "Flame Body", 1: "Unaware", H: "Zesty"},
		weightkg: 0.2,
	},
	cinderace: {
		inherit: true,
		name: "Cinderace",
		num: -95,
		types: ["Fire", "Fighting"],
		baseStats: {hp: 80, atk: 116, def: 80, spa: 65, spd: 80, spe: 109},
		abilities: {0: "Blaze", H: "Libero"},
		weightkg: 33.0,
	},
	lemonfish: {
		name: "Lemon Fish",
		num: -96,
		fish: true,
		types: ["Lemon", "Water"],
		baseStats: {hp: 100, atk: 115, def: 80, spa: 60, spd: 80, spe: 80},
		abilities: {0: "Swift Swim", 1: "Ripen", H: "Lemonade"},
		weightkg: 9,
	},
	dracolition: {
		name: "Dracolition",
		num: -97,
		types: ["Steel", "Fighting"],
		baseStats: {hp: 110, atk: 128, def: 105, spa: 60, spd: 95, spe: 40},
		abilities: {0: "Wrecking Ball", 1: "Intimidate", H: "Dragon\'s Maw"},
		weightkg: 235.4,
	},

	silvallysilly: {
		name: "Silvally-Silly",
		baseSpecies: "Silvally",
		forme: "Silly",
		types: ["Silly"],
		num: 773,
		fish: true,
		baseStats: {hp: 95, atk: 95, def: 95, spa: 95, spd: 95, spe: 95},
		abilities: {0: "RKS System", H: "Stall"},
		requiredItem: "Silly Memory",
		changesFrom: "Silvally",
	},
	silvallystellar: {
		name: "Silvally-Stellar",
		baseSpecies: "Silvally",
		forme: "Stellar",
		types: ["Stellar"],
		num: 773,
		fish: true,
		baseStats: {hp: 95, atk: 95, def: 95, spa: 95, spd: 95, spe: 95},
		abilities: {0: "RKS System", H: "Stall"},
		requiredItem: "Not Stellarium Z. Lol!",
		changesFrom: "Silvally",
	},
	goddease: {
		name: "Goddease",
		num: -100,
		types: ["Normal", "Flying"],
		baseStats: {hp: 80, atk: 120, def: 100, spa: 90, spd: 140, spe: 70},
		abilities: {0: "just the birds", H: "Pressure"},
		weightkg: 10.8,
	},
	jableye: {
		gen: 3,
		name: "Jableye",
		num: -101,
		types: ["Ghost", "Dark", "Poison"],
		baseStats: {hp: 75, atk: 75, def: 100, spa: 75, spd: 100, spe: 50},
		abilities: {0: "Justified", 1: "Monster Mash", H: "Jankster"},
		weightkg: 11.0,
	},
	kyrum: {
		name: "Kyrum",
		num: -102,
		types: ["Bug", "Dark"],
		baseStats: {hp: 71, atk: 102, def: 78, spa: 52, spd: 55, spe: 92},
		abilities: {0: "Kyrum Aura", 1: "Tinted Lens", H: "just the birds"},
		weightkg: 17.5,
	},
	raccoon: {
		name: "Raccoon",
		num: -103,
		types: ["Dark", "Silly"],
		baseStats: {hp: 75, atk: 106, def: 75, spa: 65, spd: 85, spe: 116},
		abilities: {0: "Pickpocket", 1: "Parting Gift", H: "Prankster"},
		weightkg: 15.5,
	},
	lucariocalm: {
		name: "Lucario-Calm",
		num: -104,
		types: ["Fighting", "Ice", "Steel"],
		baseStats: {hp: 70, atk: 115, def: 70, spa: 105, spd: 70, spe: 105},
		abilities: {0: "Steadfast", 1: "Sheer Force", H: "Justified"},
		weightkg: 47.0,
	},
	nedontrol: {
		name: "Nedontrol",
		diamondHand: true,
		num: -105,
		gender: "M",
		types: ["Rock"],
		baseStats: {hp: 126, atk: 95, def: 120, spa: 49, spd: 131, spe: 54},
		abilities: {0: "Bon Appetit"},
		weightkg: 434.5,
	},
	princirang: {
		name: "Princirang",
		diamondHand: true,
		num: -106,
		gender: "F",
		types: ["Fairy"],
		baseStats: {hp: 98, atk: 93, def: 86, spa: 83, spd: 80, spe: 135},
		abilities: {0: "Honor Student"},
		weightkg: 110.3,
	},
	ironclown: {
		name: "Iron Clown",
		num: -107,
		types: ["Steel", "Silly"],
		baseStats: {hp: 90, atk: 72, def: 100, spa: 122, spd: 108, spe: 98},
		abilities: {0: "Quark Drive", H: "Honker"},
		weightkg: 156.0,
	},
	thepearlhand: {
		name: "The Pearl Hand",
		num: -108,
		types: ["Water", "Dragon"],
		baseStats: {hp: 50, atk: 100, def: 100, spa: 100, spd: 100, spe: 125},
		abilities: {0: "Sour Sipper", H: "The Pearl Hand"},
		weightkg: 0.5,
	},
	mcfish: {
		name: "McFish",
		num: -109,
		types: ["Normal", "Dark"],
		baseStats: {hp: 125, atk: 125, def: 90, spa: 50, spd: 90, spe: 60},
		abilities: {0: "The Ever-Growing Hunger of Capitalism™"},
		weightkg: 999.9,
	},
	applwirm: {
		name: "Applwirm",
		num: -110,
		types: ["Ground", "Bug"],
		baseStats: {hp: 80, atk: 125, def: 100, spa: 75, spd: 80, spe: 115},
		abilities: {0: "Hustle", H: "Katabatic Winds"},
		weightkg: 7.2,
	},
	minunandplusled: {
		name: "minun and plusle :D",
		num: -111,
		gen: 3,
		types: ["Electric", "Electric"],
		baseStats: {hp: 120, atk: 90, def: 90, spa: 160, spd: 160, spe: 190},
		abilities: {0: "best friends"},
		weightkg: 8.4,
	},

	traike: {
		name: "Traike",
		trans: true,
		num: -112,
		types: ["Fighting", "Electric"],
		baseStats: {hp: 75, atk: 130, def: 60, spa: 115, spd: 60, spe: 115},
		abilities: {0: "Sturdy", 1: "No Guard", H: "Sand Force"},
		weightkg: 71.4,
	},
	groudon: {
		inherit: true,
		gen: 3,
		name: "Groudon",
		num: -113,
		types: ["Ground"],
		baseStats: {hp: 90, atk: 130, def: 120, spa: 90, spd: 80, spe: 70},
		abilities: {0: "Drought"},
		weightkg: 950.0,
	},
	drliberty: {
		name: "Dr. Liberty",
		diamondHand: true,
		num: -114,
		types: ["Psychic"],
		baseStats: {hp: 89, atk: 71, def: 78, spa: 120, spd: 117, spe: 100},
		abilities: {0: "Mad Scientist"},
		weightkg: 76.4,
	},
	absol: {
		inherit: true,
		name: "Absol",
		gen: 3,
		num: -115,
		types: ["Dark", "Ghost"],
		baseStats: {hp: 94, atk: 104, def: 74, spa: 104, spd: 74, spe: 114},
		abilities: {0: "Forewarn", 1: "Divining Horn", H: "Hoenn Stan"},
		weightkg: 47.0,
	},
	sunfloragrave: {
		name: "Sunflora-Grave",
		num: -116,
		types: ["Ghost"],
		baseStats: {hp: 95, atk: 120, def: 75, spa: 120, spd: 110, spe: 20},
		abilities: {0: "Zombies on Your Lawn"},
		weightkg: 0.1,
	},
	ironboulder: {
		name: "Iron Boulder",
		num: -117,
		types: ["Rock", "Psychic"],
		baseStats: {hp: 90, atk: 120, def: 80, spa: 68, spd: 108, spe: 124},
		abilities: {0: "Quark Drive"},
		weightkg: 162.5,
	},
	hydralemon: {
		name: "Hydralemon",
		num: -118,
		types: ["Lemon", "Dragon"],
		baseStats: {hp: 106, atk: 80, def: 110, spa: 120, spd: 80, spe: 44},
		abilities: {0: "Supersour Syrup", 1: "Regenerator", H: "Sticky Hold"},
		weightkg: 93.0,
	},
	zapdos: {
		inherit: true,
		name: "Zapdos",
		num: -119,
		bird: true,
		types: ["Electric", "Flying"],
		baseStats: {hp: 90, atk: 90, def: 85, spa: 125, spd: 90, spe: 100},
		abilities: {0: "Pressure", 1: "just the birds", H: "Static"},
		weightkg: 52.6,
	},
	solarbean: {
		name: "Solar Bean",
		num: -120,
		types: ["Grass", "Rock"],
		baseStats: {hp: 85, atk: 50, def: 75, spa: 110, spd: 110, spe: 70},
		abilities: {0: "Chlorophyll", 1: "Solar Power", H: "Harvest"},
		weightkg: 8.5,
	},
	
	hiveweb: {
		num: -121,
		name: "Hiveweb",
		types: ["Bug", "Normal"],
		baseStats: {hp: 95, atk: 90, def: 105, spa: 90, spd: 115, spe: 50},
		abilities: {0: "Honeyed Web", 1: "Shield Dust", H: "Compound Eyes"},
		weightkg: 9.1,
	},
	syndicant: {
		num: -123,
		name: "Syndican\'T",
		diamondHand: true,
		types: ["Bug"],
		baseStats: {hp: 129, atk: 107, def: 101, spa: 65, spd: 99, spe: 74},
		abilities: {0: "Eusociality"},
		weightkg: 128.4,
	},
	fishmarketing3: {
		num: -125,
		name: "Fish Marketing 3",
		types: ["Psychic", "Electric"],
		baseStats: {hp: 90, atk: 50, def: 60, spa: 125, spd: 70, spe: 115},
		abilities: {0: "buy fish"},
		weightkg: 0.1,
	},
	lemonganium: {
		num: -127,
		name: "Lemonganium",
		types: ["Lemon"],
		baseStats: {hp: 80, atk: 82, def: 100, spa: 83, spd: 100, spe: 80},
		abilities: {0: "Sour Hour", 1: "Flower Gift", H: "Unaware"},
		weightkg: 100.5,
	},
	carnivineif: {
		num: -129,
		name: "Carnivine-IF",
		types: ["Fairy", "Silly"],
		baseStats: {hp: 109, atk: 117, def: 96, spa: 64, spd: 96, spe: 58},
		abilities: {0: "Ghoul Gobbler", 1: "Zesty", H: "What the Sigma"},
		weightkg: 35.6,
	},
	grumpig: {
		inherit: true,
		num: -131,
		name: "Grumpig",
		gen: 3,
		types: ["Psychic"],
		baseStats: {hp: 125, atk: 40, def: 100, spa: 110, spd: 115, spe: 70},
		abilities: {0: "Pure Power", 1: "Thick Fat", H: "Gluttony"},
		weightkg: 71.5,
	},

	impromancer: {
		name: "Impromancer",
		num: -134,
		types: ["Ghost"],
		baseStats: {hp: 82, atk: 94, def: 90, spa: 120, spd: 89, spe: 100},
		abilities: {0: "Blight of the Fallen"},
		weightkg: 0.2,
	},
	panderdragoon: {
		name: "Pander Dragoon",
		num: -136,
		types: ["Dragon", "Ground"],
		baseStats: {hp: 120, atk: 65, def: 85, spa: 115, spd: 95, spe: 90},
		abilities: {0: "Timefrozen Body"},
		weightkg: 1.0,
	},
	soruarc: {
		name: "Soruarc",
		num: -138,
		types: ["Fire", "Fairy"],
		baseStats: {hp: 55, atk: 75, def: 70, spa: 125, spd: 80, spe: 105},
		abilities: {0: "Bloodline Spell: Blazeball"},
		weightkg: 66.0,
	},
	emolga: {
		inherit: true,
		name: "Emolga",
		num: -140,
		copen: true,
		bird: true,
		types: ["Electric", "Flying"],
		baseStats: {hp: 55, atk: 75, def: 60, spa: 75, spd: 60, spe: 104},
		abilities: {0: "United Party"},
		weightkg: 5.0,
	},
	skibidragon: {
		name: "Skibidragon",
		num: -142,
		types: ["Dragon"],
		baseStats: {hp: 97, atk: 80, def: 89, spa: 100, spd: 117, spe: 72},
		abilities: {0: "Rain Dish", 1: "Bathroom Break", H: "MILF"},
		weightkg: 43.7,
	},
	hitmontopmega: {
		name: "Hitmontop-Mega",
		num: -143,
		baseSpecies: "Hitmontop",
		forme: "Mega",
		types: ["Fighting", "Normal"],
		baseStats: {hp: 67, atk: 152, def: 102, spa: 37, spd: 147, spe: 82},
		abilities: {0: "Necrodancer"},
		weightkg: 72.0,
		requiredItem: "Hitmontopite",
	},
	porygonz: {
		inherit: true,
		name: "Porygon-Z",
		num: -144,
		bird: true,
		types: ["Normal"],
		baseStats: {hp: 85, atk: 80, def: 70, spa: 135, spd: 75, spe: 90},
		abilities: {0: "Adaptability", 1: "Download", H: "Analytic"},
		weightkg: 34.0,
	},
	porygonzmega: {
		name: "Porygon-Z-Mega",
		num: -145,
		baseSpecies: "Porygon-Z",
		forme: "Mega",
		types: ["Normal"],
		baseStats: {hp: 85, atk: 105, def: 85, spa: 145, spd: 110, spe: 105},
		abilities: {0: "Protean"},
		weightkg: 34.0,
		requiredItem: "Porygonite-Z",
	},
	furumo: {
		name: "Furumo",
		num: -146,
		bird: true,
		types: ["Water", "Fairy", "Ice"],
		baseStats: {hp: 85, atk: 105, def: 60, spa: 109, spd: 60, spe: 111},
		abilities: {0: "Water Veil", 1: "Pixilate", H: "Refrigerate"},
		weightkg: 36.0,
	},
	megaman: {
		name: "mega man",
		num: -147,
		types: ["Normal", "Fire"],
		baseStats: {hp: 99, atk: 99, def: 64, spa: 99, spd: 64, spe: 120},
		abilities: {0: "Magic Guard", 1: "Solid Rock", H: "MILF"},
		weightkg: 77.1,
	},
	fudgesaur: {
		name: "Fudgesaur",
		num: -148,
		types: ["Silly", "Ground"],
		baseStats: {hp: 80, atk: 85, def: 86, spa: 122, spd: 65, spe: 87},
		abilities: {0: "Disgusting, Repulsive Dessert"},
		weightkg: 100.0,
	},
	fudgesaurmega: {
		name: "Fudgesaur-Mega",
		num: -149,
		baseSpecies: "Fudgesaur",
		forme: "Mega",
		types: ["Silly", "Ground"],
		baseStats: {hp: 82, atk: 107, def: 125, spa: 124, spd: 127, spe: 72},
		abilities: {0: "Pristine Dessert"},
		weightkg: 100.0,
		requiredItem: "Fudgesaurite",
	},
	darkpoison: {
		name: "darkpoison",
		num: -151,
		types: ["Dark", "Poison"],
		baseStats: {hp: 200, atk: 100, def: 50, spa: 75, spd: 50, spe: 25},
		abilities: {0: "Rough Skin", 1: "Shed Skin", H: "Stakeout"},
		weightkg: 0.3,
	},

	swampertmega: {
		inherit: true,
		baseStats: {hp: 102, atk: 152, def: 112, spa: 97, spd: 112, spe: 72},
		abilities: {0: "Sour Sipper"},
		weightkg: 102.0,
	},
	sigmaricelion: {
		name: "Sigma Rice Lion",
		num: -152,
		types: ["Normal", "Electric"],
		baseStats: {hp: 100, atk: 75, def: 90, spa: 66, spd: 68, spe: 95},
		abilities: {0: "Sigma Surge"},
		weightkg: 190.0,
	},
	zoruahisui: {
		inherit: true,
		name: "Zorua-Hisui",
		num: -153,
		types: ["Normal", "Ghost"],
		baseStats: {hp: 35, atk: 40, def: 45, spa: 55, spd: 55, spe: 50},
		abilities: {0: "Monster Mash", H: "Illusion"},
		weightkg: 12.5,
	},
	wishiwashi: {
		inherit: true,
		name: "Wishiwashi",
		num: -154,
		fish: true,
		types: ["Water", "Bug"],
		baseStats: {hp: 44, atk: 49, def: 44, spa: 49, spd: 44, spe: 50},
		abilities: {0: "MILF", 1: "Technician", H: "Schooling"},
		weightkg: 0.3,
	},
	necrozmaultra2: {
		name: "Necrozma-Ultra 2",
		num: -155,
		bird: true,
		types: ["Psychic", "Dragon"],
		baseStats: {hp: 97, atk: 151, def: 97, spa: 151, spd: 97, spe: 129},
		abilities: {0: "Ultra Necrozma Clause"},
		weightkg: 230.0,
	},
	nihilego: {
		inherit: true,
		name: "Nihilego",
		num: -156,
		types: ["Poison", "Rock"],
		baseStats: {hp: 109, atk: 53, def: 47, spa: 127, spd: 131, spe: 103},
		abilities: {0: "Beast Boost", H: "Toxic Chain"},
		weightkg: 55.5,
	},
	lickilord: {
		name: "Lickilord",
		num: -157,
		trans: true,
		gender: "M",
		types: ["Silly"],
		baseStats: {hp: 100, atk: 80, def: 85, spa: 85, spd: 140, spe: 35},
		abilities: {0: "Bon Appetit", 1: "Pride Rally", H: "Cloud Nine"},
		weightkg: 94.3,
	},
	citrusjams: {
		name: "Citrus Jams",
		num: -158,
		types: ["Lemon", "Electric"],
		baseStats: {hp: 97, atk: 11, def: 40, spa: 111, spd: 83, spe: 103},
		abilities: {0: "Punk Rock", 1: "Gex Server", H: "Dazzling"},
		weightkg: 3.3,
	},
	everhl: {
		name: "Everhál",
		num: -159,
		diamondHand: true,
		gender: "F",
		types: ["Dragon"],
		baseStats: {hp: 109, atk: 143, def: 98, spa: 75, spd: 100, spe: 50},
		abilities: {0: "Slayer of Beasts"},
		weightkg: 241.8,
	},
	rayquaza: {
		inherit: true,
		name: "Rayquaza",
		num: -161,
		gen: 3,
		bird: true,
		types: ["Dragon", "Flying"],
		baseStats: {hp: 95, atk: 130, def: 80, spa: 130, spd: 80, spe: 95},
		abilities: {0: "Air Lock", H: "Bon Appetit"},
		weightkg: 206.5,
	},
	chinchou: {
		inherit: true,
		name: "Chinchou",
		num: -162,
		fish: true,
		types: ["Water", "Electric"],
		baseStats: {hp: 74, atk: 1, def: 46, spa: 35, spd: 58, spe: 66},
		abilities: {0: "Volt Absorb", 1: "Water Absorb", H: "Just a Little Guy"},
		weightkg: 12.0,
	},
	grimace: {
		name: "Grimace",
		num: -163,
		types: ["Poison"],
		baseStats: {hp: 105, atk: 108, def: 105, spa: 99, spd: 75, spe: 45},
		abilities: {0: "Unaware", 1: "Thick Fat", H: "Corrosion"},
		weightkg: 500.0,
	},
	pyroaring: {
		name: "Pyroaring",
		num: -164,
		types: ["Poison", "Ground"],
		baseStats: {hp: 130, atk: 75, def: 60, spa: 45, spd: 100, spe: 20},
		abilities: {0: "Poison Point", 1: "Water Absorb", H: "Unaware"},
		weightkg: 81.5,
	},
	houndoom: {
		inherit: true,
		name: "Houndoom",
		num: -165,
		types: ["Dark", "Steel"],
		baseStats: {hp: 75, atk: 90, def: 70, spa: 125, spd: 85, spe: 105},
		abilities: {0: "Doomer"},
		weightkg: 49.5,
	},
	togepi: {
		inherit: true,
		name: "togepi",
		num: -166,
		types: ["Normal", "Fairy"],
		baseStats: {hp: 35, atk: 20, def: 83, spa: 40, spd: 83, spe: 20},
		abilities: {0: "Simple", 1: "Serene Grace", H: "Super Luck"},
		weightkg: 1.5,
	},
	tylerthecreator: {
		name: "Tyler the Creator",
		num: -167,
		gender: "M",
		types: ["Lemon", "Ground"],
		baseStats: {hp: 109, atk: 88, def: 95, spa: 110, spd: 102, spe: 56},
		abilities: {0: "Fruity Bars", 1: "Alpha Sigma Rizz", H: "Inner Focus"},
		weightkg: 90.7,
	},
	
	abomasnowmega: {
		inherit: true,
		num: -177,
		name: "Abomasnow-Mega",
		types: ["Ice", "Dragon"],
		baseStats: {hp: 110, atk: 140, def: 104, spa: 114, spd: 100, spe: 69},
		abilities: {0: "Slush Rush"},
		weightkg: 185.0,
	},
	bart: {
		num: -176,
		name: "Bart",
		types: ["Lemon", "Silly"],
		baseStats: {hp: 76, atk: 80, def: 95, spa: 119, spd: 52, spe: 110},
		abilities: {0: "Gex Server", 1: "Prankster", H: "Regenerator"},
		weightkg: 38.0,
	},
	kyogre: {
		inherit: true,
		num: -175,
		name: "Kyogre",
		hoenn: true,
		gen: 3,
		types: ["Water"],
		baseStats: {hp: 90, atk: 85, def: 95, spa: 115, spd: 110, spe: 85},
		abilities: {0: "Drizzle"},
		weightkg: 430.0,
	},
	upvybones: {
		num: -174,
		name: "Upvybones",
		diamondHand: true,
		gender: 'M',
		types: ["Water"],
		baseStats: {hp: 105, atk: 115, def: 105, spa: 55, spd: 100, spe: 95},
		abilities: {0: "Still Water"},
		weightkg: 107.3,
	},
	ludicolo: {
		inherit: true,
		num: -173,
		name: "Ludicolo",
		hoenn: true,
		gen: 3,
		types: ["Silly", "Water", "Grass"],
		baseStats: {hp: 100, atk: 80, def: 83, spa: 105, spd: 103, spe: 75},
		abilities: {0: "Macabre Mourner", 1: "Hoenn Stan", H: "Hoenn Lover"},
		weightkg: 55.0,
	},
	tlagh: {
		num: -172,
		name: "T\'La\'Ágh",
		types: ["Silly"],
		diamondHand: true,
		gender: 'M',
		baseStats: {hp: 74, atk: 109, def: 79, spa: 109, spd: 79, spe: 125},
		abilities: {0: "Jester's Folly"},
		weightkg: 4.7,
	},
	talonflame: {
		inherit: true,
		num: -171,
		name: "Talonflame",
		bird: true,
		types: ["Fire", "Flying"],
		baseStats: {hp: 78, atk: 81, def: 71, spa: 74, spd: 69, spe: 126},
		abilities: {0: "Flame Body", 1: "Aerilate", H: "Gale Wings", S: "WRATH OF THE SMOGONBIRD"},
		weightkg: 24.5,
	},
	regibloom: {
		num: -170,
		name: "Regibloom",
		types: ["Grass"],
		baseStats: {hp: 80, atk: 100, def: 100, spa: 80, spd: 150, spe: 70},
		abilities: {0: "Overgrow", 1: "Solar Power", H: "Flower Gift"},
		weightkg: 230.0,
	},
	oldduke: {
		num: -169,
		name: "Old Duke",
		fish: true,
		types: ["Poison", "Water"],
		baseStats: {hp: 90, atk: 120, def: 70, spa: 75, spd: 65, spe: 100},
		abilities: {0: "Defiant", H: "Toxic Wisdom"},
		weightkg: 252.6,
	},
	biganvil: {
		num: -168,
		name: "BIG ANVIL",
		types: ["Fairy"],
		baseStats: {hp: 95, atk: 70, def: 73, spa: 95, spd: 90, spe: 60},
		abilities: {0: "Cute Charm", 1: "Magic Guard", H: "Unaware"},
		weightkg: 40.0,
	},

	ironfist: {
		num: -196,
		name: "Iron Fist",
		types: ["Dragon", "Fighting"],
		baseStats: {hp: 96, atk: 104, def: 82, spa: 74, spd: 96, spe: 118},
		abilities: {0: "Quark Drive", H: "Iron Fist"},
		weightkg: 136.0,
	},
	victreebelmega: {
		num: -195,
		name: "Victreebel-Mega",
		baseSpecies: "Victreebel",
		forme: "Mega",
		types: ["Grass", "Poison"],
		baseStats: {hp: 82, atk: 127, def: 137, spa: 122, spd: 137, spe: 62},
		abilities: {0: "Fly Eater"},
		weightkg: 31.0,
		requiredItem: "Victreebelite",
	},
	lucariocalmmega: {
		num: -194,
		name: "Lucario-Calm-Mega",
		baseSpecies: "Lucario-Calm",
		forme: "Mega",
		types: ["Fighting", "Ice", "Steel"],
		baseStats: {hp: 72, atk: 137, def: 112, spa: 137, spd: 112, spe: 77},
		abilities: {0: "Technician"},
		weightkg: 108.0,
		requiredItem: "Lucarionite",
	},
	awesomepossum: {
		num: -193,
		name: "awesome possum",
		types: ["Stellar"],
		baseStats: {hp: 98, atk: 108, def: 106, spa: 108, spd: 106, spe: 74},
		abilities: {0: "awesomeability", H: "Infiltrator"},
	},
	tiredofitowl: {
		num: -192,
		name: "Tired of it Owl",
		types: ["Silly", "Flying"],
		baseStats: {hp: 95, atk: 95, def: 105, spa: 70, spd: 105, spe: 95},
		abilities: {0: "just the birds", 1: "Sticky Hold", H: "Water Absorb", S: "Fishy Surge"},
		weightkg: 12.0,
	},
	caracal: {
		num: -191,
		name: "Caracal",
		types: ["Fighting", "Silly"],
		baseStats: {hp: 108, atk: 125, def: 90, spa: 75, spd: 90, spe: 75},
		abilities: {0: "Frisk", 1: "Sand Rush", H: "Stakeout"},
		weightkg: 19.0,
	},
	solarbeanprimal: {
		num: -190,
		name: "Solar Bean-Primal",
		baseSpecies: "Solar Bean",
		forme: "Primal",
		types: ["Grass", "Fire"],
		baseStats: {hp: 85, atk: 130, def: 80, spa: 130, spd: 80, spe: 95},
		abilities: {0: "Drought"},
		weightkg: 8.5,
		requiredItem: "Red Orb",
	},
	circall: {
		num: -189,
		name: "Circall",
		types: ["Fairy", "Silly", "Flying"],
		gender: "N",
		baseStats: {hp: 88, atk: 48, def: 108, spa: 48, spd: 108, spe: 84},
		abilities: {0: "Friend Guard", 1: "Healer", H: "Bloodline: Greatest Achievement"},
		weightkg: 999.9,
	},
	warioforbiddenone: {
		num: -189,
		name: "Wario-Forbidden-One",
		types: ["Poison", "Bug", "Dark"],
		baseStats: {hp: 166, atk: 176, def: 76, spa: 166, spd: 56, spe: 76},
		abilities: {0: "Bloodline: Greatest Achievement"},
		weightkg: 999.9,
	},
	kyogreoriginal: {
		num: -175,
		name: "Kyogre-Original",
		baseSpecies: "Kyogre",
		forme: "Original",
		types: ["Water"],
		gen: 3,
		baseStats: {hp: 90, atk: 130, def: 95, spa: 135, spd: 125, spe: 85},
		abilities: {0: "Original Sea"},
		requiredItem: "Original Item",
	},
	lawset: {
		num: -187,
		name: "Lawset",
		types: ["Ground"],
		gender: "M",
		diamondHand: true,
		baseStats: {hp: 76, atk: 100, def: 104, spa: 100, spd: 105, spe: 100},
		abilities: {0: "Lawbender"},
		weightkg: 32.3,
	},
	daiyakuzaorigin: {
		num: -60,
		name: "Daiyakuza-Origin",
		baseSpecies: "Daiyakuza",
		forme: "Origin",
		types: ["Normal"],
		baseStats: {hp: 125, atk: 110, def: 60, spa: 110, spd: 60, spe: 110},
		abilities: {0: "Intimidate"},
		requiredItem: "Diamond Heart",
		changesFrom: "Daiyakuza",
		weightkg: 142.5,
	},
	princirangmega: {
		num: -185,
		name: "Princirang-Mega",
		baseSpecies: "Princirang",
		forme: "Mega",
		types: ["Fairy"],
		baseStats: {hp: 110, atk: 105, def: 108, spa: 125, spd: 112, spe: 127},
		abilities: {0: "Miracle Student"},
		requiredItem: "Princirangite",
		weightkg: 86.4,
	},
	ratstagoonnewyorkian: {
		num: -184,
		name: "Ratstagoon-NewYorkian",
		types: ["Poison", "Steel"],
		baseStats: {hp: 93, atk: 113, def: 101, spa: 65, spd: 83, spe: 95},
		abilities: {0: "Steelworker", 1: "Merciless", H: "Feels Like Home"},
		weightkg: 115.0,
	},
	spewpamega: {
		num: -183,
		name: "Spewpa-Mega",
		baseSpecies: "Spewpa",
		forme: "Mega",
		types: ["Bug", "Ghost"],
		baseStats: {hp: 72, atk: 112, def: 162, spa: 127, spd: 117, spe: 22},
		abilities: {0: "Mold Breaker"},
		weightkg: 16.7,
		requiredItem: "Spewpanite",
	},
	zapmolcuno: {
		num: -182,
		name: "Zapmolcuno",
		types: ["Ice", "Electric", "Fire", "Flying"],
		baseStats: {hp: 90, atk: 93, def: 90, spa: 115, spd: 100, spe: 93},
		abilities: {0: "Pressure", 1: "just the birds", H: "Flame Body", S: "Static"},
		weightkg: 168.0,
	},
	nosepass: {
		inherit: true,
		num: -181,
		name: "Nosepass",
		types: ["Water", "Bug"],
		baseStats: {hp: 78, atk: 78, def: 133, spa: 78, spd: 133, spe: 33},
		abilities: {0: "Hoenn Sniffer"},
		evos: null,
	},
	girafarig: {
		inherit: true,
		num: -180,
		name: "Girafarig",
		trans: true,
		types: ["Normal", "Psychic"],
		baseStats: {hp: 70, atk: 80, def: 65, spa: 90, spd: 65, spe: 85},
		abilities: {0: "Inner Focus", 1: "Early Bird", H: "Sap Sipper", S: "Psychic Surge"},
	},
	girafarigmega: {
		num: -179,
		name: "Girafarig-Mega",
		baseSpecies: "Girafarig",
		forme: "Mega",
		trans: true,
		types: ["Normal", "Psychic", "Silly"],
		baseStats: {hp: 70, atk: 100, def: 75, spa: 130, spd: 75, spe: 105},
		abilities: {0: "Parental Bond"},
		weightkg: 50.5,
		requiredItem: "Etigirafarigite",
	},

	charjabug: {
		num: -201,
		name: "Charjabug",
		types: ["Bug", "Electric"],
		baseStats: {hp: 47, atk: 112, def: 145, spa: 105, spd: 145, spe: 46},
		abilities: {0: "Electric Surge", 1: "Intimidate", H: "Zinzolin's Resolve"},
		weightkg: 10.5,
		evos: null,
		prevo: null,
	},
	fatfuck: {
		num: -200,
		name: "fat fuck",
		types: ["Water", "Fairy"],
		trans: true,
		baseStats: {hp: 70, atk: 20, def: 50, spa: 20, spd: 50, spe: 45},
		abilities: {0: "Thick Fat", 1: "Sap Sipper", H: "Bouncy Bastard"},
		weightkg: 200.0,
	},
	flygon: {
		num: -199,
		name: "Flygon",
		types: ["Bug", "Dragon"],
		gen: 3,
		hoenn: true,
		baseStats: {hp: 80, atk: 100, def: 80, spa: 100, spd: 80, spe: 100},
		abilities: {0: "Levitate", H: "i'll get a mega this time i swear"},
		weightkg: 82.0,
	},
	flygonplus: {
		num: -199,
		name: "Flygon-Plus",
		baseSpecies: "Flygon",
		forme: "Plus",
		types: ["Bug", "Dragon"],
		gen: 3,
		hoenn: true,
		baseStats: {hp: 82, atk: 122, def: 102, spa: 122, spd: 102, spe: 122},
		abilities: {0: "i'll get a mega this time i swear"},
		weightkg: 82.0,
	},
	vishzolt: {
		num: -198,
		name: "VishZolt",
		types: ["Water", "Electric"],
		baseStats: {hp: 90, atk: 90, def: 100, spa: 90, spd: 100, spe: 70},
		abilities: {0: "Water Absorb", 1: "Volt Absorb", H: "Hustle Jaw"},
		weightkg: 182.5,
	},
	flaaffy: {
		num: -197,
		name: "Flaaffy",
		types: ["Fire", "Rock"],
		baseStats: {hp: 95, atk: 115, def: 80, spa: 95, spd: 80, spe: 90},
		abilities: {0: "Intimidate", 1: "Flash Fire", H: "Rock Head"},
		weightkg: 13.3,
		prevo: null,
	},
	murcowd: {
		num: -196,
		name: "murcow :D",
		types: ["Dark", "Flying"],
		baseStats: {hp: 60, atk: 42, def: 85, spa: 42, spd: 85, spe: 91},
		abilities: {0: "Insomnia", 1: "Regenerator", H: "Prankster"},
		weightkg: 10.0,
	},
	greenguillotina: {
		num: -195,
		name: "Green Guillotina",
		types: ["Grass", "Normal"],
		gender: "F",
		baseStats: {hp: 160, atk: 110, def: 65, spa: 65, spd: 110, spe: 40},
		abilities: {0: "Mmmm Green", H: "Thick Fat"},
		weightkg: 67.0,
	},
	burmysand: {
		num: -194,
		name: "Burmy-Sand",
		types: ["Steel", "Lemon"],
		baseStats: {hp: 60, atk: 34, def: 50, spa: 42, spd: 50, spe: 25},
		abilities: {0: "Class Dynamics"},
		weightkg: 3.4,
	},
	'1000threarthmover': {
		num: -193,
		name: "1000-THR Earthmover",
		types: ["Ground", "Steel"],
		baseStats: {hp: 100, atk: 100, def: 135, spa: 100, spd: 74, spe: 1},
		abilities: {0: "Stamina", 1: "Heavy Metal", H: "The Ever-Growing Hunger of Capitalism™"},
		weightkg: 999.9,
	},
	pronouns: {
		num: -192,
		name: "PRONOUNS",
		types: ["Fire", "Silly"],
		baseStats: {hp: 120, atk: 125, def: 95, spa: 45, spd: 85, spe: 111},
		abilities: {0: "Gender Ambiguity"},
		weightkg: 460.0,
	},
	lemonganiumlemonga: {
		num: -191,
		name: "Lemonganium-Lemonga",
		baseSpecies: "Lemonganium",
		forme: "Lemonga",
		types: ["Lemon", "Fairy"],
		baseStats: {hp: 82, atk: 94, def: 117, spa: 145, spd: 117, spe: 82},
		abilities: {0: "Lemonga Sour"},
		weightkg: 201.0,
		requiredItem: "Lemonganite",
	},
	miniormeteor: {
		num: -190,
		name: "Minior-Meteor",
		types: ["Rock"],
		baseStats: {hp: 80, atk: 80, def: 120, spa: 80, spd: 120, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 40.0,
		cosmeticFormes: null,
	},
	minior: {
		num: -190,
		name: "Minior",
		baseSpecies: "Minior-Meteor",
		forme: "Red",
		types: ["Rock", "Fire"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Fire",
		isCosmeticForme: null,
	},
	miniororange: {
		num: -190,
		name: "Minior-Orange",
		baseSpecies: "Minior-Meteor",
		forme: "Orange",
		types: ["Rock", "Ground"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Ground",
		isCosmeticForme: null,
	},
	minioryellow: {
		num: -190,
		name: "Minior-Yellow",
		baseSpecies: "Minior-Meteor",
		forme: "Yellow",
		types: ["Rock", "Lemon"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Lemon",
		isCosmeticForme: null,
	},
	miniorgreen: {
		num: -190,
		name: "Minior-Green",
		baseSpecies: "Minior-Meteor",
		forme: "Green",
		types: ["Rock", "Silly"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Silly",
		isCosmeticForme: null,
	},
	miniorblue: {
		num: -190,
		name: "Minior-Blue",
		baseSpecies: "Minior-Meteor",
		forme: "Blue",
		types: ["Rock", "Flying"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Flying",
		isCosmeticForme: null,
	},
	miniorindigo: {
		num: -190,
		name: "Minior-Indigo",
		baseSpecies: "Minior-Meteor",
		forme: "Indigo",
		types: ["Rock", "Water"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Water",
		isCosmeticForme: null,
	},
	miniorviolet: {
		num: -190,
		name: "Minior-Violet",
		baseSpecies: "Minior-Meteor",
		forme: "Violet",
		types: ["Rock", "Ghost"],
		baseStats: {hp: 80, atk: 120, def: 80, spa: 120, spd: 80, spe: 100},
		abilities: {0: "Shields Up"},
		weightkg: 0.3,
		requiredTeraType: "Ghost",
		isCosmeticForme: null,
	},
	kanon: {
		num: -189,
		name: "Kanon",
		types: ["Dark", "Silly"],
		baseStats: {hp: 80, atk: 67, def: 86, spa: 116, spd: 119, spe: 60},
		abilities: {0: "Without Love..."},
		weightkg: 6.7,
	},
	kanonbluesea: {
		num: -189,
		name: "Kanon-Blue-Sea",
		baseSpecies: "Kanon",
		forme: "Blue-Sea",
		types: ["Dark", "Silly"],
		baseStats: {hp: 80, atk: 67, def: 86, spa: 136, spd: 100, spe: 119},
		abilities: {0: "Without Love..."},
		weightkg: 6.7,
	},
	marlboro: {
		num: -188,
		name: "Marlboro",
		types: ["Fire", "Ground"],
		baseStats: {hp: 80, atk: 125, def: 50, spa: 70, spd: 70, spe: 79},
		abilities: {0: "Thermal Exchange", H: "Habit Forming"},
		weightkg: 0.1,
	},
	dip: {
		num: -187,
		name: "Dip",
		types: ["Poison"],
		baseStats: {hp: 115, atk: 50, def: 80, spa: 100, spd: 80, spe: 60},
		abilities: {0: "Stench", H: "I Love Sewers"},
		weightkg: 1.0,
	},
	flushmaster: {
		num: -187,
		name: "Flushmaster",
		baseSpecies: "Dip",
		forme: "",
		types: ["Water", "Poison"],
		baseStats: {hp: 115, atk: 90, def: 105, spa: 140, spd: 125, spe: 75},
		abilities: {0: "I Love Sewers"},
		weightkg: 581.7,
	},
	bluminion: {
		num: -186,
		name: "Bluminion",
		types: ["Water", "Fairy"],
		fish: true,
		baseStats: {hp: 101, atk: 59, def: 78, spa: 72, spd: 73, spe: 73},
		abilities: {0: "Shield Dust", 1: "Storm Drain", H: "Regenerator"},
		weightkg: 25.0,
	},
	biganvilmega: {
		num: -185,
		name: "BIG ANVIL-MEGA",
		baseSpecies: "BIG ANVIL",
		forme: "MEGA",
		types: ["Ghost"],
		baseStats: {hp: 100, atk: 65, def: 60, spa: 145, spd: 80, spe: 130},
		abilities: {0: "Grim Neigh"},
		weightkg: 44.5,
		requiredItem: "BIG ANVILITE",
	},
	melmetal3: {
		num: -184,
		name: "melmetal 3",
		types: ["Steel"],
		baseStats: {hp: 135, atk: 145, def: 145, spa: 80, spd: 65, spe: 34},
		abilities: {0: "Iron Fist", H: "What the Sigma"},
		weightkg: 804.0,
	},
	birbevilbot: {
		num: -183,
		name: "birbevil bot",
		types: ["Dark", "Electric"],
		baseStats: {hp: 85, atk: 125, def: 60, spa: 110, spd: 60, spe: 90},
		abilities: {0: "Intimidate", 1: "No Guard", H: "Guard Dog"},
		weightkg: 40.7,
	},

};
