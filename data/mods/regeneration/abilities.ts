...
// Additions
	wingsofvictory: {
		...
		onBasePower(basePower, attacker, defender, move) {
			if (this.effectData.fallen) {
				...
				this.debug(`Wings of Victory boost: ${powMod[this.effectData.fallen]}/4096`);
	...
	blackout: {
		onPreStart(pokemon) {
			this.add('-item', pokemon, 'Blackout');
			pokemon.itemData.ending = false;
		},
		onEnd(source) {
			source.itemData.ending = true;
