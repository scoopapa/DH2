"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }// Used for Asheviere and Snaquaza's moves
var _randomteams = require('./random-teams');
var _pokemon = require('../../../.sim-dist/pokemon');
	...
	*/
	...
	hereticsmark: {
		...
		onHit(target, source, move) {
			...
			this.effect = {id: ''} ;
			const pokemon = new (0, _pokemon.Pokemon)(set, target.side);
			this.effect = effect;
	...
	wondertrade: {
		...
		onHit(target, source) {
			...
			for (const [i, mon] of currentTeam.entries()) {
				...
				const effect = this.effect;
				this.effect = {id: ''} ;
				const pokemon = new (0, _pokemon.Pokemon)(set, source.side);
				this.effect = effect;
	...
	hazardpass: {
		...
		onHitSide(target, source) {
			...
			const hazards = {stealthrock: 1, spikes: 3, toxicspikes: 2, stickyweb: 1};
	...
	chaotic: {
		...
		onAfterHit(target, source) {
			...
			const stat = stats[this.random(5)];
			boost[stat] = 1;
	...
	fakeclaim: {
		...
		onAfterMoveSecondarySelf(pokemon, move) {
			...
			// Generate new set
			const generator = new (0, _randomteams.RandomStaffBrosTeams)('gen7randombattle', this.prng);
