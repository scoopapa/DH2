	...
	teambuilderConfig: {
		...
		runAction(action: Action) {
			...
			case 'start': {
				...
				this.add('start');
				// Change Don Quixote into Sancho & Heathcliff into Erlking
				for (const pokemon of this.getAllPokemon()) {
					let rawSpecies: Species | null = null;
					console.log(`${pokemon.species.id}\n`);
					if (pokemon.species.id === 'donquixote' && pokemon.item === 'rocinante') {
						rawSpecies = this.dex.species.get('Sancho');
						const fireFang = pokemon.baseMoves.indexOf('firefang');
						if (fireFang >= 0) {
							const move = this.dex.moves.get('Ecstasy Of Blood');
							pokemon.baseMoveSlots[fireFang] = {
								move: move.name,
								id: move.id,
					...
					} else if (pokemon.species.id === 'heathcliff' && pokemon.item === 'cathysmirror') {
						rawSpecies = this.dex.species.get('Erlking Heathcliff');
						const shadowPunch = pokemon.baseMoves.indexOf('shadowpunch');
						if (shadowPunch >= 0) {
							const move = this.dex.moves.get('Greatsword Rend');
							pokemon.baseMoveSlots[shadowPunch] = {
								move: move.name,
								id: move.id,
								pp: (move.noPPBoosts || move.isZ) ? move.pp : move.pp * 8 / 5,
								maxpp: (move.noPPBoosts || move.isZ) ? move.pp : move.pp * 8 / 5,
					...
					} else if (pokemon.ensemble && pokemon.item === 'moteoflight') {
						// check for appropriate ensemble member and change on switch in
						switch (pokemon?.speciesid) {
							case 'phillipchild':
								rawSpecies = this.dex.species.get('Distorted Phillip');
								break;
							case 'eileen' :
								rawSpecies = this.dex.species.get('Distorted Eileen');
								break;	
							case 'bremen' :
								rawSpecies = this.dex.species.get('Distorted Bremen');
								break;
							case 'greta' :
								rawSpecies = this.dex.species.get('Distorted Greta');
								break;
							case 'oswald' :
								rawSpecies = this.dex.species.get('Distorted Oswald');
								break;
							case 'tanya' :
								rawSpecies = this.dex.species.get('Distorted Tanya');
								break;
							case 'jaeheon' :
								rawSpecies = this.dex.species.get('Distorted Jae-Heon');
								break;
							case 'pluto' :
								rawSpecies = this.dex.species.get('Distorted Pluto-Shade');
								break;
							case 'elena' :
								rawSpecies = this.dex.species.get('Distorted Elena');
								break;
							case 'argalia' :
								rawSpecies = this.dex.species.get('Distorted Argalia');
								break;
						}
					}
			...
			case 'rewindclock':
				...
				action.target.hp = 1; // Needed so hp functions works
				action.target.sethp(action.target.maxhp / 2);
				this.add('-heal', action.target, action.target.getHealth, '[from] move: Rewind Clock');
				action.pokemon.side.removeSlotCondition(action.pokemon, 'rewindclock');
				break;
			case 'runUnnerve':
			...
			for (let i = 0; i < this.sides.length; i++) {
				...
				if (switches[i] && !this.canSwitch(this.sides[i])) {
					for (const pokemon of this.sides[i].active) {
						if (this.sides[i].slotConditions[pokemon.position]['revivalblessing'] || this.sides[i].slotConditions[pokemon.position]['rewindclock']) {
							reviveSwitch = true;
							continue;
						}
					...
					for (const pokemon of this.sides[i].active) {
						if (pokemon.hp && pokemon.switchFlag && pokemon.switchFlag !== 'revivalblessing' && pokemon.switchFlag !== 'rewindclock' &&
	...
	side: {
		...
		chooseSwitch(slotText?: string) {
			...
			if (!slotText) {
				...
				} else if (this.slotConditions[pokemon.position]['rewindclock']) {
					slot = 0;
					while ((!this.pokemon[slot].fainted) && this.pokemon[slot].sinner) slot++;
				} else {
			...
			} else if (slot < this.active.length && !this.slotConditions[pokemon.position]['rewindclock']) {
				return this.emitChoiceError(`Can't switch: You can't switch to an active Pokémon`);
			}
			...
			if (this.slotConditions[pokemon.position]['rewindclock']) {
				if (!targetPokemon.fainted) {
					return this.emitChoiceError(`Can't switch: You have to pass to a fainted Pokémon`);
				}
				if (!targetPokemon.sinner) {
					return this.emitChoiceError(`Can't switch: You have to pass to an LCB Sinner`);
				}
				// Should always subtract, but stop at 0 to prevent errors.
				this.choice.forcedSwitchesLeft = this.battle.clampIntRange(this.choice.forcedSwitchesLeft - 1, 0);
				pokemon.switchFlag = false;
