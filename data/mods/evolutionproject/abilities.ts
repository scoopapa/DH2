export const Abilities: {[abilityid: string]: ModdedAbilityData} = {
	...
	blowout: {
		...
		onSourceHit(target, source, move) {
			...
			if (oldAbility) {
				this.add('-ability', target, 'Soundproof', '[from] ability: Blowout', '[of] ' + source);
			}
		},
		name: "Blowout",
		rating: 3.5,
	...
	scaleshift: {
		...
		onUpdate(pokemon) {
			if (!pokemon.isStarted) return; // should activate *after* Data Mod
			let type = 'Normal'; // default to Normal just in case
			if (
				this.dex.getSpecies(pokemon.species.name) && this.dex.getSpecies(pokemon.species.name).types[0]
			) type = this.dex.getSpecies(pokemon.species.name).types[0]; // default to the correct primary type for the species if there is no ally
			for (const ally of pokemon.side.active) {
				if (ally !== pokemon && !ally.hasAbility('scaleshift')) {
					type = ally.types[0];
				}
			}
