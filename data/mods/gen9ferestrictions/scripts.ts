export const Scripts: ModdedBattleScriptsData = {
	...
	pokemon: {
		...
		isGrounded(negateImmunity = false) {
			...
			if (
				(this.hasAbility(['levitate', 'aerialassassin', 'gasleak'])) &&
		...
		ignoringAbility() {
			...
			if (this.hasItem('Ability Shield') || this.ability === ('neutralizinggas' as ID) || this.ability === ('gasleak' as ID)) return false;
			for (const pokemon of this.battle.getAllActive()) {
				// can't use hasAbility because it would lead to infinite recursion
				if ((pokemon.ability === ('neutralizinggas' as ID) || pokemon.ability === ('gasleak' as ID)) && !pokemon.volatiles['gastroacid'] &&
	...
	actions: {
		...
		modifyDamage(
			...
			if (pokemon.status === 'brn' && move.category === 'Physical' &&
				 !pokemon.hasAbility(['guts', 'abominable'])) {
