...
const SETUP = [
	'acidarmor', 'agility', 'barrier', 'bellydrum', 'curse', 'growth', 'meditate', 'raindance', 'sunnyday', 'swordsdance',
];
...
export class RandomGen2Teams extends RandomGen3Teams {
	...
	constructor(format: string | Format, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			...
			Flying: (movePool, moves, abilities, types, counter, species) => (
				!counter.get('Flying') && ['gligar', 'murkrow', 'xatu', 'dragonite'].includes(species.id)
			),
			...
			Psychic: (movePool, moves, abilities, types, counter, species) => (
			    !counter.get('Psychic') && ['starmie', 'jynx'].includes(species.id)
		    ),
	...
	randomMoveset(
		...
		if (role.includes('Support')) {
			if (movePool.includes('safeguard')) {
				counter = this.addMove('safeguard', moves, types, abilities, teamDetails, species, isLead,
					movePool, preferredType, role);
			}
		}
		...
		if (role.includes('Support')) {
			if (movePool.includes('encore')) {
				counter = this.addMove('encore', moves, types, abilities, teamDetails, species, isLead,
					movePool, preferredType, role);
			}
		}
	...
	getItem(
		...
		if (moves.has('flail')) return 'Miracle Berry';
		if (moves.has('reversal')) return 'Miracle Berry';

		if (moves.has('rest')) return 'Mint Berry';
		...
		if (role.includes('Sweeper')) return 'Miracle Berry';

		if (role.includes('Attacker')) return 'Miracle Berry';

		if (moves.has('protect') && this.randomChance(1, 2)) return 'Leftovers';
		// Default to Miracle Berry 
