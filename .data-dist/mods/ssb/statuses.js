"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; } const Statuses = {
	/*
	// Example:
	userid: {
		noCopy: true,
	...
	*/
	...
	elgino: {
		...
		onDamage(damage, target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _ => _.effectType]) === 'Move' && ['mimikyu', 'mimikyutotem'].includes(target.species.id) && !target.transformed) {
	...
	kay: {
		...
		onBoost(boost, target, source, effect) {
			if (_optionalChain([target, 'optionalAccess', _2 => _2.illusion])) return;
			if (_optionalChain([effect, 'optionalAccess', _3 => _3.id]) === 'zpower') return;
			let i;
			for (i in boost) {
				boost[i] *= 2;
			}
	...
	lifeisdank: {
		...
		// Mountaineer innate
		onDamage(damage, target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _4 => _4.id]) === 'stealthrock') {
				return false;
			}
		},
		onTryHit(target, source, move) {
	...
	auroraveil: {
		...
		durationCallback(target, source, effect) {
			if (_optionalChain([source, 'optionalAccess', _7 => _7.hasItem, 'call', _8 => _8('lightclay')])) {
				return 8;
			}
			return 5;
		},
