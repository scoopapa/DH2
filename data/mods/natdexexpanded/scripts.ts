export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
		...
		if (type !== '???') {
			...
			if (isSTAB) {
				stab = 1.5;
			} else if ((pokemon.hasType('dragon') && (type === 'electric' || type === 'fire' || type === 'grass' || type === 'water')))
				stab = 1.3;
	...
	pokemon: {
		...
		isGrounded(negateImmunity = false) {
			...
			// weird hack cuz im dumb
			if (this.species.abilities.H === 'levitate' && !this.battle.suppressingAbility(this)) return null;
	...
	init() {
		...
		this.modData("Learnsets", "aerodactyl").learnset.bravebird = ["9L1"];
	}
};
