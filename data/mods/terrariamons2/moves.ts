export const Moves: {[moveid: string]: ModdedMoveData} = {
	...
	*/
	...
	enchantedboomerang: {
		...
		accuracy: 90,
		pp: 10,
		shortDesc: "Attacks with a magical boomerang that hits twice.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	diamondbeam: {
		...
		accuracy: 100,
		pp: 5,
		shortDesc: "A beam of pure energy",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	crimsonrod: {
		...
		accuracy: 100,
		pp: 5,
		shortDesc: "Creates a cloud of crimtane energy that causes heavy rain to fall.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	starfury: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Attacks with a blade of starlight, becomes special if the user has higher special attack.",
		priority: 0,
	...
	nightsedge: {
		...
		accuracy: 100,
		pp: 5,
		shortDesc: "Attacks with a giant blade of night. High critical hit rate.",
		priority: 0,
	...
	crystaldart: {
		...
		accuracy: 100,
		pp: 15,
		shortDesc: "Fires a dart of hallowed crystal that has a chance to flinch the target.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	cursedinferno: {
		...
		shortDesc: "Fires a blast of cursed flames that have a high chance to burn the target and can burn anything.",
		priority: 0,
	...
	ichorstream: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Fires a stream of corrosive ichor that lowers the target's special defense.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	zapinator: { 
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Attacks with a glitchy beam that can have various effects, deals extra damage on a critical hit.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	terrablade: { 
		...
		accuracy: true,
		pp: 10,
		shortDesc: "Fires a gleaming blade of the earth's energy. Can't miss.",
		priority: 0,
		flags: {slicing: 1, protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	xenopopper: {
		...
		accuracy: 185,
		pp: 5,
		shortDesc: "Fires a cloud of bubbles that explode into water bullets, has a chance to confuse the target.",
		priority: 0,
	...
	crystalserpent: {
		...
		accuracy: 90,
		pp: 10,
		shortDesc: "Attacks with an explosive charge of crystal that damages the target's ally.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	spacegun: {
		...
		accuracy: 100,
		pp: 40,
		shortDesc: "A laser of meteor power that can be fired near-indefinitely.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	naturesgift: {
		...
		accuracy: true,
		pp: 10,
		shortDesc: "The user calls on the power of nature, raising its Special Attack and Special Defense by 1 stage and curing its own status effects.",
		priority: 0,
	...
	spinaltap: {
		...
		accuracy: 95,
		pp: 15,
		shortDesc: "Whips the target with a sharpened spine, enhances the user's Beat Up after striking.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	beegun: {
		...
		accuracy: 100,
		pp: 20,
		shortDesc: "Fires a swarm of bees at the target. This move usually goes first.",
		priority: 1,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	poisonstaff: {
		...
		accuracy: 85,
		pp: 10,
		shortDesc: "Fires a wide burst of magical poison fangs.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	cutlass: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Attacks with a sharp steel blade. High critical hit rate.",
		priority: 0,
		critRatio: 2,
		flags: {slicing: 1, protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
	...
	laserdrill: {
		...
		accuracy: true,
		pp: 15,
		shortDesc: "Projects a laser-powered drill to drill the opponent down. Doesn’t miss.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	daedalusstormbow: { 
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Fires a barrage of arrows from the sky at a foe. Hits 5 times as a Steel type move, then hits 5 times as a Fairy type move. Only Steel is considered when determining STAB.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	excalibur: {
		...
		accuracy: 100,
		pp: 5,
		shortDesc: "Attacks with a giant blade of holy energy. Ignores changes in defense.",
		priority: 0,
	...
	superstarshooter: {
		...
		accuracy: 90,
		pp: 2.5,
		shortDesc: "Fires a fallen star with high power but low ammo.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	brandoftheinferno: { 
		...
		accuracy: 100,
		pp: 15,
		shortDesc: "The user wields a fiery blade that allows it to parry incoming attacks. This move goes last, but burns attackers who come into contact with it.",
		priority: -3,
	...
	explosivetrapstaff: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Lays and detonates an explosive mine that covers the target in tar.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	tomeofinfinitewisdom: {
		...
		accuracy: 100,
		pp: 15,
		shortDesc: "Conjures a tornado to attack the foe.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	spiritwrath: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Attacks with a burst of orbs. Hits twice at high hp, heals the user when at low hp.",
		priority: 0,
		multihit: 1,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
	...
	hoverboard: {
		...
		accuracy: true,
		pp: 10,
		shortDesc: "The user gets on its hoverboard, increasing its speed and making it immune to Ground type attacks temporarily.",
		priority: 0,
		flags: {mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	flyingdragon: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Attacks with a blade powered by the heart of a dragon. Can burn.",
		priority: 0,
	...
	aerialbane: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Fires a flaming arrow that seeks targets in the sky.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	phantomphoenix: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "A fiery phoenix shot from a magical bow. Targets Defense over Special Defense.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	nebulablaze: {
		...
		accuracy: 100,
		pp: 5,
		shortDesc: "A powerful blast fueled by Orion's Belt. Is sometimes powered up, and can burn.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
	...
	daybreak: {
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Hurls a volley of blazing spears at the target. Hits multiple times.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	stardustdragon: { 
		...
		accuracy: 100,
		pp: 10,
		shortDesc: "Summons a Stardust Dragon to attack the foe. Gains power with every summon.",
		priority: 0,
	...
	vortexbeater: {
		...
		accuracy: 80,
		pp: 10,
		shortDesc: "Fires wildly from an otherworldly gun.",
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1},
		onPrepareHit(target, pokemon, move) {
			this.attrLastMove('[still]');
	...
	shroud: {
		...
		accuracy: true,
		pp: 10,
		shortDesc: "Shrouds the user, causing all moves to miss. If an attack is successfully dodged, the user's next attack always hits.",
		priority: 6,
		flags: {mirror: 1, metronome: 1},
		stallingMove: true,
		onPrepareHit(target, pokemon, move) {
