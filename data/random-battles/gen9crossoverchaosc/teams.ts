...
const RECOVERY_MOVES = [
	'healorder', 'milkdrink', 'moonlight', 'morningsun', 'recover', 'roost', 'shoreup', 'slackoff', 'softboiled', 'strengthsap', 'synthesis', 'shademend'
];
...
const SPEED_CONTROL = [
	'electroweb', 'glare', 'icywind', 'lowsweep', 'quash', 'stringshot', 'tailwind', 'thunderwave', 'trickroom', 'gossamerstorm'
];
...
const PIVOT_MOVES = [
	'chillyreception', 'flipturn', 'partingshot', 'shedtail', 'teleport', 'uturn', 'voltswitch', 'dimensionalcape', 'orbofdiscord'
];
...
export class RandomCCVCTeams extends RandomTeams {
	...
	override randomMoveset(
		...
		// Enforce U-Turn on V1
		if (species.id === 'v1' && movePool.includes('uturn')) {
			counter = this.addMove('uturn', moves, types, abilities, teamDetails, species, isLead, isDoubles,
				movePool, teraType, role);
		}
		// Enforce Dragon Fang on Corrin
		if (movePool.includes('dragonfang')) {
			counter = this.addMove('dragonfang', moves, types, abilities, teamDetails, species, isLead, isDoubles,
				movePool, teraType, role);
		}
		// Enforce Toxic on certain Corrin sets
		if (species.id === 'corrin' && movePool.includes('toxic')) {
	...
	override getPriorityItem(
		...
		// CCvC Hardcodes
		if (species.id === 'luigi' && role === 'Z-Move user') return 'Bondium Z';
		if (species.id === 'suwako' && role === 'Z-Move user') return 'Suwakium Z';
		if (species.id === 'ana' && role === 'Z-Move user') return 'Ananium Z';
		if (species.id === 'metaknight' && role === 'Z-Move user') return 'Metaknightium Z';
		if (species.id === 'corrin' && role === 'Z-Move user') return 'Corrinium Z';
		if (species.id === 'morian' && role === 'Z-Move user') return 'Morium Z';
		if (species.id === 'monokuma' && role === 'Z-Move user') return 'Monokumium Z';
		// Non Z-Crystals
		...
		if (moves.has('sunnyday')) return 'Life Orb';
		...
		if (this.dex.getEffectiveness('Rock', species) >= 2 && species.id !== 'morian') return 'Heavy-Duty Boots';
	...
	randomCCVCTeam() {
		...
		while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
			...
			else if (!isMonotype && !this.forceMonotype) {
				let skip = false;
			...
			if (set.role === 'Z-Move user' || item.zMove) teamDetails.zMove = 1;
...
export default RandomCCVCTeams;