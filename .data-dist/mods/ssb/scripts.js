"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; } const Scripts = {
		...
		if (item.zMoveFrom) {
			...
			if (move.type === item.zMoveType) {
				if (move.category === "Status") {
					return move.name;
				} else if (_optionalChain([move, 'access', _3 => _3.zMove, 'optionalAccess', _4 => _4.basePower])) {
	...
	getActiveZMove(move, pokemon) {
		...
		if (pokemon) {
			...
			if (_optionalChain([zMoveFrom, 'optionalAccess', _5 => _5.includes, 'call', _6 => _6(move.name)])) {
	...
	canZMove(pokemon) {
		if (
			_optionalChain([pokemon, 'access', _7 => _7.m, 'optionalAccess', _8 => _8.zMoveUsed]) ||
	...
	runZPower(move, pokemon) {
		...
		} else if (_optionalChain([move, 'access', _9 => _9.zMove, 'optionalAccess', _10 => _10.boost])) {
			this.boost(move.zMove.boost, pokemon, pokemon, zPower);
		} else {
			switch (_optionalChain([move, 'access', _11 => _11.zMove, 'optionalAccess', _12 => _12.effect])) {
			case 'heal':
	...
	pokemon: {
		ignoringAbility() {
			...
			for (const foeActive of this.side.foe.active) {
				...
				if (_optionalChain([foeActive, 'optionalAccess', _13 => _13.ability, 'access', _14 => _14.includes, 'call', _15 => _15('neutralizingspores')]) && !_optionalChain([foeActive, 'optionalAccess', _16 => _16.volatiles, 'access', _17 => _17['gastroacid']])) sporeEffect = true;
			}
			for (const allyActive of this.side.active) {
				// allyActive can be null when a pokemon isn't active
				if (_optionalChain([allyActive, 'optionalAccess', _18 => _18.ability, 'access', _19 => _19.includes, 'call', _20 => _20('neutralizingspores')]) && !_optionalChain([allyActive, 'optionalAccess', _21 => _21.volatiles, 'access', _22 => _22['gastroacid']])) sporeEffect = true;
			}
		...
		setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
			...
			if (this.status === status.id) {
				if (_optionalChain([(sourceEffect ), 'optionalAccess', _23 => _23.status]) === this.status) {
					this.battle.add('-fail', this, this.status);
				} else if (_optionalChain([(sourceEffect ), 'optionalAccess', _24 => _24.status])) {
			...
			if (!ignoreImmunities && status.id &&
					!(_optionalChain([source, 'optionalAccess', _25 => _25.hasAbility, 'call', _26 => _26('corrosion')]) && ['tox', 'psn'].includes(status.id)) &&
					!(_optionalChain([sourceEffect, 'optionalAccess', _27 => _27.id]) === 'corrosivetoxic')) {
				// the game currently never ignores immunities
				if (!this.runStatusImmunity(status.id === 'tox' ? 'psn' : status.id)) {
					this.battle.debug('immune to status');
					if (_optionalChain([(sourceEffect ), 'optionalAccess', _28 => _28.status])) {
