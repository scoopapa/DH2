	...
	actions: {
		...
		runMove(
            ...
            if (move.flags['dance'] && moveDidSomething && !move.isExternal
                && pokemon.hasAbility('dancer') && !pokemon.isSemiInvulnerable() 
                && pokemon.activeMoveActions <= 1) 
            {
                const targetOf1stDance = this.battle.activeTarget!;
                if (!this.battle.faintMessages() && !pokemon.fainted) {
                    this.battle.add('-activate', pokemon, 'ability: Dancer');
                    this.runMove(move.id, pokemon, targetLoc, this.dex.abilities.get('dancer'), undefined, true);
                }
            }
