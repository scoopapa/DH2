...
exports.BattleAbilities = {
    ...
    "electricalefflux": {
        ...
        onStart: function (source) {
            this.field.setTerrain('electricalefflux');
        },
        onAnySetTerrain: function (target, source, terrain) {
            if (this.field.isTerrain('electricalefflux') && !(terrain.id in {electricalefflux:1, psychicpour:1, grassygush:1, mistymount:1})) return false;
        },
        onEnd: function (pokemon) {
	...
	"evergreen": {
		...
		onTryHit: function (target, source, move) {
			if (target !== source && move.type === 'Ice') {
				if (!this.heal(target.maxhp / 4)) {
					this.add('-immune', target, '[msg]', '[from] ability: Evergreen');
				}
				return null;
			}
		},
    ...
    "grassygush": {
        ...
        onAnySetTerrain: function (target, source, terrain) {
            if (this.field.isTerrain('grassygush') && !(terrain.id in {electricalefflux:1, psychicpour:1, grassygush:1, mistymount:1})) return false;
        },
        onEnd: function (pokemon) {
            if (this.terrainData.source !== pokemon) return;
            for (let i = 0; i < this.sides.length; i++) {
    ...
    "mistymount": {
        ...
        onAnySetTerrain: function (target, source, terrain) {
            if (this.field.isTerrain('mistymount') && !(terrain.id in {electricalefflux:1, psychicpour:1, grassygush:1, mistymount:1})) return false;
        },
        onEnd: function (pokemon) {
            if (this.terrainData.source !== pokemon) return;
            for (let i = 0; i < this.sides.length; i++) {
    ...
    "psychicpour": {
        ...
        onAnySetTerrain: function (target, source, terrain) {
            if (this.field.isTerrain('psychicpour') && !(terrain.id in {electricalefflux:1, psychicpour:1, grassygush:1, mistymount:1})) return false;
        },
        onEnd: function (pokemon) {
            if (this.terrainData.source !== pokemon) return;
            for (let i = 0; i < this.sides.length; i++) {
	...
	"pyromaniac": {
		...
		onModifyCritRatio: function (critRatio, source, target) {
			if (target && target.status in {'brn':1}) return 5;
		},
		id: "pyromaniac",
		name: "Pyromaniac",
		rating: 2,
