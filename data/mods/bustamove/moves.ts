...
export const Moves: {[moveid: string]: MoveData} = {
	...
	eternabeam: {
		...
		onAfterMove(pokemon, target, move) {
			if (move.mindBlownRecoil && !move.multihit) {
				this.damage(Math.round(pokemon.maxhp / 10), pokemon, pokemon, this.dex.conditions.get('Eternabeam'), true);
			}
		},
		secondary: null,
		target: "normal",
	...
	ominouswind: {
		...
		flags: {protect: 1, mirror: 1},
		onAfterMoveSecondarySelf(pokemon, target, move) {
			if (!target || target.fainted || target.hp <= 0) { this.boost({atk: 1, def: 1, spa: 1, spd: 1, spe: 1}, pokemon, pokemon, move); } else { this.damage(pokemon.baseMaxhp / 8); }
		},
		secondary: null,
		target: "normal",
		type: "Ghost",
	...
	selfdestruct: {
		...
		onAfterMove(pokemon, target, move) {
			if (move.mindBlownRecoil && !move.multihit) {
				this.damage(Math.round(pokemon.maxhp / 2), pokemon, pokemon, this.dex.conditions.get('Self-Destruct'), true);
			}
		},
		secondary: null,
		target: "normal",
