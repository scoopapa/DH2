...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	healer: {
		...
		onResidual(pokemon) {
			...
			for (const allyActive of pokemon.side.active) {
				if (
					allyActive &&
					(allyActive.hp && pokemon.isAdjacent(allyActive) && allyActive.status) && this.randomChance(3, 10)
				) {
	...
	lightningfists: {
		onModifyPriority(priority, pokemon, target, move) {
			if (move?.flags['punch'] && pokemon.hp > pokemon.maxhp / 1.75) return priority + 1;
		},
	...
	rebirth: {
		...
		rating: 2.5,
		num: 106,	
		shortdesc: "The user uses healing wish upon fainting from a damaging move.",
		desc: "The user uses healing wish upon fainting from a damaging move.",
	},
	exmachina: {
	...
	lambentlight: {
		onStart(source) {
			this.useMove("Magic Powder", source);
		},
		name: "Lambent Light",
		rating: 4.5,
		num: 7001,
