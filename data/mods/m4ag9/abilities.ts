...
export const Abilities: { [abilityid: string]: ModdedAbilityData; } = {
	...
	iceface: {
		inherit: true,
		onStart(pokemon) {
			if (this.field.isWeather(['hail', 'diamonddust', 'snow']) && (pokemon.species.id === 'eiscuenoice' || pokemon.species.id === 'perrserkermegabusted')) {
		...
		onWeatherChange(pokemon, source, sourceEffect) {
			...
			if (this.field.isWeather(['hail', 'diamonddust', 'snow']) && pokemon.species.id === 'eiscuenoice') {
				this.add('-activate', pokemon, 'ability: Ice Face');
				this.effectState.busted = false;
				pokemon.formeChange('Eiscue', this.effect, true);
			}
			else if (this.field.isWeather(['hail', 'diamonddust', 'snow']) && pokemon.species.id === 'perrserkermegabusted') {
				this.add('-activate', pokemon, 'ability: Ice Face');
				this.effectState.busted = false;
				pokemon.formeChange('Perrserker-Mega', this.effect, true);
			}
		},
		desc: "If this Pokemon is an Eiscue or a Perrserker-Mega, the first physical hit it takes in battle deals 0 neutral damage. Its ice face is then broken and it changes forme to Noice Face. Eiscue regains its Ice Face forme when Snow begins or when Eiscue switches in while Snow, Hail or Diamond Dust is active. Confusion damage also breaks the ice face.",
		shortDesc: "If Eiscue or Perrserker-Mega, the first physical hit it takes deals 0 damage. Effect is restored in Snow, Hail, Diamond Dust.",
	},
	slushrush: {
		inherit: true,
