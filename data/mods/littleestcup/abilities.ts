...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	leafguard: {
		onEffectiveness(typeMod, target, type, move) {
			if (type === 'Grass' && typeMod > 0) return -1;
		},
		flags: {breakable: 1},
		shortDesc: "The user's Grass type weaknesses become resists.",
		name: "Leaf Guard",
		rating: 0.5,
		num: 102,
	},
...
*/
	...
	pickpocket: {
		...
		flags: {},
		shortDesc: "If this Pokemon has no item and is hit by a move, it steals the attacker's item",
		name: "Pickpocket",
		rating: 1,
		num: 124,
	},
	...
	purepower: {
		...
		onModifyMove(move, pokemon) {
			if (['seismictoss', 'psywave', 'nightshade'].includes(move.id)) {
				move.damage = 200;
			}
			if (['dragonrage'].includes(move.id)) {
				move.damage = 80;
			}
			if (['sonicboom'].includes(move.id)) {
				move.damage = 40;
			}
		},
		flags: {},
		shortDesc: "This Pokemon deals twice the amount of damage they would normally.",
		name: "Pure Power",
		rating: 5,
		num: 74,
	},
	...
	sniper: {
		...
		flags: {},
		shortDesc: "This Pokémon strikes with a critical hit, the damage is multiplied by 1.5.",
		name: "Sniper",
		rating: 2,
		num: 97,
	},
	...
	solidrock: {
		...
		onHit(target, source, move) {
			if (target.getMoveHitData(move).typeMod > 0) {
				this.damage(33, target, source);
			}
		},
		flags: {breakable: 1},
		shortDesc: "This Pokemon receives 34 damage from supereffective attacks.",
		name: "Solid Rock",
		rating: 3,
		num: 116,
	},
	...
	supersweetsyrup: {
		...
		flags: {},
		shortDesc: "On switch-in, this Pokemon lowers the evasiveness of opponents 1 stage. Once per battle, this Pokemon restores 1/3 of its maximum HP, rounded down, when it switches out. ",
		name: "Supersweet Syrup",
		rating: 1.5,
		num: 306,
	},
	...
	swarm: {
		...
		flags: {},
		shortDesc: "At 1/3 or more of its max HP, this Pokemon's offensive stat is 1.5x with Bug attacks.",
		name: "Swarm",
		rating: 2,
		num: 68,
	},
	...
	tintedlens: {
		...
		// placeholder for the effect, i believe coding it as it literally states makes it never double the power. would have to find a way to account for 4x resists
		flags: {},
		shortDesc: "This Pokemon's attacks that are not very effective on a target become neutral and deal 2x damage.",
		name: "Tinted Lens",
		rating: 4,
		num: 110,
	},
	...
	unnerve: {
		...
		onStart(pokemon) {
			...
			for (const foes of pokemon.side.foe.active) {
				const item = foes.getItem();
				const items = [
					'berryjuice', 'mentalherb', 'powerherb', 'absorbbulb', 'adrenalineorb', 'airballoon', 'berserkgene', 'blunderpolicy', 'cellbattery',
					'ejectbutton', 'ejectpack', 'electricseed', 'grassyseed', 'mistyseed', 'psychicseed', 'focussash', 'focusband', 'luminousmoss', 'mirrorherb',
					'redcard', 'roomservice', 'throatspray', 'boosterenergy',
				];
				if (!activated) {
					this.add('-ability', pokemon, 'Unnerve');
				}
				activated = true;
				if (!foes.volatiles['embargo'] && (items.includes(foes.item) || item.isGem)) {
					foes.addVolatile('embargo');
				}
			}
		},
		onAnySwitchIn(pokemon) {
			...
			for (const target of source.side.foe.active) {
				...
				if (!target.volatiles['embargo'] && (items.includes(target.item) || item.isGem)) {
		...
		flags: {},
		shortDesc: "While this Pokemon is active, it prevents opposing Pokemon from using their consumable items.",
		name: "Unnerve",
		rating: 1,
		num: 127,
	},
