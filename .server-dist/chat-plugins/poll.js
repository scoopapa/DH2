...
var _utils = require('../../.lib-dist/utils');
 ...
 class Poll {
	...
	constructor(room, questionData, options, multi) {
	...
	generateVotes(user) {
		...
		if (this.multiPoll) {
			...
			for (const [num, option] of this.options) {
				...
				output += `<div style="margin-top: 5px"><button style="text-align: left; border: none; background: none; color: inherit;" value="/poll ${selected ? 'de' : ''}select ${num}" name="send" title="${selected ? "Deselect" : "Select"} ${num}. ${_utils.Utils.escapeHTML(option.name)}">${selected ? "<strong>" : ''}${selected ? chosen : empty} ${num}. ${this.getOptionMarkup(option)}${selected ? "</strong>" : ''}</button></div>`;
			}
			...
			for (const [num, option] of this.options) {
				output += `<div style="margin-top: 5px"><button class="button" style="text-align: left" value="/poll vote ${num}" name="send" title="${this.room.tr`Vote for ${num}`}. ${_utils.Utils.escapeHTML(option.name)}">${num}. <strong>${this.getOptionMarkup(option)}</strong></button></div>`;
			}
	...
	generateResults(ended = false, option = null) {
		const iconText = this.isQuiz ?
			`<i class="fa fa-question"></i> ${this.room.tr`Quiz`}` :
			`<i class="fa fa-bar-chart"></i> ${this.room.tr`Poll`}`;
		...
		while (!i.done) {
			const selected = _optionalChain([option, 'optionalAccess', _ => _.includes, 'call', _2 => _2(i.value[0])]);
	...
	poll: {
		...
		deletequeue(target, room, user, connection, cmd) {
			...
			if (!target) {
				...
				if (!_optionalChain([curRoom, 'access', _3 => _3.minorActivityQueue, 'optionalAccess', _4 => _4.length])) curRoom.minorActivityQueue = null;
		...
		timer(target, room, user) {
			...
			if (target) {
				...
				poll.timeout = setTimeout(() => {
					...
					if (_optionalChain([room, 'access', _5 => _5.minorActivityQueue, 'optionalAccess', _6 => _6.length])) {
		...
		stop: 'end',
			...
			poll.end();
			room.minorActivity = null;
			if (_optionalChain([room, 'access', _7 => _7.minorActivityQueue, 'optionalAccess', _8 => _8.length])) {
 ...
 const pages = {
	pollqueue(args, user) {
		...
		if (!_optionalChain([room, 'access', _9 => _9.minorActivityQueue, 'optionalAccess', _10 => _10.length])) {
