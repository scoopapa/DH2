...
export const Scripts: ModdedBattleScriptsData = {
	...
	pokemon: {
		...
		getSmartTargets(target: Pokemon, move: ActiveMove) { //Triple Battle targeting and Commander/Play Dead
			...
			if (target.nearbyAllies().length === 2 && (this.tacticianBoosted || this.position === 1)){
				const targetPos = this.battle.sample([0, 1]); //Pick random ally
				target2 = target.nearbyAllies()[targetPos];
				if (!target2 || target2 === this || !target2.hp || !target2.isValidTarget()) { //Ally is invalid, try the other
					target2 = target.nearbyAllies()[1 - targetPos];
				...
				if (target === this || !target.hp || !target.isValidTarget()) {
					target = target.nearbyAllies()[1 - targetPos]; //Check other ally
		...
		getStrongestMove(target: Pokemon, seeItem?: boolean){ //New function used by Forewarn & Glyphic Spell for finding the move to reveal
			...
			for (const moveSlot of this.moveSlots) {
				...
				if(!direct){ //Further calculations
					//STAB
					if(move.onModifyType) move.onModifyType(move, target);
					bp *= target.hasType(move.type) ? 1.5 : 1;
					bp *= (move.twoType && target.hasType(move.twoType)) ? 1.5 : 1;
					//Type effectiveness
					bp *= Math.pow(2, this.battle.dex.conditions.getiveness(move, target));
					if (move.multihit){
		...
		transformInto(pokemon: Pokemon, effect?: Effect) { //Copy Power Trick, and Glyphic Spell if you're an Unown too
			...
			if(pokemon.species.baseSpecies === 'Unown' && this.species.baseSpecies === 'Unown' && pokemon.abilityData.unownType){
				this.abilityData.unownType = pokemon.abilityData.unownType;
				delete this.abilityData.formeDecided; //will need to re-assign Copy spell on next switch
			}
			if (effect) {
	...
	field: {
		...
		setTerrain(status: string | Effect, source: Pokemon | 'debug' | null = null, sourceEffect: Effect | null = null) { //Add SetTerrain event check
			status = this.battle.dex.conditions.get(status);
	...
	battle: {
		validTargetLoc(targetLoc: number, source: Pokemon, targetType: string) { //Tactician and ally or self targeting
			...
			case 'adjacentAllyOrSelf':
				return isSelf || ((canTargetAny || isAdjacent) && !isFoe);
			case 'anyAlly':
				return !isFoe;
			case 'adjacentFoe':
				return (canTargetAny || isAdjacent) && isFoe;
		...
		getTarget(pokemon: Pokemon, move: string | Move, targetLoc: number, originalTarget?: Pokemon) { //Play Dead, Commander, Ally Switch
			...
			if (pokemon.hasAbility(['stalwart', 'againstcurrent'])){
				tracksTarget = true;
				//console.log("Targeting spot " + targetLoc);
				//console.log("Target is " + originalTarget + " at " + originalTarget.position);
			}
			if (tracksTarget && originalTarget && originalTarget.isActive && (targetLoc === originalTarget.position + 1) && originalTarget.isValidTarget()) {
				// smart-tracking: move's original target is on the field, not Ally Switched, and not Commanding or Playing Dead: target it
			...
			if (move.target !== 'randomNormal' && this.validTargetLoc(targetLoc, pokemon, move.target)) {
				...
				if (target && target.isValidTarget()) {
					// Target is unfainted and not Playing Dead/Commanding: use selected target location
					return target;
				}
	...
	actions: {
		...
		canMegaEvo(pokemon) { //Magic Room suppression, Mega-Ray change, Mega Evolution friendship req
			if('magicroom' in this.field.pseudoWeather || pokemon.happiness < 180) return null;
		...
		hitStepBreakProtect(targets, pokemon, move) { //New protecting moves and breakers
	...
	//dex: {
		...
		getBestEffectiveness(
			...
			if(resistFirst){
				...
				if(typesTest.length){
					...
					if(Array.isArray(targetTyping)){
						...
						for(const targetType in targetTyping){
							typesTest = typesTest.concat(bestTypes.filter(type => dex.conditions.getiveness(targetType, type) < 0)); //No immunities means these are only resistances
						}
						bestTypes = Object.assign([], typesTest);
					} else {
						bestTypes = bestTypes.filter(type => dex.conditions.getiveness(targetTyping, type) < 0);
					}
					...
					if(bestTypes.length){
						...
						if(targetTyping.length){
							typesTest = [];
							for(const targetType in targetTyping){
								typesTest = typesTest.concat(bestTypes.filter(type => dex.conditions.getiveness(targetType, type) === -2)); //No immunities means these are only resistances
							}
						} else typesTest = bestTypes.filter(type => dex.conditions.getiveness(targetTyping, type) === -2);
				...
				typesTest = bestTypes.filter(type => dex.conditions.getiveness(type, targetTyping) > 0 && dex.getImmunity(type, targetTyping));
				...
				if(typesTest.length){
					...
					bestTypes = Object.assign([], typesTest);
					//Check x4
					typesTest = bestTypes.filter(type => dex.conditions.getiveness(type, targetTyping) === 2);
				...
				bestTypes = bestTypes.filter(type => dex.conditions.getiveness(type, targetTyping) > 0 && dex.getImmunity(type, targetTyping));
				...
				if(typesTest.length){
					...
					} else typesTest = bestTypes.filter(type => dex.conditions.getiveness(targetTyping, type) < 0);
	...
	init() {
		...
		for (let pokemon of this.species.all()) {
					...
					case undefined:
						if(!pokemon.battleOnly){ //Reset tiers for all Pokemon that have their own tiering data
							if(pokemon.evos) {
								...
								this.modData('Pokedex', pokemonID).tier = pokemon.prevo ? "NFE" : "LC";
							} else {
			...
			for(let move of this.moves.all()) {
				...
				if(moveID === "lashout" && moveMeans == "9M"){ //Gen 8 is machine, ES is not
					...
					this.modData('Learnsets', pokemonID).learnset[this.toID('compensation')] = ["9M"];
		...
		/*for(let i = 0; i < renamedPokemon.length; i++){
			...
			if(!this.modData('Pokedex', oldID).battleOnly) this.data['Learnsets'][newID] = this.modData('Learnsets', oldID);
		...
		this.modData("Learnsets", "charmander").learnset.flamethrower = ["9L32"];
		...
		this.modData("Learnsets", "charmeleon").learnset.flamethrower = ["9L42"];
		// Charizard
		...
		this.modData("Learnsets", "charizard").learnset.flamethrower = ["9L46"];
		...
		delete this.modData('Learnsets', 'venomoth').learnset.bugbuzz;
		...
		this.modData("Learnsets", "golduck").learnset.nastyplot = ["9M"];
		this.modData("Learnsets", "golduck").learnset.powergem = ["9M"];
		delete this.modData('Learnsets', 'golduck').learnset.blizzard;
		// Mankey
		this.modData("Learnsets", "mankey").learnset.megapunch = ["9D"];
		this.modData("Learnsets", "mankey").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "primeape").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "poliwag").learnset.chillywater = ["9M"];
		delete this.modData('Learnsets', 'poliwag').learnset.blizzard;
		// Poliwhirl
		this.modData("Learnsets", "poliwhirl").learnset.slipaway = ["9D"];
		this.modData("Learnsets", "poliwhirl").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "poliwrath").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "farfetchd").learnset.cut = ["9L35"];
		this.modData("Learnsets", "farfetchd").learnset.retaliate = ["9L55", "9M"];
		this.modData("Learnsets", "farfetchd").learnset.leafblade = ["9E"];
		...
		this.modData("Learnsets", "doduo").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "dodrio").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "seel").learnset.charm = ["9M"];
		this.modData("Learnsets", "seel").learnset.chillywater = ["9M"];
		// Dewgong
		...
		this.modData("Learnsets", "dewgong").learnset.charm = ["9M"];
		this.modData("Learnsets", "dewgong").learnset.chillywater = ["9M"];
		// Grimer
		...
		this.modData("Learnsets", "voltorb").learnset.lightscreen = ["9L34"];
		this.modData("Learnsets", "voltorb").learnset.magnetrise = ["9L37"];
		this.modData("Learnsets", "voltorb").learnset.discharge = ["9L41"];
		this.modData("Learnsets", "voltorb").learnset.explosion = ["9L46"];
		this.modData("Learnsets", "voltorb").learnset.electricterrain = ["9M"];
		...
		this.modData("Learnsets", "voltorbhisui").learnset.magnetrise = ["9L37"];
		this.modData("Learnsets", "voltorbhisui").learnset.discharge = ["9L41"];
		this.modData("Learnsets", "voltorbhisui").learnset.explosion = ["9L46"];
		this.modData("Learnsets", "voltorbhisui").learnset.electricterrain = ["9M"];
		...
		this.modData("Learnsets", "electrode").learnset.discharge = ["9L47","9M"];
		this.modData("Learnsets", "electrode").learnset.explosion = ["9L54","9M"];
		this.modData("Learnsets", "electrode").learnset.electricterrain = ["9M"];
		...
		// Lapras
		this.modData("Learnsets", "lapras").learnset.sparklingaria = ["9D"];
		...
		// Jolteon
		this.modData("Learnsets", "jolteon").learnset.zingzap = ["9D"];
		// Flareon
		this.modData("Learnsets", "flareon").learnset.rage = ["9D"];
		this.modData("Learnsets", "flareon").learnset.flash = ["9M"];
		this.modData("Learnsets", "flareon").learnset.preheat = ["9L33"];
		...
		this.modData("Learnsets", "snorlax").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "furret").learnset.chillywater = ["9M"];
		this.modData("Learnsets", "furret").learnset.faketears = ["9M"];
		this.modData("Learnsets", "furret").learnset.incinerate = ["9M"];
		this.modData("Learnsets", "furret").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "politoed").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "murkrow").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "misdreavus").learnset.poltergeist = ["9M"];
		...
		this.modData("Learnsets", "qwilfishhisui").learnset.compensation = ["9M"];
		...
		// Remoraid
		this.modData("Learnsets", "remoraid").learnset.laserfocus = ["9D"];
		...
		this.modData("Learnsets", "houndour").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "houndoom").learnset.compensation = ["9M"];
		...
		// Kingdra
		this.modData("Learnsets", "kingdra").learnset.storedpower = ["9D"];
		...
		this.modData("Learnsets", "raikou").learnset.electroball = ["9M"];
		this.modData("Learnsets", "raikou").learnset.flash = ["9M"];
		this.modData("Learnsets", "raikou").learnset.trailhead = ["9M"];
		// Entei
		this.modData("Learnsets", "entei").learnset.napalm = ["9D"];
		this.modData("Learnsets", "entei").learnset.flash = ["9M"];
		this.modData("Learnsets", "entei").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "suicune").learnset.chillywater = ["9M"];
		this.modData("Learnsets", "suicune").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "treecko").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "grovyle").learnset.trailhead = ["9M"];
		// Sceptile
		...
		this.modData("Learnsets", "sceptile").learnset.trailhead = ["9M"];
		// Torchic
		...
		this.modData("Learnsets", "mudkip").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "marshtomp").learnset.wavecrash = ["9L42"];
		this.modData("Learnsets", "marshtomp").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "swampert").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "poochyena").learnset.compensation = ["9M"];
		this.modData("Learnsets", "poochyena").learnset.toxic = ["9M"];
		this.modData("Learnsets", "poochyena").learnset.trailhead = ["9M"];
		// Mightyena
		this.modData("Learnsets", "mightyena").learnset.partingshot = ["9D"];
		this.modData("Learnsets", "mightyena").learnset.compensation = ["9M"];
		this.modData("Learnsets", "mightyena").learnset.toxic = ["9M"];
		this.modData("Learnsets", "mightyena").learnset.trailhead = ["9M"];
		// Zigzagoon
		...
		this.modData("Learnsets", "lotad").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "lombre").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "ludicolo").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "seedot").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "nuzleaf").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "shiftry").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "masquerain").learnset.terrify = ["9L22"];
		...
		this.modData("Learnsets", "vigoroth").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "slaking").learnset.bodypress = ["9M"];
		this.modData("Learnsets", "slaking").learnset.chillywater = ["9M"];
		this.modData("Learnsets", "slaking").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "hariyama").learnset.bodypress = ["9L34","9M"];
		...
		this.modData("Learnsets", "camerupt").learnset.bodypress = ["9M"];
		this.modData("Learnsets", "camerupt").learnset.compensation = ["9M"];
		// Torkoal
		...
		this.modData("Learnsets", "flygon").learnset.dragonclaw = ["9L1", "9M"];
		this.modData("Learnsets", "flygon").learnset.fellswoop = ["9L47"];
		this.modData("Learnsets", "flygon").learnset.boomburst = ["9L53"];
		// Cacnea
		...
		this.modData("Learnsets", "cacturne").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "seviper").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "corphish").learnset.chillywater = ["9M"];
		this.modData("Learnsets", "corphish").learnset.compensation = ["9M"];
		// Crawdaunt
		this.modData("Learnsets", "crawdaunt").learnset.muddywater = ["9D"];
		this.modData("Learnsets", "crawdaunt").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "shuppet").learnset.poltergeist = ["9M"];
		...
		this.modData("Learnsets", "banette").learnset.poltergeist = ["9M"];
		...
		this.modData("Learnsets", "rayquaza").learnset.hypervoice = ["9L27"];
		...
		// Jirachi
		this.modData("Learnsets", "jirachi").learnset.meteormash = ["9D"];
		...
		this.modData("Learnsets", "turtwig").learnset.trailhead = ["9M"];
		// Grotle
		...
		this.modData("Learnsets", "grotle").learnset.trailhead = ["9M"];
		// Torterra
		...
		this.modData("Learnsets", "torterra").learnset.trailhead = ["9M"];
		// Chimchar
		...
		this.modData("Learnsets", "empoleon").learnset.steelbeam = ["9T"];
		// Starly
		...
		this.modData("Learnsets", "wormadam").learnset.hiddenpower = ["9L0","9M"];
		this.modData("Learnsets", "wormadam").learnset.petaldance = ["9L1"];
		...
		this.modData("Learnsets", "wormadamsandy").learnset.hiddenpower = ["9L0","9M"];
		this.modData("Learnsets", "wormadamsandy").learnset.shoreup = ["9L1"];
		this.modData("Learnsets", "wormadamsandy").learnset.sandtomb = ["9L26"];
		...
		this.modData("Learnsets", "wormadamtrash").learnset.hiddenpower = ["9L0","9M"];
		...
		// Mismagius
		this.modData("Learnsets", "mismagius").learnset.healblock = ["9D"];
		...
		this.modData("Learnsets", "mismagius").learnset.poltergeist = ["9M"];
		delete this.modData('Learnsets', 'mismagius').learnset.thunder;
		// Honchkrow
		this.modData("Learnsets", "honchkrow").learnset.midnight = ["9D"];
		this.modData("Learnsets", "honchkrow").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "chatot").learnset.partingshot = ["9L53"];
		...
		this.modData("Learnsets", "spiritomb").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "leafeon").learnset.sunnyday = ["9L27","9M"];
		this.modData("Learnsets", "leafeon").learnset.leafblade = ["9L37"];
		this.modData("Learnsets", "leafeon").learnset.solarblade = ["9L45"];
		...
		this.modData("Learnsets", "glaceon").learnset.icebeam = ["9L37","9M"];
		this.modData("Learnsets", "glaceon").learnset.trailhead = ["9M"];
		delete this.modData('Learnsets', 'glaceon').learnset.iceshard;
		// Gliscor
		this.modData("Learnsets", "gliscor").learnset.skydrop = ["9D"];
		this.modData("Learnsets", "gliscor").learnset.breakingswipe = ["9M"];
		...
		this.modData("Learnsets", "probopass").learnset.steelbeam = ["9M"];
		// Dusknoir
		...
		this.modData("Learnsets", "rotomwash").learnset.flash = ["9M"];
		this.modData("Learnsets", "rotomwash").learnset.hex = ["9M"];
		...
		this.modData("Learnsets", "rotomheat").learnset.flash = ["9M"];
		this.modData("Learnsets", "rotomheat").learnset.hex = ["9M"];
		...
		this.modData("Learnsets", "rotomfrost").learnset.icywind = ["9L1"];
		...
		this.modData("Learnsets", "rotomfrost").learnset.flash = ["9M"];
		this.modData("Learnsets", "rotomfrost").learnset.hex = ["9M"];
		...
		this.modData("Learnsets", "rotomfan").learnset.flash = ["9M"];
		this.modData("Learnsets", "rotomfan").learnset.hex = ["9M"];
		...
		this.modData("Learnsets", "rotommow").learnset.flash = ["9M"];
		this.modData("Learnsets", "rotommow").learnset.hex = ["9M"];
		...
		// Regigigas
		this.modData("Learnsets", "regigigas").learnset.storedpower = ["9D"];
		...
		this.modData("Learnsets", "giratina").learnset.shadowclaw = ["9L64","9M"];
		...
		this.modData("Learnsets", "phione").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "manaphy").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "darkrai").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "shayminsky").learnset.amnesia = ["9M"];
		this.modData("Learnsets", "shayminsky").learnset.flash = ["9M"];
		...
		this.modData("Learnsets", "arceus").learnset.roaroftime = ["9E"];
		this.modData("Learnsets", "arceus").learnset.shadowforce = ["9E"];
		this.modData("Learnsets", "arceus").learnset.spacialrend = ["9E"];
		// Victini
		...
		this.modData("Learnsets", "victini").learnset.blueflare = ["9E"];
		this.modData("Learnsets", "victini").learnset.boltstrike = ["9E"];
		...
		this.modData("Learnsets", "blitzle").learnset.trailhead = ["9M"];
		// Zebstrika
		this.modData("Learnsets", "zebstrika").learnset.volttackle = ["9D"];
		this.modData("Learnsets", "zebstrika").learnset.eerieimpulse = ["9M"];
		this.modData("Learnsets", "zebstrika").learnset.electricterrain = ["9M"];
		this.modData("Learnsets", "zebstrika").learnset.flash = ["9M"];
		this.modData("Learnsets", "zebstrika").learnset.trailhead = ["9M"];
		// Roggenrola
		...
		this.modData("Learnsets", "boldore").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "basculinbluestriped").learnset.glare = ["9D"];
		this.modData("Learnsets", "basculinbluestriped").learnset.compensation = ["9M"];
		this.modData("Learnsets", "basculinbluestriped").learnset.whitewater = ["9L8"];
		this.modData("Learnsets", "basculinbluestriped").learnset.flail = ["9L12"];
		this.modData("Learnsets", "basculinbluestriped").learnset.aquajet = ["9L16"];
		this.modData("Learnsets", "basculinbluestriped").learnset.bite = ["9L20"];
		this.modData("Learnsets", "basculinbluestriped").learnset.scaryface = ["9L24"];
		this.modData("Learnsets", "basculinbluestriped").learnset.headbutt = ["9L28"];
		this.modData("Learnsets", "basculinbluestriped").learnset.soak = ["9L32"];
		this.modData("Learnsets", "basculinbluestriped").learnset.crunch = ["9L36"];
		this.modData("Learnsets", "basculinbluestriped").learnset.takedown = ["9L40"];
		this.modData("Learnsets", "basculinbluestriped").learnset.metaledge = ["9L44"];
		this.modData("Learnsets", "basculinbluestriped").learnset.finalgambit = ["9L48"];
		this.modData("Learnsets", "basculinbluestriped").learnset.doubleedge = ["9L52"];
		this.modData("Learnsets", "basculinbluestriped").learnset.thrash = ["9L56"];
		this.modData("Learnsets", "basculinbluestriped").learnset.wavecrash = ["9L60"];
		this.modData("Learnsets", "basculinbluestriped").learnset.headsmash = ["9L64"];
		delete this.modData('Learnsets', 'basculinbluestriped').learnset.blizzard;
		delete this.modData('Learnsets', 'basculinbluestriped').learnset.submission;
		// Basculin White-Striped
		this.modData("Learnsets", "basculinwhitestriped").learnset = this.modData("Learnsets", "basculin");
		...
		this.modData("Learnsets", "dwebble").learnset.stealthrock = ["9L25", "9M"];
		this.modData("Learnsets", "dwebble").learnset.shelter = ["9L55"];
		this.modData("Learnsets", "dwebble").learnset.rockwrecker = ["9L62"];
		...
		// Garbodor
		this.modData("Learnsets", "garbodor").learnset.irontail = ["9D"];
		...
		// Reuniclus
		this.modData("Learnsets", "reuniclus").learnset.aquaring = ["9D"];
		...
		this.modData("Learnsets", "ducklett").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "swanna").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "mienfoo").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "mienshao").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "meloetta").learnset.round = ["9L1"];
		...
		this.modData("Learnsets", "meloetta").learnset.encore = ["9L29"];
		this.modData("Learnsets", "meloetta").learnset.echoedvoice = ["9L36"];
		this.modData("Learnsets", "meloetta").learnset.psybeam = ["9L43"];
		this.modData("Learnsets", "meloetta").learnset.relicsong = ["9L50"];
		this.modData("Learnsets", "meloetta").learnset.hypervoice = ["9L57"];
		this.modData("Learnsets", "meloetta").learnset.psychic = ["9L64"];
		this.modData("Learnsets", "meloetta").learnset.roleplay = ["9L71"];
		...
		this.modData("Learnsets", "meloettapirouette").learnset.encore = ["9L29"];
		this.modData("Learnsets", "meloettapirouette").learnset.acrobatics = ["9L36"];
		this.modData("Learnsets", "meloettapirouette").learnset.wakeupslap = ["9L43"];
		this.modData("Learnsets", "meloettapirouette").learnset.relicsong = ["9L50"];
		this.modData("Learnsets", "meloettapirouette").learnset.uturn = ["9L57"];
		this.modData("Learnsets", "meloettapirouette").learnset.closecombat = ["9L64"];
		this.modData("Learnsets", "meloettapirouette").learnset.roleplay = ["9L71"];
		this.modData("Learnsets", "meloettapirouette").learnset.victorydance = ["9L78"];
		this.modData("Learnsets", "meloettapirouette").learnset.chillywater = ["9M"];
		this.modData("Learnsets", "meloettapirouette").learnset.flash = ["9M"];
		...
		this.modData("Learnsets", "spritzee").learnset.strangesmoke = ["9L31"];
		...
		this.modData("Learnsets", "aromatisse").learnset.strangesmoke = ["9L31"];
		...
		this.modData("Learnsets", "sliggoohisui").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "zygarde").learnset.crunch = ["9L20"];
		this.modData("Learnsets", "zygarde").learnset.safeguard = ["9L25","9M"];
		this.modData("Learnsets", "zygarde").learnset.glare = ["9L30"];
		this.modData("Learnsets", "zygarde").learnset.sandstorm = ["9L35","9M"];
		this.modData("Learnsets", "zygarde").learnset.dragonpulse = ["9L40","9M"];
		this.modData("Learnsets", "zygarde").learnset.coil = ["9L45"];
		this.modData("Learnsets", "zygarde").learnset.landswrath = ["9L50"];
		this.modData("Learnsets", "zygarde").learnset.extremespeed = ["9L55"];
		this.modData("Learnsets", "zygarde").learnset.dragonrush = ["9L60"];
		this.modData("Learnsets", "zygarde").learnset.dragondance = ["9L65"];
		this.modData("Learnsets", "zygarde").learnset.earthquake = ["9L70","9M"];
		this.modData("Learnsets", "zygarde").learnset.outrage = ["9L75","9M"];
		this.modData("Learnsets", "zygarde").learnset.thousandwaves = ["9L80"];
		this.modData("Learnsets", "zygarde").learnset.thousandarrows = ["9L80"];
		this.modData("Learnsets", "zygarde").learnset.coreenforcer = ["9L85"];
		this.modData("Learnsets", "zygarde").learnset.avalanche = ["9M"];
		this.modData("Learnsets", "zygarde").learnset.bodypress = ["9M"];
		this.modData("Learnsets", "zygarde").learnset.trailhead = ["9M"];
		this.modData("Learnsets", "zygarde").learnset.stasis = ["9T"];
		// Zygarde 10%
		this.modData("Learnsets", "zygarde10").learnset.equalizer = ["9D"];
		this.modData("Learnsets", "zygarde10").learnset.bind = ["9L1"];
		this.modData("Learnsets", "zygarde10").learnset.dig = ["9L1","9M"];
		this.modData("Learnsets", "zygarde10").learnset.dragonbreath = ["9L1"];
		this.modData("Learnsets", "zygarde10").learnset.bite = ["9L1"];
		this.modData("Learnsets", "zygarde10").learnset.haze = ["9L5"];
		this.modData("Learnsets", "zygarde10").learnset.bulldoze = ["9L10","9M"];
		this.modData("Learnsets", "zygarde10").learnset.camouflage = ["9L15"];
		this.modData("Learnsets", "zygarde10").learnset.crunch = ["9L20"];
		this.modData("Learnsets", "zygarde10").learnset.safeguard = ["9L25","9M"];
		this.modData("Learnsets", "zygarde10").learnset.glare = ["9L30"];
		this.modData("Learnsets", "zygarde10").learnset.sandstorm = ["9L35","9M"];
		this.modData("Learnsets", "zygarde10").learnset.dragonpulse = ["9L40","9M"];
		this.modData("Learnsets", "zygarde10").learnset.coil = ["9L45"];
		this.modData("Learnsets", "zygarde10").learnset.landswrath = ["9L50"];
		this.modData("Learnsets", "zygarde10").learnset.extremespeed = ["9L55"];
		this.modData("Learnsets", "zygarde10").learnset.dragonrush = ["9L60"];
		this.modData("Learnsets", "zygarde10").learnset.dragondance = ["9L65"];
		this.modData("Learnsets", "zygarde10").learnset.earthquake = ["9L70","9M"];
		this.modData("Learnsets", "zygarde10").learnset.outrage = ["9L75","9M"];
		this.modData("Learnsets", "zygarde10").learnset.thousandwaves = ["9L80"];
		this.modData("Learnsets", "zygarde10").learnset.thousandarrows = ["9L80"];
		this.modData("Learnsets", "zygarde10").learnset.coreenforcer = ["9L85"];
		this.modData("Learnsets", "zygarde10").learnset.trailhead = ["9M"];
		this.modData("Learnsets", "zygarde10").learnset.stasis = ["9T"];
		// Diancie
		...
		// Volcanion
		this.modData("Learnsets", "volcanion").learnset.preheat = ["9D"];
		...
		this.modData("Learnsets", "torracat").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "incineroar").learnset.trailhead = ["9M"];
		...
		// Grubbin
		this.modData("Learnsets", "grubbin").learnset.thunderfang = ["9D"];
		// Charjabug
		this.modData("Learnsets", "charjabug").learnset.thunderfang = ["9D"];
		this.modData("Learnsets", "charjabug").learnset.flash = ["9M"];
		// Vikavolt
		this.modData("Learnsets", "vikavolt").learnset.thunderfang = ["9D"];
		...
		this.modData("Learnsets", "necrozma").learnset.psychocut = ["9L19"];
		this.modData("Learnsets", "necrozma").learnset.storedpower = ["9L23"];
		this.modData("Learnsets", "necrozma").learnset.nightslash = ["9L37"];
		...
		this.modData("Learnsets", "cinderace").learnset.smackdown = ["9M"];
		// Sobble
		...
		// Drizzile
		this.modData("Learnsets", "drizzile").learnset.trumpcard = ["9D"];
		...
		// Inteleon
		this.modData("Learnsets", "inteleon").learnset.trumpcard = ["9D"];
		...
		this.modData("Learnsets", "inteleon").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "yamper").learnset.thunderfang = ["9E"];
		// Boltund
		...
		this.modData("Learnsets", "flapple").learnset.seedbomb = ["9L32","9M"];
		...
		this.modData("Learnsets", "appletun").learnset.energyball = ["9L32","9M"];
		...
		this.modData("Learnsets", "silicobra").learnset.wringout = ["9L49"];
		...
		this.modData("Learnsets", "sandaconda").learnset.sandstorm = ["9L40"];
		this.modData("Learnsets", "sandaconda").learnset.slam = ["9L47"];
		this.modData("Learnsets", "sandaconda").learnset.coil = ["9L54"];
		this.modData("Learnsets", "sandaconda").learnset.wringout = ["9L61"];
		...
		this.modData("Learnsets", "cramorant").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "sinistea").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "polteageist").learnset.spite = ["9M"];
		...
		// Cursola
		this.modData("Learnsets", "cursola").learnset.clearsmog = ["9D"];
		...
		this.modData("Learnsets", "runerigus").learnset.terrify = ["9L1"];
		...
		this.modData("Learnsets", "snom").learnset.bugbite = ["9M"];
		...
		this.modData("Learnsets", "frosmoth").learnset.bugbite = ["9M"];
		...
		this.modData("Learnsets", "stonjourner").learnset.gravity = ["9M"];
		...
		this.modData("Learnsets", "stonjourner").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "morpeko").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "urshifurapidstrike").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "zarude").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "wyrdeer").learnset.gravity = ["9M"];
		...
		this.modData("Learnsets", "wyrdeer").learnset.spite = ["9M"];
		// Kleavor
		...
		this.modData("Learnsets", "basculegionf").learnset.poltergeist = ["9M"];
		this.modData("Learnsets", "basculegionf").learnset.spite = ["9M"];
		// Basculegion
		...
		this.modData("Learnsets", "basculegion").learnset.poltergeist = ["9M"];
		this.modData("Learnsets", "basculegion").learnset.spite = ["9M"];
		// Sneasler
		...
		this.modData("Learnsets", "overqwil").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "meowscarada").learnset.knockoff = ["9L52", "9M"];
		...
		this.modData("Learnsets", "spidops").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "nymble").learnset.bugbite = ["9M"];
		...
		this.modData("Learnsets", "lokix").learnset.bugbite = ["9M"];
		this.modData("Learnsets", "lokix").learnset.compensation = ["9M"];
		...
		// Pawmi
		this.modData("Learnsets", "pawmi").learnset.zingzap = ["9D"];
		...
		this.modData("Learnsets", "pawmi").learnset.knockoff = ["9M"];
		...
		// Pawmo
		this.modData("Learnsets", "pawmo").learnset.zingzap = ["9D"];
		...
		this.modData("Learnsets", "pawmo").learnset.knockoff = ["9M"];
		...
		// Pawmot
		this.modData("Learnsets", "pawmot").learnset.zingzap = ["9D"];
		...
		this.modData("Learnsets", "pawmot").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "fidough").learnset.roar = ["9L30", "9M"];
		...
		this.modData("Learnsets", "dachsbun").learnset.roar = ["9L33", "9M"];
		...
		// Arboliva
		this.modData("Learnsets", "arboliva").learnset.luckychant = ["9D"];
		...
		this.modData("Learnsets", "squawkabilly").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "garganacl").learnset.gravity = ["9M"];
		...
		this.modData("Learnsets", "charcadet").learnset.spite = ["9M"];
		// Armarouge
		...
		this.modData("Learnsets", "armarouge").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "ceruledge").learnset.spite = ["9M"];
		// Tadbulb
		...
		this.modData("Learnsets", "maschiff").learnset.roar = ["9L18", "9M"];
		...
		this.modData("Learnsets", "mabosstiff").learnset.roar = ["9L18", "9M"];
		...
		this.modData("Learnsets", "shroodle").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "grafaiai").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "bramblin").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "brambleghast").learnset.spite = ["9M"];
		// Toedscool
		...
		this.modData("Learnsets", "toedscruel").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "klawf").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "klawf").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "scovillain").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "rellor").learnset.bugbite = ["9L20", "9M"];
		...
		this.modData("Learnsets", "rabsca").learnset.bugbite = ["9L20", "9M"];
		...
		this.modData("Learnsets", "rabsca").learnset.gravity = ["9M"];
		...
		this.modData("Learnsets", "tinkatink").learnset.knockoff = ["9L45", "9M"];
		...
		this.modData("Learnsets", "tinkatuff").learnset.knockoff = ["9L48", "9M"];
		...
		this.modData("Learnsets", "tinkaton").learnset.knockoff = ["9L51", "9M"];
		...
		this.modData("Learnsets", "bombirdier").learnset.knockoff = ["9L53", "9M"];
		...
		this.modData("Learnsets", "varoom").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "revavroom").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "revavroomsegin").learnset.explosion = ["9M"];
		this.modData("Learnsets", "revavroomsegin").learnset.fullcollide = ["9M"];
		this.modData("Learnsets", "revavroomsegin").learnset.gastroacid = ["9M"];
		this.modData("Learnsets", "revavroomsegin").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "revavroomschedar").learnset.explosion = ["9M"];
		this.modData("Learnsets", "revavroomschedar").learnset.fullcollide = ["9M"];
		this.modData("Learnsets", "revavroomschedar").learnset.gastroacid = ["9M"];
		this.modData("Learnsets", "revavroomschedar").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "revavroomnavi").learnset.explosion = ["9M"];
		this.modData("Learnsets", "revavroomnavi").learnset.fullcollide = ["9M"];
		this.modData("Learnsets", "revavroomnavi").learnset.gastroacid = ["9M"];
		this.modData("Learnsets", "revavroomnavi").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "revavroomruchbah").learnset.explosion = ["9M"];
		this.modData("Learnsets", "revavroomruchbah").learnset.fullcollide = ["9M"];
		this.modData("Learnsets", "revavroomruchbah").learnset.gastroacid = ["9M"];
		this.modData("Learnsets", "revavroomruchbah").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "revavroomcaph").learnset.explosion = ["9M"];
		this.modData("Learnsets", "revavroomcaph").learnset.fullcollide = ["9M"];
		this.modData("Learnsets", "revavroomcaph").learnset.gastroacid = ["9M"];
		this.modData("Learnsets", "revavroomcaph").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "cyclizar").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "glimmet").learnset.smackdown = ["9M"];
		// Glimmora
		...
		this.modData("Learnsets", "glimmora").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "greavard").learnset.roar = ["9M"];
		...
		this.modData("Learnsets", "houndstone").learnset.roar = ["9M"];
		...
		this.modData("Learnsets", "cetoddle").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "cetitan").learnset.knockoff = ["9M"];
		...
		this.modData("Learnsets", "annihilape").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "farigiraf").learnset.gravity = ["9M"];
		...
		this.modData("Learnsets", "dudunsparce").learnset.gyroball = ["9M"];
		...
		this.modData("Learnsets", "kingambit").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "screamtail").learnset.roar = ["9M"];
		...
		this.modData("Learnsets", "brutebonnet").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "fluttermane").learnset.spite = ["9L1", "9M"];
		...
		this.modData("Learnsets", "slitherwing").learnset.lunge = ["9L63"];
		...
		this.modData("Learnsets", "irontreads").learnset.metalsound = ["9L14"];
		...
		this.modData("Learnsets", "irontreads").learnset.gyroball = ["9M"];
		this.modData("Learnsets", "irontreads").learnset.knockoff = ["9L42", "9M"];
		...
		this.modData("Learnsets", "irontreads").learnset.smartstrike = ["9M"];
		...
		this.modData("Learnsets", "mechajugulis").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "mechajugulis").learnset.roar = ["9L7", "9M"];
		...
		// Astro Glider
		this.modData("Learnsets", "astroglider").learnset.lockon = ["9D"];
		...
		// Armor Thorns
		this.modData("Learnsets", "armorthorns").learnset.heavyslam = ["9D"];
		...
		this.modData("Learnsets", "armorthorns").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "gimmighoulroaming").learnset.payday = ["9L1"];
		this.modData("Learnsets", "gimmighoulroaming").learnset.embargo = ["9M"];
		this.modData("Learnsets", "gimmighoulroaming").learnset.flash = ["9M"];
		this.modData("Learnsets", "gimmighoulroaming").learnset.fullcollide = ["9M"];
		this.modData("Learnsets", "gimmighoulroaming").learnset.magiccoat = ["9M"];
		this.modData("Learnsets", "gimmighoulroaming").learnset.roleplay = ["9M"];
		// Gimmighoul
		...
		this.modData("Learnsets", "gholdengo").learnset.poltergeist = ["9M"];
		...
		this.modData("Learnsets", "wochien").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "wochien").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "chienpao").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "chienpao").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "tinglu").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "tinglu").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "chiyu").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "chiyu").learnset.spite = ["9M"];
		...
		this.modData("Learnsets", "roaringmoon").learnset.crunch = ["9L1"];
		this.modData("Learnsets", "roaringmoon").learnset.twister = ["9L1"];
		...
		this.modData("Learnsets", "roaringmoon").learnset.compensation = ["9M"];
		...
		this.modData("Learnsets", "roaringmoon").learnset.knockoff = ["9M"];
		this.modData("Learnsets", "roaringmoon").learnset.payback = ["9M"];
		this.modData("Learnsets", "roaringmoon").learnset.roar = ["9M"];
		...
		// Valiant Droid
		this.modData("Learnsets", "valiantdroid").learnset.secretsword = ["9D"];
		...
		this.modData("Learnsets", "koraidon").learnset.roar = ["9M"];
		this.modData("Learnsets", "koraidon").learnset.screech = ["9L63", "9M"];
		this.modData("Learnsets", "koraidon").learnset.smackdown = ["9M"];
		...
		this.modData("Learnsets", "walkingwake").learnset.scald = ["9L56", "9M"];
		this.modData("Learnsets", "walkingwake").learnset.breakingswipe = ["9L35", "9M"];
		...
		this.modData("Learnsets", "walkingwake").learnset.knockoff = ["9M"];
		this.modData("Learnsets", "walkingwake").learnset.roar = ["9L1", "9M"];
		...
		this.modData("Learnsets", "saberleaves").learnset.psychocut = ["9L56"];
		...
		// Syclant
		this.modData("Learnsets", "syclant").learnset.firstimpression = ["9D"];
		...
		this.modData("Learnsets", "syclant").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "embirch").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "flarelm").learnset.trailhead = ["9M"];
		...
		this.modData("Learnsets", "pyroak").learnset.trailhead = ["9M"];
		// Breezi
		...
		this.modData("Learnsets", "privatyke").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "arghonaut").learnset.lashout = ["9L1"];
		this.modData("Learnsets", "arghonaut").learnset.whitewater = ["9L4"];
		this.modData("Learnsets", "arghonaut").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "monohm").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "duohm").learnset.chillywater = ["9M"];
		...
		// Cyclohm
		this.modData("Learnsets", "cyclohm").learnset.mindreader = ["9D"];
		this.modData("Learnsets", "cyclohm").learnset.cyclohm = ["9M"];
		...
		this.modData("Learnsets", "protowatt").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "krilowatt").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "scratchet").learnset.trailhead = ["9M"];
		// Tomohawk
		...
		this.modData("Learnsets", "tomohawk").learnset.trailhead = ["9M"];
		// Necturine
		...
		this.modData("Learnsets", "snugglow").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "plasmanta").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "floatoy").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "caimanoe").learnset.chillywater = ["9M"];
		delete this.modData('Learnsets', 'caimanoe').learnset.blizzard;
		// Naviathan
		this.modData("Learnsets", "naviathan").learnset.nobleroar = ["9D"];
		...
		this.modData("Learnsets", "naviathan").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "smogecko").learnset.preheat = ["9L1"];
		this.modData("Learnsets", "smogecko").learnset.tussle = ["9L1"];
		this.modData("Learnsets", "smogecko").learnset.toxic = ["9M"];
		// Smoguana
		this.modData("Learnsets", "smoguana").learnset.strangesmoke = ["9D"];
		this.modData("Learnsets", "smoguana").learnset.preheat = ["9L1"];
		this.modData("Learnsets", "smoguana").learnset.tussle = ["9L1"];
		this.modData("Learnsets", "smoguana").learnset.toxic = ["9M"];
		// Smokomodo
		this.modData("Learnsets", "smokomodo").learnset.strangesmoke = ["9D"];
		this.modData("Learnsets", "smokomodo").learnset.preheat = ["9L1"];
		this.modData("Learnsets", "smokomodo").learnset.tussle = ["9L1"];
		this.modData("Learnsets", "smokomodo").learnset.toxic = ["9M"];
		// Swirlpool
		this.modData("Learnsets", "swirlpool").learnset.lifedew = ["9D"];
		this.modData("Learnsets", "swirlpool").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "coribalis").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "snaelstrom").learnset.chillywater = ["9M"];
		...
		this.modData("Learnsets", "saharascal").learnset.earthquake = ["9L44", "9M"];
