...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	// For Hide and Seaking Slate 5, here temporarily
	// If you cheat off of this you are a coward and will lose your next 3 tournament sets due to hax 
	// and you smell
   characterclause: {
        effectType: 'ValidatorRule',
        name: 'Character Clause',
        // desc: "Any Pokémon, move, item, nickname, or ability is illegal if its name contains a character not in the alphabet (this includes spaces).",
		onBegin() {
			this.add('rule', 'Character Clause: What does this do?');
		},
        onValidateTeam(team) {
            const illegalReasons = [];
            const letters = /[A-Za-z]/g;
            for (const set of team) {
                if (set.name.replace(letters,'')) {
                    illegalReasons.push(set.name + '\'s nickname has characters outside the alphabet.');
                }
    ...
    soupclause: {
		effectType: 'ValidatorRule',
		name: 'Soup Clause',
		// desc: "No two Pokémon on the same team can be of species starting with the same letter.",
		onBegin() {
			this.add('rule', 'Soup Clause: What does this do?');
		},
		onValidateTeam(team) {
            const letters = {};
            for (const set of team) {
                const firstLetter = set.species.toLowerCase().charAt(0);
                (letters[firstLetter] ||= []).push(set.species);
            }
            if (team.length <= letters.length) {
                return;
            }
            const reasons = [];
    ...
    disadvantageclause: {
		effectType: 'ValidatorRule',
		name: 'Disadvantage Clause',
		//desc: "If a Pokemon's primary type is weak to or resisted by an opponent's primary type, it will become trapped.",
		onBegin() {
			this.add('rule', 'Disadvantage Clause: What does this do?');
		},
		//does not apply to ghosts
		onFoeTrapPokemon(pokemon) {
			const source = this.effectState.target;
			if (!pokemon.isAdjacent(source) || !pokemon.types.length || !source.types.length) return;
    ...
    dayoneclause: {
		effectType: 'ValidatorRule',
		name: 'Day One Clause',
		//desc: "The first letter in the day of the week must match with the first letter of a species on the team.",
		onBegin() {
			this.add('rule', 'Day One Clause: What does this do?');
		},
		onValidateTeam(team) {
            const letter = ['s','m','t','w','t','f','s'][(new Date()).getDay()];
            for (const set of team) {
    ...
    pivotclause: {
        effectType: 'Rule',
        name: 'Pivot Clause',
        //desc: "If a Pokemon uses five pivoting moves against the same opponent, it faints.",
        onBegin() {
            for (const pokemon of this.getAllPokemon()) {
                pokemon.m.pivots = {};
            }
            this.add('rule', 'Pivot Clause: What does this do?');
        },
        onPrepareHit(source, target, move) {
            if (!move.selfSwitch) return;
            for (const foe of source.foes()) {
                if (!source.m.pivots[foe]) {
					source.m.pivots[foe] = 1;
                  } else if (source.m.pivots[foe] >= 4) {
    ...
    sweeperclause: {
		effectType: 'Rule',
		name: 'Sweeper Clause',
		//desc: "Upon landing consectutive KOs without switching out, this Pokemon's positive stat boosts are reset.",
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				pokemon.m.kos = 0;
			}
			this.add('rule', 'Sweeper Clause: What does this do?');
		},
    ...
    levelclause: {
		effectType: 'Rule',
		name: 'Level Clause',
		//desc: "Pokemon gain +5 to their level for each tier below OU.",
		onBegin() {
            const boosts: {[tier: string]: number} = {
                uu: 5,
                rubl: 5,
			...
			for (const pokemon of this.getAllPokemon()) {
				...
				if (boosts[tier]) {
				  pokemon.level += boosts[tier];
				  pokemon.set.level += boosts[tier];
				  pokemon.maxhp = pokemon.hp = pokemon.baseMaxhp = Math.floor(Math.floor(
					2 * pokemon.species.baseStats['hp'] + pokemon.set.ivs['hp'] + Math.floor(pokemon.set.evs['hp'] / 4) + 100
					) * pokemon.level / 100 + 10);
				}
			}
			this.add('rule', 'Level Clause: What does this do?');
		},
		onSwitchIn(pokemon) {
			const tier = pokemon.species.tier;
			if (tier !== 'OU') {
	...
	antihomophobiaclause: {
		effectType: 'Rule',
		name: 'Anti-Homophobia Clause',
		//desc: "Forces Ferrothorn to use Explosion when it enters the field.",
		onBegin() {
			this.add('rule', 'Anti-Homophobia Clause: What does this do?');
		},
		onSwitchIn(pokemon) {
	...
	bigrecoilclause: {
		effectType: 'Rule',
		name: 'Big Recoil Clause',
		//desc: "Moves that KO the user or make them lose half their HP always fail.",
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				pokemon.addVolatile('bigrecoil');
			}
			this.add('rule', 'Big Recoil Clause: What does this do?');
		},
	},
	chinaclause: {
		effectType: 'Rule',
		name: 'China Clause',
		//desc: "All Pokemon must have at least 4 EVs in every stat.",
		onBegin() {
			this.add('rule', 'China Clause: What does this do?');
		},
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			if (set.evs) {
	...
	clawsclause: {
		effectType: 'Rule',
		name: 'Claws Clause',
		//desc: "All non-fixed damaging moves hit twice, with the second hit dealing 0.25x damage.",
		onBegin() {
			this.add('rule', 'Claws Clause: What does this do?');
		},
		onPrepareHit(source, target, move) {
			if ([
	...
	compatibilityclause: {
		effectType: 'Rule',
		name: 'Compatibility Clause',
		//desc: "All of a Pokemon's moves are legally compatible with all of their other moves.",
		onBegin() {
			this.add('rule', 'Compatibility Clause: What does this do?');
		},
		// Coded by making every egg, virtual console, and event move in learnsets.ts a level-up move instead.
	},
	focusclause: {
		effectType: 'Rule',
		name: 'Focus Clause',
		//desc: "Any move with 80% accuracy or less always hits.",
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				pokemon.addVolatile('focusclause');
			}
			this.add('rule', 'Focus Clause: What does this do?');
		},
        onModifyMove(move, pokemon, target) {
	...
	handicapclause: { // ask kirby
		effectType: 'Rule',
		name: 'Handicap Clause',
		//desc: "For every unused team slot, this Pokemon gains +1 to all stats when it's the last Pokemon left on its side.",
		onBegin() {
			this.add('rule', 'Handicap Clause: What does this do?');
		},
        onSwitchIn(pokemon) {
            const boostVal = 6 - pokemon.side.pokemon.length;
            const benched = '';
	...
	imprisonclause: {
		effectType: 'Rule',
		name: 'Imprison Clause',
		//desc: "No Pokemon can use moves that both it and its opponent know.",
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				pokemon.addVolatile('imprison');
			}
			this.add('rule', 'Imprison Clause: What does this do?');
		},
	},
	jumpfartclause: {
		effectType: 'Rule',
		name: 'Jumpfart Clause',
		//desc: "Luvdisc and all Pokemon with less than or equal to 280 BST (except Smeargle) have all of their stats doubled.",
		onBegin() {
			this.add('rule', 'Jumpfart Clause: What does this do?');
		},
		onModifySpeciesPriority: 2,
		onModifySpecies(species) {
			const newSpecies = this.dex.deepClone(species);
			if (newSpecies.id === 'smeargle') return;
			if (newSpecies.bst <= 280 || newSpecies.id === 'luvdisc') {
				newSpecies.bst = 0;
				for (const stat in newSpecies.baseStats) {
					newSpecies.baseStats[stat] = this.clampIntRange(newSpecies.baseStats[stat] * 2, 1, 255);
					newSpecies.bst += newSpecies.baseStats[stat];
	...
	kunaiclause: {
		effectType: 'Rule',
		name: 'Kunai Clause',
		//desc: "Any Pokemon that attacks 3 times consectutively has its highest Defense raised by 1 stage.",
		onBegin() {
			this.add('rule', 'Kunai Clause: What does this do?');
		},
	...
	movedegradationclause: { // ask kirby about the healing move degradation
		effectType: 'Rule',
		name: 'Move Degradation Clause',
		//desc: "Consectutive damaging and healing moves get progressively weaker.",
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				pokemon.addVolatile('movedegradation');
				pokemon.m.consecutiveHealing = 0;
			}
			this.add('rule', 'Move Degradation Clause: What does this do?');
			// move weakening covered in conditions.ts
		},
        onBeforeMove(pokemon, target, move) {
            if (move.category !== 'Status' || !move.flags['heal'] || target !== pokemon) pokemon.m.consecutiveHealing = 0;
        },
        onTryHeal(damage, target, source, effect) {
            ...
            healingPenaltyMultiplier *= 2;
            }
            return this.chainModify([1,healingPenaltyMultiplier]);
        },
	},
	nonvolatilestatusclause: { // check if stuff that treats confusion as a volatile still works
		effectType: 'Rule',
		name: 'Non-Volatile Status Clause',
		//desc: "Curse and Confusion are now non-volatile statuses.",
		onBegin() {
			this.add('rule', 'Non-Volatile Status Clause: What does this do?');
		},
		// Coded in conditions.ts and moves.ts.
	},
	recoverymovesclause: {
		effectType: 'Rule',
		name: 'Recovery Moves Clause',
		//desc: "Teams can only have 1 move that heals 50% of the user's max HP in the same turn.",
		onBegin() {
			this.add('rule', 'Recovery Moves Clause: What does this do?');
		},
		onValidateTeam(team) {
			const healingMoves = [
				'healorder', 'milkdrink', 'recover', 'roost', 'slackoff', 'softboiled', 'shoreup', 'moonlight', 'morningsun', 'synthesis', 'purify',
			];
			let healers = 0;
			for (const set of team) {
				for (const move of set.moves) {
					if (healingMoves.includes(this.toID(move))) healers++;
				}
				if (healers > 1) {
					return [
	...
	sigmamaleclause: { // ask kirby
		effectType: 'Rule',
		name: 'Sigma Male Clause',
		//desc: "Male Pokemon can only have neutral natures.",
		onBegin() {
			this.add('rule', 'Sigma Male Clause: What does this do?');
		},
        onChangeSet(set) {
    ...
    squawkclause: {
        effectType: 'Rule',
        name: 'Squawk Clause',
        //desc: "All Teams must have at least 1 Flying-type.",
        onBegin() {
            this.add('rule', 'Squawk Clause: What does this do?');
        },
        onValidateTeam(team) {
            for (const set of team) {
	...
	thebeafcultistclause: { // ask kirby
		effectType: 'Rule',
		name: 'The Beaf Cultist Clause',
		//desc: "Activating Magnet Pull results in an instant forfeit.",
		onBegin() {
			this.add('rule', 'The Beaf Cultist Clause: What does this do?');
		},
        // implemented in abilities.ts
	},
    twobirdonestone: {
        effectType: 'Rule',
        name: 'Two Bird One Stone',
        //desc: "All Teams with exactly two Flying-types must also have exactly one Rock-type.",
        onBegin() {
            this.add('rule', 'Two Bird One Stone: What does this do?');
        },
        onValidateTeam(team) {
            let birds = 0;
            let stones = 0;
            for (const set of team) {
                let speciesTypes = this.dex.species.get(set.species).types;
                if (speciesTypes.includes('Flying')) birds++;
                if (speciesTypes.includes('Rock')) stones++;
            }
            if ((birds === 2) && (stones !== 1)) {
                return [
    ...
    bigdogclause: {
        effectType: 'Rule',
        name: 'big dog Clause',
        //desc: "Certain canine Pokemon are allowed to Dynamax.",
        onBegin() {
            this.add('rule', 'big dog Clause: i got that dawg in me');
            const formeagnosticdogs = ['vulpix','ninetales','growlithe','arcanine','zorua','zoroark','rockruff','lycanroc'];
            const dogs = ['eevee','vaporeon','jolteon','flareon','espeon','umbreon','leafeon','glaceon','sylveon','snubbull','granbull','houndour','houndoom','smeargle','poochyena','mightyena','electrike','manectric','absol','riolu','lucario','lillipup','herdier','stoutland','fennekin','braixen','delphox','furfrou','zygarde10','nickit','thievul','yamper','boltund','fidough','dachsbun','maschiff','mabosstiff','greavard','houndstone','okidogi'];
            for (const pokemon of this.getAllPokemon()) {
              const speciesid = pokemon.species.id;
              if (dogs.includes(speciesid) || formeagnosticdogs.some(prefix => speciesid.startsWith(prefix)))
                {
                    pokemon.m.dog = true;
                }
            }
    ...
    upsidedownpower: {
        ...
        //desc: "All moves work off the opposite attacking stat.",
        onBegin() {
            this.add('rule', 'Upside-Down Power: What does this do?');
        },
        onModifyMove(move, pokemon, target) {
            if (move.category === 'Physical') {
	...
	monikerclause: {
		effectType: 'Rule',
		name: 'Moniker Clause',
		//desc: "Gives Pokemon whose names in English and Japanese match a 1.5x boost to their highest stat.",
		onBegin() {
			this.add('rule', 'Moniker Clause: What does this do?');
		},
