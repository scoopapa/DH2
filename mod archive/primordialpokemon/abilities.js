...
exports.BattleAbilities = {
	...
	"spacemanipulation": {
		...
		shortDesc: "On switch-in, this Pokemon lowers the Special Defense of adjacent opponents by 1 stage.",
		onStart: function (pokemon) {
			...
			for (var i = 0; i < foeactive.length; i++) {
				if (!foeactive[i] || !this.isAdjacent(foeactive[i], pokemon)) continue;
				if (!activated) {
					this.add('-ability', pokemon, 'Space Manipulation');
					activated = true;
				}
				if (foeactive[i].volatiles['substitute']) {
					this.add('-activate', foeactive[i], 'Substitute', 'ability: Space Manipulation', '[of] ' + pokemon);
				} else {
					this.boost({spd: -1}, foeactive[i], pokemon);
				}
			}
		},
		id: "spacemanipulation",
	...
	"timemanipulation": {
		...
		onStart: function (pokemon) {
			...
			for (var i = 0; i < foeactive.length; i++) {
				if (!foeactive[i] || !this.isAdjacent(foeactive[i], pokemon)) continue;
				if (!activated) {
					this.add('-ability', pokemon, 'Time Manipulation');
					activated = true;
				}
				if (foeactive[i].volatiles['substitute']) {
					this.add('-activate', foeactive[i], 'Substitute', 'ability: Time Manipulation', '[of] ' + pokemon);
				} else {
