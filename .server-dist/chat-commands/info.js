...
var _net2 = require('../../.lib-dist/net');
 ...
 const commands = {
	...
	data(target, room, user, connection, cmd) {
		...
		} else if (_optionalChain([room, 'optionalAccess', _27 => _27.battle])) {
			...
			case 'move':
				...
				if (showDetails) {
					...
					if (dex.gen >= 7) {
						...
						} else if (_optionalChain([move, 'access', _32 => _32.zMove, 'optionalAccess', _33 => _33.basePower])) {
							details["Z-Power"] = String(move.zMove.basePower);
						} else if (_optionalChain([move, 'access', _34 => _34.zMove, 'optionalAccess', _35 => _35.effect])) {
							const zEffects = {
						...
						} else if (_optionalChain([move, 'access', _36 => _36.zMove, 'optionalAccess', _37 => _37.boost])) {
							details["Z-Effect"] = "";
		...
		if (Dex.getImmunity(source, defender) ||
			...
			if (source.effectType !== 'Move' || source.category !== 'Status' && (source.basePower || source.basePowerCallback)) {
				for (const type of defender.types) {
					...
					const moveMod = _optionalChain([source, 'access', _41 => _41.onEffectiveness, 'optionalAccess', _42 => _42.call, 'call', _43 => _43({dex: Dex} , baseMod, null, type, source)]);
	...
	coverage(target, room, user) {
		...
		const sources = [];
		let dex = Dex;
		if (_optionalChain([room, 'optionalAccess', _44 => _44.battle])) {
		...
		for (let arg of targets) {
			...
			for (const type in bestCoverage) {
				if (move.id === "struggle") {
					...
					const moveMod = _optionalChain([move, 'access', _45 => _45.onEffectiveness, 'optionalAccess', _46 => _46.call, 'call', _47 => _47({dex} , baseMod, null, type, move )]);
					...
					if (type1 === type2) {
						...
						for (const move of sources) {
							...
							if (typeof move === 'string') {
								...
								let moveMod = _optionalChain([move, 'access', _48 => _48.onEffectiveness, 'optionalAccess', _49 => _49.call, 'call', _50 => _50({dex} , baseMod, null, type1, move )]);
								curEff += typeof moveMod === 'number' ? moveMod : baseMod;
								baseMod = dex.getEffectiveness(move.type, type2);
								moveMod = _optionalChain([move, 'access', _51 => _51.onEffectiveness, 'optionalAccess', _52 => _52.call, 'call', _53 => _53({dex} , baseMod, null, type2, move )]);
								curEff += typeof moveMod === 'number' ? moveMod : baseMod;
							}
	...
	bugs(target, room, user) {
		if (!this.runBroadcast()) return;
		if (_optionalChain([room, 'optionalAccess', _54 => _54.battle])) {
	...
	calc(target, room, user, connection, cmd) {
		...
		const isRandomBattle = (_optionalChain([room, 'optionalAccess', _55 => _55.battle]) && SUPPORTED_RANDOM_FORMATS.includes(room.battle.format));
		const isBattleSpotBattle = (_optionalChain([room, 'optionalAccess', _56 => _56.battle]) && (SUPPORTED_BATTLESPOT_FORMATS.includes(room.battle.format) ||
	...
	formathelp(target, room, user, connection, cmd) {
		...
		if (totalMatches === 1) {
			...
			if (['Format', 'Rule', 'ValidatorRule'].includes(subformat.effectType)) {
				if (_optionalChain([subformat, 'access', _57 => _57.ruleset, 'optionalAccess', _58 => _58.length])) {
					rules.push(`<b>Ruleset</b> - ${_utils.Utils.escapeHTML(subformat.ruleset.join(", "))}`);
				}
				if (_optionalChain([subformat, 'access', _59 => _59.banlist, 'optionalAccess', _60 => _60.length])) {
					rules.push(`<b>Bans</b> - ${_utils.Utils.escapeHTML(subformat.banlist.join(", "))}`);
				}
				if (_optionalChain([subformat, 'access', _61 => _61.unbanlist, 'optionalAccess', _62 => _62.length])) {
					rules.push(`<b>Unbans</b> - ${_utils.Utils.escapeHTML(subformat.unbanlist.join(", "))}`);
				}
				if (_optionalChain([subformat, 'access', _63 => _63.restricted, 'optionalAccess', _64 => _64.length])) {
	...
	rules(target, room, user, connection, cmd) {
		if (!target) {
			...
			this.sendReplyBox(
				...
				`${_optionalChain([room, 'optionalAccess', _65 => _65.settings, 'access', _66 => _66.rulesLink]) ? _utils.Utils.html`- <a href="${room.settings.rulesLink}">${this.tr `${room.title} room rules`}</a><br />` : ``}` +
	...
	smogdex(target, room, user) {
		...
		if (pokemon.exists) {
			...
			if (['ou', 'uu'].includes(formatId) && generation === 'sm' &&
				_optionalChain([room, 'optionalAccess', _67 => _67.settings, 'access', _68 => _68.language]) && room.settings.language in supportedLanguages) {
	...
	async requestshow(target, room, user) {
		...
		if (_optionalChain([room, 'access', _69 => _69.pendingApprovals, 'optionalAccess', _70 => _70.has, 'call', _71 => _71(user.id)])) return this.errorReply('You have a request pending already.');
	...
	async approveshow(target, room, user) {
		...
		const request = _optionalChain([room, 'access', _72 => _72.pendingApprovals, 'optionalAccess', _73 => _73.get, 'call', _74 => _74(userid)]);
	...
	denyshow(target, room, user) {
		...
		const entry = _optionalChain([room, 'access', _75 => _75.pendingApprovals, 'optionalAccess', _76 => _76.get, 'call', _77 => _77(target)]);
