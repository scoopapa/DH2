...
var _scavengergamesjs = require('./scavenger-games.js');
...
const databaseContentsJSON = _fs.FS.call(void 0, DATABASE_FILE).readIfExistsSync();
...
function getScavsRoom(room) {
	...
	if (_optionalChain([room, 'access', _ => _.parent, 'optionalAccess', _2 => _2.roomid]) === SCAVENGER_ROOMID) return room.parent;
	return null;
}
...
class Ladder {
	...
	load() {
		const json = _fs.FS.call(void 0, this.file).readIfExistsSync();
	...
	write() {
		_fs.FS.call(void 0, this.file).writeUpdate(() => JSON.stringify(this.data));
	}
...
const Leaderboard = _optionalChain([LeaderboardRoom, 'optionalAccess', _3 => _3.scavLeaderboard, 'optionalAccess', _4 => _4.scavsLeaderboard]) || new Ladder(DATA_FILE);
const HostLeaderboard = _optionalChain([LeaderboardRoom, 'optionalAccess', _5 => _5.scavLeaderboard, 'optionalAccess', _6 => _6.scavsHostLeaderboard]) || new PlayerLadder(HOST_DATA_FILE);
const PlayerLeaderboard = _optionalChain([LeaderboardRoom, 'optionalAccess', _7 => _7.scavLeaderboard, 'optionalAccess', _8 => _8.scavsPlayerLeaderboard]) ||
...
function formatQueue(queue, viewer, room, broadcasting) {
	...
	const queueDisabled = _optionalChain([room, 'access', _9 => _9.settings, 'access', _10 => _10.scavSettings, 'optionalAccess', _11 => _11.scavQueueDisabled]);
	const timerDuration = _optionalChain([room, 'access', _12 => _12.settings, 'access', _13 => _13.scavSettings, 'optionalAccess', _14 => _14.defaultScavTimer]) || DEFAULT_TIMER_DURATION;
	let buffer;
	if (_optionalChain([queue, 'optionalAccess', _15 => _15.length])) {
		buffer = queue.map((item, index) => {
			...
			const unratedText = item.gameType === 'unrated' ?
				...
				'';
			const hosts = _utils.Utils.escapeHTML(Chat.toListString(item.hosts.map(h => h.name)));
			...
			if (!broadcasting && (item.hosts.some(h => h.id === viewer.id) || viewer.id === item.staffHostId)) {
				questions = item.questions.map(
					(q, i) => {
						if (i % 2) {
							q = q ;
							return _utils.Utils.html`<span style="color: green"><em>[${q.join(' / ')}]</em></span><br />`;
						} else {
							q = q ;
							return _utils.Utils.escapeHTML(q);
						}
					}
				).join(" ");
			} else {
...
class ScavengerHuntDatabase {
	...
	static updateDatabaseOnDisk() {
		_fs.FS.call(void 0, DATABASE_FILE).writeUpdate(() => JSON.stringify(scavengersData));
	}
 ...
 class ScavengerHunt extends Rooms.RoomGame {
	...
	constructor(
		...
		super(room);ScavengerHunt.prototype.__init.call(this);;
		...
		if (mod) {
			this.loadMods(mod);
		} else if (this.gameType === 'official' && _optionalChain([this, 'access', _16 => _16.room, 'access', _17 => _17.settings, 'access', _18 => _18.scavSettings, 'optionalAccess', _19 => _19.officialtwist])) {
			this.loadMod(_optionalChain([this, 'access', _20 => _20.room, 'access', _21 => _21.settings, 'access', _22 => _22.scavSettings, 'optionalAccess', _23 => _23.officialtwist]));
		}
	...
	loadMod(modData) {
		...
		if (typeof modData === 'string') {
			...
			if (!_scavengergamesjs.ScavMods.twists[modId]) return this.announce(`Invalid mod. Starting the hunt without the mod ${modId}.`);

			twist = _scavengergamesjs.ScavMods.twists[modId];
		} else {
			twist = modData;
		}
	...
	getCreationMessage(newHunt) {
		...
		const hosts = _utils.Utils.escapeHTML(Chat.toListString(this.hosts.map(h => h.name)));
		const staffHost = this.hosts.some(h => h.id === this.staffHostId) ?
			`` :
			_utils.Utils.html` by <em>${this.staffHostName}</em>`;
	...
	forceWrap(answer) {
		return _utils.Utils.escapeHTML(answer.replace(/[^\s]{30,}/g, word => {
			let lastBreak = 0;
			let brokenWord = '';
	...
	onComplete(player) {
		...
		const blitz = now - this.startTime <= 60000 && canBlitz &&
			(_optionalChain([this, 'access', _24 => _24.room, 'access', _25 => _25.settings, 'access', _26 => _26.scavSettings, 'optionalAccess', _27 => _27.blitzPoints, 'optionalAccess', _28 => _28[this.gameType]]) || DEFAULT_BLITZ_POINTS[this.gameType]);
		...
		this.announce(_utils.Utils.html`<em>${result.name}</em> has finished the hunt in ${place} place! (${time}${(blitz ? " - BLITZ" : "")})`);
	...
	onEnd(reset, endedBy) {
		...
		if (!ScavengerHuntDatabase.isEmpty() && _optionalChain([this, 'access', _29 => _29.room, 'access', _30 => _30.settings, 'access', _31 => _31.scavSettings, 'optionalAccess', _32 => _32.addRecycledHuntsToQueueAutomatically])) {
		...
		if (!reset) {
			...
			this.announce(
				`The ${this.gameType ? `${this.gameType} ` : ""}scavenger hunt was ended ${(endedBy ? "by " + _utils.Utils.escapeHTML(endedBy.name) : "automatically")}.<br />` +
				`${this.completed.slice(0, sliceIndex).map((p, i) => `${formatOrder(i + 1)} place: <em>${_utils.Utils.escapeHTML(p.name)}</em> <span style="color: lightgreen;">[${p.time}]</span>.<br />`).join("")}${this.completed.length > sliceIndex ? `Consolation Prize: ${this.completed.slice(sliceIndex).map(e => `<em>${_utils.Utils.escapeHTML(e.name)}</em> <span style="color: lightgreen;">[${e.time}]</span>`).join(', ')}<br />` : ''}<br />` +
				`<details style="cursor: pointer;"><summary>Solution: </summary><br />${this.questions.map((q, i) => `${i + 1}) ${Chat.formatText(q.hint)} <span style="color: lightgreen">[<em>${_utils.Utils.escapeHTML(q.answer.join(' / '))}</em>]</span>`).join("<br />")}</details>`
			);

			// give points for winning and blitzes in official games
			if (!this.runEvent('GivePoints')) {
				const winPoints = _optionalChain([this, 'access', _33 => _33.room, 'access', _34 => _34.settings, 'access', _35 => _35.scavSettings, 'optionalAccess', _36 => _36.winPoints, 'optionalAccess', _37 => _37[this.gameType]]) ||
					DEFAULT_POINTS[this.gameType];
				const blitzPoints = _optionalChain([this, 'access', _38 => _38.room, 'access', _39 => _39.settings, 'access', _40 => _40.scavSettings, 'optionalAccess', _41 => _41.blitzPoints, 'optionalAccess', _42 => _42[this.gameType]]) ||
					DEFAULT_BLITZ_POINTS[this.gameType];
				// only regular hunts give host points
				let hostPoints;
				if (this.gameType === 'regular') {
					hostPoints = _optionalChain([this, 'access', _43 => _43.room, 'access', _44 => _44.settings, 'access', _45 => _45.scavSettings, 'optionalAccess', _46 => _46.hostPoints]) ?
						_optionalChain([this, 'access', _47 => _47.room, 'access', _48 => _48.settings, 'access', _49 => _49.scavSettings, 'optionalAccess', _50 => _50.hostPoints]) :
						DEFAULT_HOST_POINTS;
				}
	...
	tryRunQueue(roomid) {
		if (this.room.scavgame || _optionalChain([this, 'access', _51 => _51.room, 'access', _52 => _52.settings, 'access', _53 => _53.scavSettings, 'optionalAccess', _54 => _54.scavQueueDisabled])) {
		...
		if (this.room.settings.scavQueue && this.room.settings.scavQueue.length) {
			setTimeout(() => {
				...
				if (!room || room.game || !_optionalChain([room, 'access', _55 => _55.settings, 'access', _56 => _56.scavQueue, 'optionalAccess', _57 => _57.length]) || _optionalChain([room, 'access', _58 => _58.settings, 'access', _59 => _59.scavSettings, 'optionalAccess', _60 => _60.scavQueueDisabled])) return;

				const next = room.settings.scavQueue.shift();
				const duration = _optionalChain([room, 'access', _61 => _61.settings, 'access', _62 => _62.scavSettings, 'optionalAccess', _63 => _63.defaultScavTimer]) || DEFAULT_TIMER_DURATION;
	...
	validatePlayer(player) {
		...
		if (uniqueConnections > 1 && _optionalChain([this, 'access', _64 => _64.room, 'access', _65 => _65.settings, 'access', _66 => _66.scavSettings, 'optionalAccess', _67 => _67.scavmod, 'optionalAccess', _68 => _68.ipcheck])) {
	...
	onChatMessage(msg) {
		...
		const filtered = this.questions.some(q => q.answer.some(a => {
			...
			if (_utils.Utils.levenshtein(msgId, a, md) <= md) return true;
			return false;
		}));
...
const ScavengerCommands = {
	...
	games: {
		...
		start(target, room, user) {
			...
			target = toID(target);
			const game = _scavengergamesjs.ScavMods.LoadGame(room, target);
		...
		async rank(target, room, user) {
			...
			if (!rank) {
				...
				this.sendReplyBox(_utils.Utils.html`User '${rank.name}' is #${rank.rank} on the scavenger games leaderboard with ${rank.points} points.`);
			}
		},
	},
	teamscavs: {
		...
		createteam(target, room, user) {
			...
			game.announce(_utils.Utils.html`A new team "${teamName}" has been created with ${leaderUser.name} as the leader.`);
		},
		...
		removeteam(target, room, user) {
			...
			game.announce(_utils.Utils.html`The team "${target}" has been removed.`);
		},
		...
		addplayer(target, room, user) {
			...
			const targetUsers = target.split(',').map(id => Users.getExact(id)).filter(u => _optionalChain([u, 'optionalAccess', _69 => _69.connected])) ;
			...
			game.announce(_utils.Utils.html`${Chat.toListString(targetUsers.map(u => u.name))} ${targetUsers.length > 1 ? 'have' : 'has'} been added into ${userTeam.name}.`);
		},
		...
		teams(target, room, user) {
			...
			for (const teamID in game.teams) {
				const team = game.teams[teamID];
				display.push(_utils.Utils.html`<strong>${team.name}</strong> - <strong>${team.players[0]}</strong>${team.players.length > 1 ? ', ' + team.players.slice(1).join(', ') : ''}`);
			}
		...
		guesses(target, room, user) {
			...
			this.sendReplyBox(_utils.Utils.html`<strong>Question #${team.question} guesses:</strong> ${team.answers.sort().join(', ')}`);
		},
		...
		note(target, room, user) {
			...
			game.teamAnnounce(user, _utils.Utils.html`<strong> Note from ${user.name}:</strong> ${target}`);
		},
	},
	teamscavshelp: [
	...
	status(target, room, user) {
		...
		const hostMsg = game.hosts.some(h => h.id === game.staffHostId) ?
			'' : _utils.Utils.html` (started by - ${game.staffHostName})`;
		const finishers = _utils.Utils.html`${game.completed.map(u => u.name).join(', ')}`;
				...
				if (!players.length) {
					...
					str += players.map(
						pl => pl.lastGuess > Date.now() - 1000 * 300 ?
							_utils.Utils.html`<strong>${pl.name}</strong>` :
							_utils.Utils.escapeHTML(pl.name)
					).join(", ");
				}
			}
			const completed = game.preCompleted ? game.preCompleted : game.completed;
			str += _utils.Utils.html`<tr><td>Completed</td><td>${completed.length ? completed.map(pl => pl.name).join(", ") : 'None'}`;
	...
	async ladder(target, room, user) {
		...
		this.sendReply(
			`|uhtml${isChange ? 'change' : ''}|scavladder|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Points</th></tr>${ladder.map(entry => {
				...
				return `<tr><td>${entry.rank}</td><td>${(isStaff ? `<em>${_utils.Utils.escapeHTML(entry.name)}</em>` : (entry.rank <= 5 ? `<strong>${_utils.Utils.escapeHTML(entry.name)}</strong>` : _utils.Utils.escapeHTML(entry.name)))}</td><td>${entry.points}</td></tr>`;
			}).join('')}</table></div>` +
	...
	async rank(target, room, user) {
		...
		if (!rank) {
			...
			this.sendReplyBox(_utils.Utils.html`User '${rank.name}' is #${rank.rank} on the scavengers leaderboard with ${rank.points} points.`);
		}
	},
	...
	settwist(target, room, user) {
		...
		if (target === 'RESET') {
			...
			if (!_scavengergamesjs.ScavMods.twists[twist] || twist === 'constructor') return this.errorReply('Invalid twist.');
	...
	twists(target, room, user) {
		...
		buffer += Object.keys(_scavengergamesjs.ScavMods.twists).map(twistid => {
			const twist = _scavengergamesjs.ScavMods.twists[twistid];
			return _utils.Utils.html`<tr><td style="padding: 5px;">${twist.name}</td><td style="padding: 5px;">${twist.desc}</td></tr>`;
		}).join('');
		buffer += `</table>`;
		...
		this.sendReply(
			`|${isUhtmlChange ? 'uhtmlchange' : 'uhtml'}|scav-huntlogs|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Hunts Created</th><th>Total Hunts Created</th><th>History</th></tr>${
				data.map(entry => {
					...
					return `<tr><td>${entry.rank}</td><td><span style="color: ${color}">${auth || '&nbsp;'}</span>${_utils.Utils.escapeHTML(entry.name)}</td>` +
	...
	modsettings: {
		...
		'update'(target, room, user) {
			...
			this.checkCan('declare', null, room);
			const settings = _optionalChain([room, 'access', _70 => _70.settings, 'access', _71 => _71.scavSettings, 'optionalAccess', _72 => _72.scavmod]) || {};
