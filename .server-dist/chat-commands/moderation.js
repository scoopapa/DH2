 ...
 const commands = {
	...
	roompromote(target, room, user, connection, cmd) {
		...
		for (const toPromote of users) {
			...
			if (this.pmTarget && targetUser) {
				...
				_optionalChain([shouldPopup, 'optionalAccess', _14 => _14.popup, 'call', _15 => _15(`You were promoted to ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			} else {
				this.addModAction(`${name} was promoted to Room ${nextGroupName} by ${user.name}.`);
				this.modlog(`ROOM${nextGroupName.toUpperCase()}`, userid);
				_optionalChain([shouldPopup, 'optionalAccess', _16 => _16.popup, 'call', _17 => _17(`You were promoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			}
	...
	authority(target, room, user, connection) {
		if (target && target !== '+') {
			...
			const availableRoom = _optionalChain([targetRoom, 'optionalAccess', _18 => _18.checkModjoin, 'call', _19 => _19(user)]);
		...
		const buffer = _utils.Utils.sortBy(
			...
			([symbol, names]) => (
				...
				_utils.Utils.sortBy(names, name => toID(name)).join(", ")
			)
		);
	...
	roomauth(target, room, user, connection, cmd) {
		...
		const buffer = _utils.Utils.sortBy(
			...
			return `${group}:\n` +
				_utils.Utils.sortBy(names).map(userid => {
					const isOnline = _optionalChain([Users, 'access', _20 => _20.get, 'call', _21 => _21(userid), 'optionalAccess', _22 => _22.statusType]) === 'online';
	...
	userauth(target, room, user, connection) {
		...
		const targetUsername = _optionalChain([targetUser, 'optionalAccess', _23 => _23.name]) || target;
	...
	part(target, room, user, connection) {
		...
		if (!targetRoom) {
			if (target.startsWith('view-')) {
				_optionalChain([connection, 'access', _24 => _24.openPages, 'optionalAccess', _25 => _25.delete, 'call', _26 => _26(target.slice(5))]);
				if (!_optionalChain([connection, 'access', _27 => _27.openPages, 'optionalAccess', _28 => _28.size])) connection.openPages = null;
				return;
			}
			return this.errorReply(`The room '${target}' does not exist.`);
		}
	...
	unmute(target, room, user) {
		...
		const successfullyUnmuted = room.unmute(
			_optionalChain([targetUser, 'optionalAccess', _29 => _29.id]) || toID(this.targetUsername), `Your mute in '${room.title}' has been lifted.`
		);
	...
	ban(target, room, user, connection, cmd) {
		...
		if (targetUser.id in room.users || user.can('lock')) {
			targetUser.popup(
				`|modal||html|<p>${_utils.Utils.escapeHTML(user.name)} has banned you from the room ${room.roomid} ` +
				`${(room.subRooms ? ` and its subrooms` : ``)}${week ? ' for a week' : ''}.` +
				`</p>${(target ? `<p>Reason: ${_utils.Utils.escapeHTML(target)}</p>` : ``)}` +
	...
	async lock(target, room, user, connection, cmd) {
		...
		_optionalChain([room, 'optionalAccess', _30 => _30.hideText, 'call', _31 => _31([userid, toID(this.inputUsername)])]);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _32 => _32.battle])) this.parse('/savereplay forpunishment');
		return true;
	},
	lockhelp: [
	...
	unlock(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _33 => _33.namelocked])) {
			return this.errorReply(`User ${targetUser.name} is namelocked, not locked. Use /unnamelock to unnamelock them.`);
		}
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _34 => _34.locked]) && targetUser.locked.startsWith('#')) {
	...
	unlockip(target, room, user) {
		...
		for (const curUser of Users.findUsers([], [target])) {
			if (
				(range ? curUser.locked === '#rangelock' : !_optionalChain([curUser, 'access', _35 => _35.locked, 'optionalAccess', _36 => _36.startsWith, 'call', _37 => _37('#')])) &&
	...
	async globalban(target, room, user, connection, cmd) {
		...
		if (targetUser.trusted) {
			if (cmd === 'forceglobalban') {
				const from = targetUser.distrust();
				Monitor.log(`[CrisisMonitor] ${name} was globally banned by ${user.name} and demoted from ${_optionalChain([from, 'optionalAccess', _38 => _38.join, 'call', _39 => _39(", ")])}.`);
				this.globalModlog("CRISISDEMOTE", targetUser, ` from ${_optionalChain([from, 'optionalAccess', _40 => _40.join, 'call', _41 => _41(", ")])}`);
			} else {
		...
		_optionalChain([room, 'optionalAccess', _42 => _42.hideText, 'call', _43 => _43([userid, toID(this.inputUsername)])]);
	...
	promote(target, room, user, connection, cmd) {
		...
		const currentGroup = _optionalChain([targetUser, 'optionalAccess', _44 => _44.tempGroup]) || Users.globalAuth.get(userid);
	...
	declare(target, room, user) {
		...
		this.add(_utils.Utils.html`|raw|<div class="broadcast-blue"><b>${target}</b></div>`);
	...
	htmldeclare(target, room, user) {
		...
		for (const u in room.users) {
			_optionalChain([Users, 'access', _45 => _45.get, 'call', _46 => _46(u), 'optionalAccess', _47 => _47.sendTo, 'call', _48 => _48(
				room,
				`|notify|${room.title} announcement!|${_utils.Utils.stripHTML(target)}`
			)]);
		}
	...
	forcerename(target, room, user) {
		...
		if (_optionalChain([room, 'optionalAccess', _49 => _49.roomid]) !== 'staff') this.privateModAction(`${targetUser.name} ${forceRenameMessage}`);
		...
		Rooms.global.notifyRooms(
			['staff'],
			`|html|${roomMessage}` + _utils.Utils.html`<span class="username">${targetUser.name}</span> ${rankMessage} ${forceRenameMessage}`
		);
	...
	async namelock(target, room, user, connection, cmd) {
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _50 => _50.battle])) this.parse('/savereplay forpunishment');
	...
	unnamelock(target, room, user) {
		...
		const targetUser = Users.get(target);
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _51 => _51.namelocked])) {
	...
	hidetext(target, room, user, connection, cmd) {
		...
		if (/^[0-9]+\s*(,|$)/.test(target)) {
			if (hasLineCount) {
				let lineCountString;
				[lineCountString, target] = _utils.Utils.splitFirst(target, ',');
	...
	blacklist(target, room, user, connection, cmd) {
		...
		if (targetUser.id in room.users || user.can('lock')) {
			targetUser.popup(
				`|modal||html|<p>${_utils.Utils.escapeHTML(user.name)} has blacklisted you from the room ${room.roomid}${(room.subRooms ? ` and its subrooms` : '')}. Reason: ${_utils.Utils.escapeHTML(target)}</p>` +
	...
	groupchatban(target, room, user, connection, cmd) {
		...
		for (const roomid of createdGroupchats) {
			...
			const participants = _optionalChain([targetRoom, 'access', _52 => _52.warnParticipants, 'optionalCall', _53 => _53(
	...
	blacklistname(target, room, user) {
		...
		const duplicates = targets.filter(userid => (
			...
			_optionalChain([Punishments, 'access', _54 => _54.roomUserids, 'access', _55 => _55.nestedGet, 'call', _56 => _56(room.roomid, userid), 'optionalAccess', _57 => _57[0]]) === 'BLACKLIST'
		));
	...
	showblacklist(target, room, user, connection, cmd) {
		...
		let buf = _utils.Utils.html`Blacklist for ${room.title}${soonExpiring ? ` (expiring within 3 months)` : ''}:<br />`;
