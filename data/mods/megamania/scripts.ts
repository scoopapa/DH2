export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
		...
		runMegaEvo(pokemon) {
			...
			// @ts-ignore
			const species: Species = this.dex.species.get(pokemon.canMegaEvo, pokemon);

			// Do we have a proper sprite for it?
			if (this.dex.species.get(pokemon.canMegaEvo!).baseSpecies) {
				pokemon.formeChange(species, pokemon.getItem(), true);
			} else {
				const oSpecies = this.dex.species.get(pokemon.canMegaEvo);
				// @ts-ignore
				const oMegaSpecies = this.dex.species.get(pokemon.canMegaEvo);
				pokemon.formeChange(species, pokemon.getItem(), true);
