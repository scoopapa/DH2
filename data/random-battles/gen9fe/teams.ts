...
export class RandomTeams {
	...
	randomMoveset(
		...
		if (movePool.includes('facade') && abilities.has('Adrenaline Aroma')) {
	...
	getPriorityItem(
		...
		if (
			!isLead && 
			((species.id === 'irontornado' && moves.has('acrobatics')) ||
			  species.id === 'ironmimic' ||
			 (species.id === 'irondirge' && role === 'Setup Sweeper'))
		) {
		...
		switch (species.id) {
			...
			case 'solvaliantmega':
				return 'Absolite';
			case 'necrotrikultra':
				return 'Depleted Ultranecrozium Z';
			case 'hatteponhearthflame':
				return 'Heartflame Mask';
			case 'hatteponwellspring':
		...
		if ((ability === 'Adrenaline Aroma' || moves.has('facade')) && !moves.has('sleeptalk')) {
	...
	randomTeam() {
		...
		while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (set.ability === 'Sand Stream' || set.ability === 'Sand Wrath') teamDetails.sand = 1;
			if (set.ability === 'Snowblind' || set.moves.includes('snowscape') || set.moves.includes('chillyreception')) {
				teamDetails.snow = 1;
			}
			...
			if (set.role === 'Tera Blast user' || species.baseSpecies === "Hattepon" || species.baseSpecies === "Terapagos") {
