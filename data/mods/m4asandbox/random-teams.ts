...
export class RandomTeams {
	...
	randomSet(species: string | Species, teamDetails: RandomTeamsTypes.TeamDetails = {}, isLead = false, isDoubles = false): RandomTeamsTypes.RandomSet {
		...
		else if (species.name.endsWith('-Mega-X') || species.name.endsWith('-Mega-Y') || species.name.endsWith('-Mega-U')) {
	...
	randomTeam() {
		...
		for (const restrict of [true, false]) {
			...
			while (pokemonPool.length && pokemon.length < 6) {
				...
				const isMega = (species.name.endsWith('-Mega') || species.name.endsWith('-Mega-Y') || species.name.endsWith('-Mega-X') || species.name.endsWith('-Mega-U'));
