				...
				if (set.moves) {
					...
					for (const moveID of set.moves) {
						...
						if (pokeLearnsMove == "8D") {
							...
							if (pokemon.changesFrom && pokemon.name !== pokemon.changesFrom) { // There is a base forme
								const baseLearns = this.dex.getLearnsetData(this.dex.species.get(pokemon.changesFrom).id).learnset[moveID];
							...
							if (prevo) {
								const prevoLearns = this.dex.getLearnsetData(prevo.id).learnset[moveID];
								...
								if (prevoLearns == "8D") { // This move is prevo's Hidden Move
									...
									if (first) {
										const firstLearns = this.dex.getLearnsetData(first.id).learnset[moveID];
					...
					if (isHidden) { // Pokemon knows a Hidden Move, therefore we must ensure no one else in its family knows one
						...
						if (base.evos) {
							for (const evo of base.evos) {
								if (!family.includes(evo)) {
									...
									if (evoMon.otherFormes) {
										for (const evoForme of evoMon.otherFormes) {
											if (!family.includes(evoForme)) {
												...
												if (evoMonForme.evos) {
													for (const evoFormeFinal of evoMonForme.evos) {
							...
							for (const forme of base.otherFormes) {
								// console.log("Adding " + forme);
								family.push(forme);
								const formeMon = this.dex.species.get(forme);
								if (formeMon.evos) {
									for (const formeEvo of formeMon.evos) {
										if (!family.includes(formeEvo)) {
											...
											if (formeEvoMon.evos) {
												for (const formeEvoFinal of formeEvoMon.evos) {
											...
											if (formeEvoMon.otherFormes) {
												for (const formeEvoForme of formeEvoMon.otherFormes) {
						...
						for (const poke of learnedHiddenTable) {
							...
							{ problems.push(`No more than one ${pokemon.baseSpecies} can know its Hidden Move.`); } else if (family.includes(poke.name)) { problems.push(`${poke.name} and ${pokemon.name} cannot both know their Hidden Moves because they are in the same family.`); }
						}
