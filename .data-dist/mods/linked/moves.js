	...
	encore: {
		...
		effect: {
			...
			onStart(target) {
				...
				if (!lastMove || target.volatiles['dynamax']) return false;
				if ((lastMove ).isZOrMaxPowered) lastMove = this.dex.getMove(lastMove.baseMove);
				// @ts-ignore
	...
	iceball: {
		...
		effect: {
			...
			onResidual(target) {
				...
				const lastMove = target.m.lastMoveAbsolute;
				if (_optionalChain([lastMove, 'optionalAccess', _ => _.id]) === 'struggle') {
	...
	rollout: {
		...
		effect: {
			...
			onResidual(target) {
				// This is just to ensure the volatile is deleted correctly
				const lastMove = target.m.lastMoveAbsolute;
				if (_optionalChain([lastMove, 'optionalAccess', _2 => _2.id]) === 'struggle') {
